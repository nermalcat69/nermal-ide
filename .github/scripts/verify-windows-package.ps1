...
#   core::update::package_for_current_install    — nermal-updater.exe beside the app
#   nermal-updater `windows::verify_portable_payload`  — portable layout + versions
#   nermal-updater `windows::extract_portable_archive` — ZIP entry rules
#   nermal-updater `windows::verify_file_version`      — setup.exe PE version
#
...
$Name  = "nermal-$Version-windows-$Arch"
$Zip   = "dist/$Name.zip"
...
if (-not (Test-Path -LiteralPath $Zip)) {
    ...
    $managed = @(
        'nermal-app.exe', 'nermal.exe', 'nermal-updater.exe', '.nermal-portable',
    ...
    if ($entries -contains '.nermal-inno-install') {
        Fail "the portable archive carries the Inno install marker"
    }

    $unzipped = Join-Path ([System.IO.Path]::GetTempPath()) "nermal-verify-portable-$([guid]::NewGuid())"
    ...
    try {
        ...
        foreach ($required in @('nermal-app.exe', 'nermal.exe', 'nermal-updater.exe',
                                '.nermal-portable', 'LICENSE.txt', 'README.md')) {
        ...
        $markerPath = Join-Path $unzipped '.nermal-portable'
        ...
        Assert-BinaryVersion (Join-Path $unzipped 'nermal-app.exe') 'the portable nermal-app.exe'
        Assert-BinaryVersion (Join-Path $unzipped 'nermal-updater.exe') 'the portable nermal-updater.exe'
...
if (-not (Test-Path -LiteralPath $Stage -PathType Container)) {
    ...
    if (-not (Test-Path -LiteralPath (Join-Path $Stage '.nermal-inno-install') -PathType Leaf)) {
        Fail "the Inno payload is missing the .nermal-inno-install marker; installed copies would never be offered an in-app update"
    }
    if (Test-Path -LiteralPath (Join-Path $Stage '.nermal-portable')) {
        Fail "the Inno payload carries the portable marker, which would misroute the updater"
    }
    if (-not (Test-Path -LiteralPath (Join-Path $Stage 'nermal-updater.exe') -PathType Leaf)) {
        Fail "the Inno payload is missing nermal-updater.exe"
    }
    Assert-BinaryVersion (Join-Path $Stage 'nermal-app.exe') 'the installed nermal-app.exe'
    Assert-BinaryVersion (Join-Path $Stage 'nermal-updater.exe') 'the installed nermal-updater.exe'
...
Write-Output "Windows package verified: markers, nermal-updater.exe and versions match $Version"
