; nermal Windows installer (Inno Setup 6 — preinstalled on GitHub's
...
;
; Defaults to a per-user install ({localappdata}\Programs\nermal — no UAC
...
WizardStyle=modern
; The persistent daemon (nermal-app.exe --daemon) is a detached background process
; that outlives the GUI and holds the running image of nermal-app.exe, so Windows
...
; Windows treat theirs — not a runtime preference, so nermal has no setting for
...
Name: "explorermenu"; Description: "Add ""Open in nermal"" to the folder context menu"; GroupDescription: "Shell integration:"; Flags: unchecked

; Builds before the nermal/nermal-app split installed the GUI as nermal.exe. Upgrading
; only *adds* nermal-app.exe, so the old binary would stay on disk — and a taskbar
...
[Files]
Source: "{#StageDir}\nermal-app.exe"; DestDir: "{app}"; Flags: ignoreversion
...
Source: "{#StageDir}\nermal.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "{#StageDir}\nermal-updater.exe"; DestDir: "{app}"; Flags: ignoreversion skipifsourcedoesntexist
...
Source: "{#StageDir}\.nermal-inno-install"; DestDir: "{app}"; Flags: ignoreversion skipifsourcedoesntexist
...
; {app}\nermal-app.exe). The two files are a matched pair — never update one
...
; The Linux musl nermal-server, for serving WSL distros without a download.
...
; AppUserModelID is what lets toast notifications carry the nermal name and icon
...
Filename: "{app}\nermal-app.exe"; Parameters: "--stop-daemon --update-install-dir ""{app}"""; Flags: runhidden waituntilterminated; RunOnceId: "StopDaemon"
...
Filename: "{app}\nermal-app.exe"; Parameters: "--unregister-explorer-menu"; Flags: runhidden waituntilterminated; RunOnceId: "UnregisterExplorerMenu"

[Code]
(* Gracefully stop the persistent daemon before we overwrite nermal-app.exe. We can't
  ...
  *new* nermal-app.exe to {tmp} and run that. It connects to the running daemon, hangs
...
var
  ResultCode: Integer;
begin
  ExtractTemporaryFile('nermal-app.exe');
...
(* Take {app} back out of the user's PATH.
  ...
  users ran nermal, this clears the one uninstalling — the others keep a dead
...
(* Remove the optional Explorer verbs if this user enabled them in Settings.
  ...
  The application owns only the final `nermal` subkeys. Deleting those trees
