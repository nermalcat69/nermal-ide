...
# Package the release binary into dist/nermal.app, then publish both:
#   dist/nermal-<version>-macos-<arch>.zip  (in-app updater)
#   dist/nermal-<version>-macos-<arch>.dmg  (drag-to-Applications install)
#
...
PACKAGE_UPDATE_ZIP="${nermal_PACKAGE_UPDATE_ZIP:-1}"
APP="dist/nermal.app"

rm -rf dist
mkdir -p "$APP/Contents/MacOS" "$APP/Contents/Resources"
cp "target/${TARGET}/release/nermal-app" "$APP/Contents/MacOS/nermal-app"
chmod +x "$APP/Contents/MacOS/nermal-app"
...
# sits next to nermal-app under MacOS/ — that is the directory the GUI resolves
# relative to its own executable.
cp "target/${TARGET}/release/nermal" "$APP/Contents/MacOS/nermal"
chmod +x "$APP/Contents/MacOS/nermal"
if [[ "$PACKAGE_UPDATE_ZIP" != "0" ]]; then
    ...
    cp "target/${TARGET}/release/nermal-updater" "$APP/Contents/MacOS/nermal-updater"
    chmod +x "$APP/Contents/MacOS/nermal-updater"
fi
cp assets/nermal.icns "$APP/Contents/Resources/nermal.icns"
...
<dict>
    ...
    <key>CFBundleExecutable</key><string>nermal-app</string>
    <key>CFBundleIconFile</key><string>nermal</string>
    <key>CFBundlePackageType</key><string>APPL</string>
    <key>NSHighResolutionCapable</key><true/>
    <key>NSPrincipalClass</key><string>NSApplication</string>
    <!-- LaunchServices has no global default-terminal setting. These declare
         the specific document types nermal can open, and Settings lets users
         select nermal as their handler for them. -->
    ...
    <!-- nermal is a terminal workbench: panes are forked from the bundled
         executable, so macOS attributes a child process's protected-resource
         requests to nermal.app. Without these usage strings a program you run in
    ...
    <string>A program running inside nermal would like to access the camera.</string>
    <key>NSMicrophoneUsageDescription</key>
    <string>A program running inside nermal would like to access the microphone.</string>
    <key>NSContactsUsageDescription</key>
    <string>A program running inside nermal would like to access your contacts.</string>
    <key>NSCalendarsFullAccessUsageDescription</key>
    <string>A program running inside nermal would like to access your calendar data.</string>
    <key>NSRemindersFullAccessUsageDescription</key>
    <string>A program running inside nermal would like to access your reminders.</string>
    <key>NSPhotoLibraryUsageDescription</key>
    <string>A program running inside nermal would like to access your photo library.</string>
    <key>NSLocationUsageDescription</key>
    <string>A program running inside nermal would like to access your location information.</string>
    <key>NSMotionUsageDescription</key>
    <string>A program running inside nermal would like to access motion data.</string>
    <key>NSLocalNetworkUsageDescription</key>
    <string>A program running inside nermal would like to access the local network.</string>
    <key>NSBluetoothAlwaysUsageDescription</key>
    <string>A program running inside nermal would like to use Bluetooth.</string>
    <key>NSSpeechRecognitionUsageDescription</key>
    <string>A program running inside nermal would like to use speech recognition.</string>
    <key>NSSystemAdministrationUsageDescription</key>
    <string>A program running inside nermal requires elevated privileges.</string>
    <key>NSAppleEventsUsageDescription</key>
    <string>A program running inside nermal would like to control other applications via Apple Events.</string>
</dict>
</plist>
PLIST
...
if [[ -n "$SIGN_ID" && -n "${APPLE_CERTIFICATE:-}" ]]; then
    ...
    KEYCHAIN="${RUNNER_TEMP:-/tmp}/nermal-sign.keychain-db"
    CERT_PATH="${RUNNER_TEMP:-/tmp}/nermal-cert.p12"
    KEYCHAIN_PASSWORD="${KEYCHAIN_PASSWORD:-nermal-ci}"
...
<dict>
    ...
    <!-- Deliberately nothing beyond those three, and in particular no TCC
         ...
         are about a *child* process's request: macOS attributes it to nermal.app
         ...
         So camera / microphone / location / apple-events on nermal.app would do
         nothing for a pane, while widening what injected code could reach
         under nermal's identity; this bundle already carries
...
ENT
    ...
    codesign --force --options runtime --timestamp \
        --sign "$SIGN_ID" "$APP/Contents/MacOS/nermal"
    if [[ "$PACKAGE_UPDATE_ZIP" != "0" ]]; then
        codesign --force --options runtime --timestamp \
            --sign "$SIGN_ID" "$APP/Contents/MacOS/nermal-updater"
    fi
    codesign --force --options runtime --timestamp --entitlements "$ENTITLEMENTS" \
        --sign "$SIGN_ID" "$APP/Contents/MacOS/nermal-app"
    codesign --force --options runtime --timestamp --entitlements "$ENTITLEMENTS" \
        --sign "$SIGN_ID" "$APP"
...
# standalone nermal-server asset, so a helper built without --target, a dylib
...
# an artifact — and before the `mv` below, after which dist/nermal.app no longer
...
BUNDLED_BINS=(nermal-app nermal)
if [[ "$PACKAGE_UPDATE_ZIP" != "0" ]]; then
    BUNDLED_BINS+=(nermal-updater)
fi
...
ZIP=""
if [[ "$PACKAGE_UPDATE_ZIP" != "0" ]]; then
    ZIP="dist/nermal-${VERSION}-macos-${ARCH}.zip"
    ditto -c -k --keepParent "$APP" "$ZIP"
fi

# Package the (now stapled) bundle as a drag-to-Applications DMG.
DMG="dist/nermal-${VERSION}-macos-${ARCH}.dmg"
...
# dist/nermal.app after this point — the zip above is what the updater ships and
# what nightly.yml verifies (it extracts that, not this), and release.yml only
# knows about nermal.app as an intermediate to keep out of the upload globs.
mv "$APP" "$STAGE/"
...
# that message is under /Volumes/nermal, not on the host: three nightlies died
...
hdiutil create -volname "nermal" -srcfolder "$STAGE" -ov -format UDZO \
