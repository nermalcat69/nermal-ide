...
#   dist/nermal-<version>-windows-<arch>.zip        portable (unzip anywhere)
#   dist/nermal-<version>-windows-<arch>-setup.exe  Inno Setup installer
...
# executable as a resource (see build.rs). So the payload is nermal-app.exe plus a
...
$Stage = "dist/$Name"
$PackageUpdater = $env:nermal_PACKAGE_UPDATE_HELPER -ne '0'

Remove-Item -Recurse -Force dist -ErrorAction SilentlyContinue
New-Item -ItemType Directory -Force -Path $Stage | Out-Null

Copy-Item "target/$Target/release/nermal-app.exe" "$Stage/nermal-app.exe"
# The CLI, staged beside the GUI so both the zip and the installer carry it.
# `core::cli_install` resolves it relative to nermal-app.exe and puts that
# directory on the user's PATH.
Copy-Item "target/$Target/release/nermal.exe" "$Stage/nermal.exe"
if ($PackageUpdater) {
    ...
    Copy-Item "target/$Target/release/nermal-updater.exe" "$Stage/nermal-updater.exe"
}
...
# `nermal-app.exe --daemon`, so beside it is exactly here. The in-box host
...
# The Linux musl `nermal-server`, staged at server/ so a WSL distro can be handed
...
# <dir of nermal-app.exe>/server/<asset> first — so this directory name is not free to
...
$ServerAsset = "nermal-server-linux-x86_64-musl"
...
if ($PackageUpdater) {
    Set-Content -Path "$Stage/.nermal-portable" -Value 'portable-v1' -NoNewline -Encoding ascii
}
...
if ($PackageUpdater) {
    ...
    Remove-Item -LiteralPath "$Stage/.nermal-portable" -Force
    Set-Content -Path "$Stage/.nermal-inno-install" -Value 'inno-v1' -NoNewline -Encoding ascii
}
