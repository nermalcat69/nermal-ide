...
another. nermal gives that loop a primitive instead of leaving it to screen
scraping.
...
# 2. hand it a task
nermal send "$PANE" 'claude -p "add tests for the parser"' --enter

# 3. sleep until it needs you or finishes
nermal wait "$PANE" --until waiting,done --changed --timeout 600
...
nermal wait "$PANE" --until free --changed --timeout 900
...
<Warning>
  ...
  own root process *is* the command — what `nermal run` spawns — looks free the
  whole time it runs; wait on `nermal run` itself instead, it already blocks. And
...
```bash
nermal wait "$PANE" --until waiting --changed   # it needs something
nermal capture "$PANE" --plain | tail -20       # see what it is asking
nermal send "$PANE" --key down --key enter      # answer it
nermal send "$PANE" --key C-c                   # or stop it altogether
```
...
[full vocabulary](/cli/reference#nermal-send-pane-text-enter-key-key) is in the
reference.

## When an agent never moves

If `nermal wait` times out and `nermal agents` shows a status that never changes,
the likely cause is that the agent's status hooks are not installed or are out
of date — the agent is working fine, it just has no way to say so. `nermal doctor`
reports where every agent's hooks stand, and `nermal agents` names the specific
one when it can see the gap.

## Watching everything at once

```bash
nermal agents          # every agent on the machine: pane, agent, status, message
nermal agents --json   # the same, parseable
```
...
nermal installs nothing into `~/.claude` for it — no switch in **Settings →
...
```bash
npx skills add nermalcat69/nermal-ide
```
...
- **Never `send` into a pane you did not open.** Check `nermal agents` first.
- **Never close a pane, tab, or workspace you did not create.**
- **Never `server stop` or `server restart`.** Every pane on the machine dies
  with it.
- **Clean up what you did create** — `nermal pane close %83` when you are done.
  An interrupted `run` leaves its pane behind; `nermal pane ls --all` shows those,
  and `nermal pane close --orphans` clears them. That last one is a human's
