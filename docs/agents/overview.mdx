---
title: "Coding agents"
description: "What nermal does around Claude Code, Codex, TraeCode, and 19 others — without ever wrapping them."
---

nermal recognises coding agents running in a pane and builds around them. It does
not wrap them, proxy them, or replace their interface: the agent you start is
the agent you get, running in a normal PTY, with its own UI. nermal adds the
...
<Frame caption="Placeholder — screenshot: a sidebar of agent sessions across several repos, each with a brand avatar and status dot">
  <img src="/images/placeholder.svg" alt="Agent sessions in the nermal sidebar" />
</Frame>
...
itself, over a channel nermal installs into that agent's configuration. It powers
the status dots, the notifications, the tray icon, and `nermal wait`.
...
<CardGroup cols={3}>
  ...
  <Card title="Orchestration" icon="diagram-project" href="/agents/orchestration">
    One agent driving another with `nermal wait`.
  </Card>
</CardGroup>
