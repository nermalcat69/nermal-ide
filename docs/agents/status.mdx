...
waiting for permission look identical from outside. nermal fixes that by letting
...
becomes **Reinstall** — or **Update**, against an **Outdated** state, when nermal
ships a newer hook.
...
<Note>
  ...
  carries no status dot and `nermal wait` times out rather than returning.
</Note>

<Note>
  The hooks only do anything inside nermal. Running the same agent in another
...
The same three states are what `nermal agents` reports as `working` / `waiting` /
`done`, and what [`nermal wait`](/agents/orchestration) blocks on. A fourth state,
...
nermal keeps a status item in the system tray (menu bar on macOS). It flips to an
...
If nothing recognisable is running, nermal says *"No running coding agent found"*
