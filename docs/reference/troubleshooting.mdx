...
## `nermal: command not found`

The CLI is put on PATH the first time the app launches. If it is missing:

- Check **Settings → Agents → Install the nermal command on PATH** is on.
...
- A `nermal` you installed yourself is never replaced, so an old one earlier in
  PATH will win.

Inside a nermal pane it works regardless, since panes inherit the app's
environment.

## The server is unreachable

`nermal doctor` says so, and the GUI cannot open panes.

Start it with `nermal server start`. If you are an agent or a script,
...
```bash
nermal_LOG=info   # must be set before the server starts
nermal server logs
```
...
unavoidable. The *screens* should come back: nermal keeps a capped tail of each
...
nermal updated in place, so the app is new and your panes are still served by the
...
|---|---|
| *"running an old nermal server that this copy cannot talk to"* | The server there predates your client's protocol. Let nermal update it — this ends every session on that machine. |
| *"running a newer nermal server than this copy"* | Update nermal here instead, or replace the server there. |
| *"answered, but not as a nermal server"* | Something else is listening, or the binary is not what nermal expects. |
| *"nermal no longer has a way to reach &lt;machine&gt;"* | The SSH link dropped. Reconnect from the switcher. |

`nermal -m <machine>` never dials a fresh connection by design — it uses a link
...
The usual cause is a shell you started by hand. nermal injects its
...
**Make it the shell nermal starts.** On the remote host:

```sh
chsh -s /bin/zsh      # then reconnect
```

nermal asks the host for `$SHELL` at connect time and bootstraps *that* shell,
...
`.bashrc`, does not help: both run after (or instead of) the shell nermal set
up.
...
```zsh
# Report the working directory to nermal (OSC 7).
autoload -Uz add-zsh-hook
__nermal_cwd() { printf '\e]7;file://%s%s\a' "${HOST:-localhost}" "${PWD//\%/%25}" }
add-zsh-hook precmd __nermal_cwd
```

It is safe to leave in place afterwards: when nermal *is* integrating that
shell, the two hooks simply report the same directory twice. The bash
equivalent is `PROMPT_COMMAND`:

```bash
__nermal_cwd() { printf '\e]7;file://%s%s\a' "${HOSTNAME:-localhost}" "${PWD//%/%25}"; }
PROMPT_COMMAND="__nermal_cwd${PROMPT_COMMAND:+;$PROMPT_COMMAND}"
```

Two other reasons a pane never reports its directory:

- **The shell is not one nermal integrates.** It injects into zsh, bash, fish,
...
Where nermal can see the pane's processes it falls back to reading the
...
If the file cannot be parsed, nermal starts on defaults and keeps your original at
...
## `nermal capture … | head -1` printed a Rust panic
...
<CardGroup cols={2}>
  ...
  <Card title="Report an issue" icon="github" href="https://github.com/nermalcat69/nermal-ide/issues/new">
    Include `nermal doctor` output and your platform.
  </Card>
</CardGroup>
