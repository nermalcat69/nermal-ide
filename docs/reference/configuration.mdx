---
title: "config.json"
description: "Every key nermal reads, its type, and its default."
---

| | |
|---|---|
| macOS / Linux | `~/.config/nermal/config.json` |
| Windows | `%APPDATA%\nermal\config.json` |
| Override the whole directory | `nermal_CONFIG_DIR` |
...
| Key | Type | Default | |
|---|---|---|---|
| `prompt_editor` | bool | `true` | nermal edits the shell prompt. Off sends every key at the prompt to the shell, so its own line editor (ZLE, readline) owns editing. Shell integration stays on. |
| `tab_completion` | bool | `true` | nermal's completion menu on <kbd>⇥</kbd>. Off hands the key to the shell. Needs `prompt_editor`. |
| `history_search` | bool | `true` | nermal's fuzzy history on <kbd>⌃ R</kbd>. Off hands the key to the shell. Needs `prompt_editor`. |
...
| `link_file_open` | string | `internal` | What a file link opens: `internal` (nermal's editor, at the line), `system` (the OS file association), `command`. A file on another machine always uses `internal`. |
...
| `install_cli_on_path` | bool | `true` | Put the bundled `nermal` command on PATH at launch. |
...
| `http_proxy` | string | — | For nermal's *own* traffic only — update checks, downloads, remote-server installs. `http://…` or `socks5://…`. Programs in a pane are unaffected. |

[Updates →](/reference/updates)

## Keys nermal manages itself
...
<Note>
  If the file cannot be parsed, nermal starts on defaults, keeps your original at
