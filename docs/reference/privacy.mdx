...
description: "What macOS asks you, why, and what nermal itself holds."
---

## Why macOS asks nermal for permission

Panes are forked from nermal's own bundled executable, so when a program you run
asks macOS for a protected resource, macOS attributes the request to **nermal.app**
— not to the program.

If nermal declared no usage strings, that request would be **denied outright with
no prompt at all**, and the program would look broken for no visible reason.

So nermal declares the matching usage strings, and you get the normal one-time
prompt:
...
<Warning>
  **Declaring a usage string is not the same as holding the permission.**
  nermal.app itself is granted none of these. Every prompt you see belongs to
...
| **Remote server installs** | Downloading a `nermal-server` binary for a machine you connected to — or, for WSL, copying the one already bundled with your install. |
...
| | |
|---|---|
| Settings, themes, window state | `~/.config/nermal/` (`%APPDATA%\nermal\` on Windows) |
