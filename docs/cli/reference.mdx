...
Set inside every nermal pane, inherited by anything launched from one.

| Variable | Meaning |
|---|---|
| `nermal_PANE` | This pane's id, e.g. `71` or `%71` (both accepted). Default target of `split`, `send`, `capture`, `procs`, `wait`, `pane close`. |
| `nermal_WS` | This pane's workspace id. Default for `run --keep`, `tab new`, `tab ls`, `ws tree`. |
| `nermal_CONFIG_DIR` | The server's config dir — how the CLI finds the right server. You never pass a socket path. |

Outside a nermal shell, address-taking verbs fail with
`not inside a nermal shell — pass an explicit %pane/@tab/workspace`.
...
|---|---|
| `0` | Success |
| `1` | The command failed; one line on stderr, prefixed `nermal:` |
| `2` | Usage error — unknown verb, missing argument, bad type |
| `124` | `nermal wait` timed out (the `timeout(1)` convention) |
...
| *other* | Only from `nermal run`, which passes the child's exit code through |
...
### `nermal run [--keep] [--cwd DIR] [--ws WORKSPACE] -- CMD...`
...
- `--keep` leaves the pane alive as a new tab afterwards. Needs a workspace, so
  it requires `--ws` or `$nermal_WS` — without one it is an error, not a silent
  fallback.
- `--cwd` sets the working directory. `--ws` also sets the pane's `nermal_WS`.
...
### `nermal split [%PANE] (--v|--h) [--ratio R]`

Alias of `pane split`. Splits `%PANE` (default `$nermal_PANE`), spawning a shell
...
### `nermal send [%PANE] [TEXT] [--enter] [--key KEY]…`
...
none, so `nermal send %42 --enter` runs whatever pane 42 already has typed. With
one argument the text is the argument and the pane comes from `$nermal_PANE` —
...
to do. `--enter` is that key only for the `%`-marked spelling: `nermal send 83
...
type an address-shaped string, name the pane as well: `nermal send %42 %3x`.
...
### `nermal capture [%PANE] [--plain] [--scrollback] [--tail N]`
...
### `nermal wait [%PANE] [--until STATE,…] [--changed] [--timeout SECS] [--interval MS]`
...
nothing, so `nermal agents` shows it standing still and `nermal wait` sits there
...
prefix `nermal ls` prints). An ambiguous name or prefix is an error that lists the
candidates.
...
Prefer `nermal new <path>` over `ws new` when you want something usable: `ws new`
leaves an empty workspace you then have to populate, while `nermal new --json`
...
With no `WORKSPACE` and no `nermal_WS`, the pane goes back to the workspace it was
...
| Command | Effect |
|---|---|
| `server status` | Same as `nermal status` |
| `server logs` | Tail the server log; prints the path, and says so when logging was never enabled (`nermal_LOG=info` before the server starts) |
