...
description: "Teaching a coding agent to use nermal properly — including when not to."
---
...
The source lives at
[`skills/nermal/`](https://github.com/nermalcat69/nermal-ide/tree/main/skills/nermal) in the
...
<Note>
  This is the only skill nermal has — nothing in **Settings → Agents** installs
  one for you. Using the CLI to run *other* agents — a worker pane, `nermal wait`,
...
command, and the skill is named `nermal`:

```bash
npx skills update nermal    # this one
...
has drifted. `npx skills remove nermal` takes it back out.
...
- **Never `send` into a pane you did not open.** Keystrokes land in the middle
  of whatever is happening there. Check `nermal agents` first.
...
- **Never `nermal server start` on your own initiative** when `doctor` says the
  server is unreachable — starting one the user did not ask for changes what
  their GUI attaches to. Tell them instead.
- **Clean up what you did create.** `nermal pane close %83` when the scratch pane
  is done with.
...
nermal wait %3 --until waiting,done --changed --timeout 600
```
...
command actually started, babysit with `nermal wait`, collect the result out of
...
shortest description of how to use nermal as a job runner. Start with the
