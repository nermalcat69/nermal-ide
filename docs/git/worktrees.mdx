...
working tree. A git worktree is the fix, and nermal makes it a single dialog.
...
Confirm and nermal creates the worktree, opens a tab in it, and starts a shell
...
```
<repo>/.nermal/worktrees/<name>
```

`<repo>/.nermal/.gitignore` is created with `*` in it the first time, so the
