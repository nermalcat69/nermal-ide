...
Four words explain most of nermal. Three of them you can see; the fourth is the
reason the other three survive a reboot.

## Pane

A **pane** is one terminal: one shell (or one program) attached to one PTY. It
is the only thing in nermal that actually runs something.

Panes have stable ids — `%42` — for their whole life. That id is what the
[CLI](/cli/overview) addresses, and what `$nermal_PANE` holds inside the pane
itself.
...
label is the best evidence nermal has: a name you set, else the coding agent
...
Workspaces are how nermal keeps ten repositories from becoming forty
...
<Frame caption="Placeholder — screenshot: the workspace switcher, workspaces on the left, that workspace's tabs on the right">
  <img src="/images/placeholder.svg" alt="The nermal workspace switcher" />
</Frame>
...
Quitting nermal closes the window and leaves that server running. Your build keeps
building, your agent keeps working, your SSH session stays up. Open nermal again
and it reattaches to exactly what was there.

This is also why:

- **`nermal` works from any terminal.** The CLI talks to the same server. The GUI
...
| | Close a tab | Quit nermal | Stop the server | Reboot |
|---|:--:|:--:|:--:|:--:|
...
|---|---|
| `nermal_PANE` | This pane's id — the default target of `nermal split`, `send`, `capture`, `procs`. |
| `nermal_WS` | This pane's workspace id. |
| `nermal_CONFIG_DIR` | The config directory, which is how the CLI finds the right server. |

`echo $nermal_PANE` is the fastest way to tell whether you are inside nermal at all.
