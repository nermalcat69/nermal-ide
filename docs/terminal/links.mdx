...
across two lines — nermal stitches it back together before opening it.
...
opens in nermal's own editor, on the line the link named, and the Files panel
...
| Mode | Opens in |
| --- | --- |
| Built-in editor | nermal's editor, at `:line:column`. The default. |
...
When the pane is inside an SSH session it usually is not, so nermal opens a port
