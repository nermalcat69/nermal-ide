...
nermal puts an editor at the shell prompt. Nothing to install, no plugin to source
...
<Frame caption="Placeholder — video: typing a command with a ghost suggestion, accepting it, then opening the completion menu">
  <img src="/images/placeholder.svg" alt="The nermal prompt" />
</Frame>
...
When nermal has nothing useful to offer, the <kbd>⇥</kbd> falls through to your
...
The prompt features arrive through nermal's shell integration, which is injected
...
readout, the "command finished" notification, and `nermal procs` are built on.
...
it does outside nermal:

```zsh
# ~/.zshrc — works at a nermal prompt with the editor off
...
Settings while it is off — both are menus nermal opens inside that editor, so
...
branch readout, `nermal procs`. The setting moves the line editor, not the
integration.
