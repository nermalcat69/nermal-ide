...
nermal speaks SSH itself, over [russh](https://github.com/Eugeny/russh). It never
...
<Frame caption="Placeholder — screenshot: an SSH connection sheet asking for a key passphrase inside a pane">
  <img src="/images/placeholder.svg" alt="Connecting over SSH in nermal" />
</Frame>
...
<AccordionGroup>
  ...
  <Accordion title="An alias from ~/.ssh/config">
    Type an alias you already have and nermal resolves it natively — common fields,
...
clipboard on the machine running nermal with the OSC 5522 clipboard protocol.
...
if len(data) > 16 * 1024 * 1024:
    raise SystemExit("image exceeds nermal's 16 MiB clipboard limit")
...
Run `nermal-copy-image screenshot.png`. A compliant sender may include an OSC
5522 request id and wait for nermal's `DONE`, `EPERM`, `EINVAL`, or `ENOSYS`
