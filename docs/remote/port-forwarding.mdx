...
When a pane is on a remote machine, nermal watches what its processes are
...
<Note>
  In a remote workspace the ports are found by the **`nermal-server` on that
  ...
  belong to no nermal server, so nothing lists them.
</Note>
...
<Note>
  These proxy settings are for reaching the SSH server. nermal's *own* network
