...
pub fn git_watch_dirs(
    ...
    use nermal_core::host::default_join;
...
impl NermalApp {
    ...
    /// As [`NermalApp::scm_invalidate`], for a caller that knows a directory
    ...
    /// cannot supply — offer the one thing nermal has that a GUI does not: a
...
fn scm_watch_dirs(host: &dyn Host, root: &Path) -> Option<Vec<PathBuf>> {
    ...
    let heads = nermal_core::host::default_join(
        &nermal_core::host::default_join(&common_dir, "refs", sep),
        "heads",
        sep,
    );
...
mod tests {
    ...
    /// The same pin list `nermal_core::core::git::test_support` keeps for the
    /// four modules over there. It cannot be shared with them: those helpers
    /// are `#[cfg(test)]`, so they do not exist in the `nermal-core` this crate
