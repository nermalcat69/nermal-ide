...
fn normalize(raw: Vec<Raw>) -> History {
    let mut raw = raw;
    // Sources are concatenated (shell files, then nermal's own file), so vector
    ...
    // recorded in nermal last week, or ↑ shows the wrong line. Untimestamped
...
fn load_shell_history() -> Vec<Raw> {
    ...
    for path in files {
        if let Ok(bytes) = std::fs::read(&path) {
            ...
            // against zsh/fish/nermal records that do carry one — otherwise a
            // month-old nermal file concatenated last always wins ↑.
...
mod tests {
    ...
    fn normalize_orders_by_timestamp_not_by_which_file_was_concatenated_last() {
        // load() appends nermal's own file after the shell histories, so without
        // a timestamp sort an old nermal record becomes "most recent" and ↑
        ...
        assert_eq!(h.entries, ["old nermal", "recent shell"]);
    }
    ...
    fn file_mtime_secs_reads_a_real_file_and_gives_up_on_a_missing_one() {
        let dir = std::env::temp_dir().join(format!("nermal-hist-mtime-{}", std::process::id()));
    ...
    fn append_then_load_recovers_the_command_and_metadata() {
        ...
        let unique = format!("nermal_cov_marker_{}", std::process::id());
        append(
            &Scope::Local,
            &unique,
            Some(Path::new("/tmp")),
    ...
    fn concurrent_appends_never_interleave_records() {
        ...
        let tag = format!("nermal_race_{}", std::process::id());
    ...
    fn a_remote_scope_never_serves_the_local_machine_s_history() {
        ...
        let tag = format!("nermal_scope_{}", std::process::id());
    ...
    fn two_remotes_keep_their_own_stores() {
        ...
        let tag = format!("nermal_twohosts_{}", std::process::id());
    ...
    fn append_rejects_a_cwd_that_would_break_the_line_format() {
        ...
        let unique = format!("nermal_nlcwd_marker_{}", std::process::id());
        append(
            &Scope::Local,
            &unique,
