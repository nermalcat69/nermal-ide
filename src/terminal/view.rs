...
/// the output pump consults instead. `NermalApp::render` declares it each frame
...
pub(crate) const DEFAULT_TITLE: &str = "nermal";
...
/// pane to its workspace's name, and those are names nermal gave the pane
/// deliberately (#438) rather than the absence of one. The literal string
/// `nermal` is the only title that says nothing, because it is the app's own
...
/// every host nermal installs a server on over SSH or WSL spells paths that way.
fn link_path_style(
...
pub struct TerminalView {
    ...
    /// session ends. "nermal" for a local shell; for an SSH pane it is the host
...
fn write_clipboard_image(img: &gpui::Image) -> Option<std::path::PathBuf> {
    ...
    let dir = std::env::temp_dir().join("nermal-clipboard");
...
/// and needs no such check: nermal reaches those distros by running `wsl.exe`
...
const REMOTE_CLIPBOARD_PATH: [&str; 3] = [".cache", "nermal", "clipboard"];
...
/// owner change its mode, so a directory nermal can chmod and then observe at
...
fn validate_remote_clipboard_image(
    write: nermal_core::core::clipboard::ClipboardWrite,
...
impl TerminalView {
    ...
    /// [`PaneRecord::osc_title`](nermal_core::core::machine::PaneRecord::osc_title),
    ...
    /// reports are on a third machine nermal has no link to, and neither this
    ...
    pub fn busy(&self) -> Option<PaneBusy> {
        ...
        // "nermal is still running. Closing ends it." about a folder. The
    ...
    fn poll_remote_clipboard_write(&mut self, cx: &mut Context<Self>) {
        ...
        let write = loop {
            ...
            self.terminal.write(nermal_core::core::clipboard::response(
                write.id.as_deref(),
                "EPERM",
            ));
        };
    ...
    fn on_key_down(&mut self, ev: &KeyDownEvent, window: &mut Window, cx: &mut Context<Self>) {
        ...
        // expects nermal's menu. With the prompt editor off, the shell owning
    ...
    fn remote_shell_history_sources(
        ...
        // start from what nermal recorded for the inner target, like bare ssh.
    ...
    /// Moves this pane between nermal's inline editor and the shell's own line
    ...
    fn shell_owns_prompt(&self) -> bool {
        ...
        // exist to feed nermal's editor, and one of which erases the line with
    ...
    fn tab_pressed(&mut self, forward: bool, cx: &mut Context<Self>) {
        ...
        if let Some(reason) = self.link_inactive_reason(cx) {
            log::debug!(target: "nermal::completion", "Tab does nothing and the line stays: {reason}");
            return;
        }
        if let Some(reason) = self.input_inactive_reason() {
            log::debug!(target: "nermal::completion", "Tab goes straight to the PTY: {reason}");
    ...
    fn complete_tab(&mut self, forward: bool, cx: &mut Context<Self>) {
        ...
        if !cx.global::<Config>().tab_completion {
            log::debug!(target: "nermal::completion", "handing the line to the shell: tab_completion is off");
    ...
    fn spawn_remote_path_completion(
        ...
        log::debug!(target: "nermal::completion", "listing {dir} over the remote's own connection");
        cx.spawn(async move |this, cx| {
    ...
    pub fn commit_text(&mut self, text: &str, cx: &mut Context<Self>) {
        ...
        // taken when nermal is *not* driving the line — so scrolling up and
    ...
    /// one place — [`NermalApp::change_font_size`](crate::ui::app::NermalApp).
    ...
    pub fn hover_link_at(
        ...
        // the mouse itself, so pointing things out inside it is nermal drawing
        // on somebody else's window. Holding the modifier says the user wants
        // nermal's reading of the screen anyway, and then it is theirs to have.
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .context_menu(move |menu, window, cx| {
                ...
                // `fork_label` is nermal-core's capability probe, and core has no
...
/// Whether a right-click belongs to nermal's own context menu rather than to the
...
/// is the escape hatch that reaches nermal, matching how Shift already overrides
...
mod tests {
    ...
    fn a_pane_states_a_title_whenever_it_is_not_the_placeholder() {
        ...
        assert_eq!(stated_title("nermal"), None);
        assert_eq!(stated_title("  nermal  "), None);
        ...
        // A default nermal chose for the pane itself is a name, not the absence
        ...
        assert_eq!(
            stated_title("nermal — process exited"),
            Some("nermal — process exited")
        );
    }
    ...
    fn a_notification_title_keeps_at_most_two_segments() {
        ...
        assert_eq!(compose_notification_title(None, None, ws()), "nermal");
        assert_eq!(
            compose_notification_title(Some("Claude".into()), None, None),
            "Claude"
        );
        assert_eq!(compose_notification_title(None, None, None), "nermal");
    }
    ...
    fn remote_clipboard_images_must_match_their_declared_mime() {
        ...
        let valid = nermal_core::core::clipboard::ClipboardWrite {
            mime: "image/png".into(),
            data: png.clone(),
            id: None,
        };
        ...
        let mismatched = nermal_core::core::clipboard::ClipboardWrite {
    ...
    fn a_wsl_pane_gets_the_automount_path_not_the_windows_one() {
        ...
        assert_eq!(
            staged_path_for_pane(
                r"C:\Users\me\AppData\Local\Temp\nermal-clipboard\paste-1.png",
                true
            ),
            "/mnt/c/Users/me/AppData/Local/Temp/nermal-clipboard/paste-1.png"
        );
        ...
        assert_eq!(
            staged_path_for_pane("/tmp/nermal-clipboard/paste-1.png", false),
            "/tmp/nermal-clipboard/paste-1.png"
        );
        assert_eq!(
    ...
    fn a_failed_staging_preparation_is_retried_rather_than_latched() {
        assert_eq!(
            staging_cache(&Ok("/home/me/.cache/nermal/clipboard".to_string())),
            Some("/home/me/.cache/nermal/clipboard".to_string())
        );
    ...
    fn staged_images_land_under_the_remote_users_own_home() {
        ...
        assert_eq!(dir, "/home/me/.cache/nermal/clipboard");
        assert!(
            !dir.starts_with("/tmp"),
    ...
    fn a_file_command_that_cannot_spawn_comes_back_as_an_error() {
        ...
        assert!(super::run_file_command("nermal-no-such-binary {path}", path, None, None).is_err());
    ...
    fn right_click_belongs_to_the_app_while_mouse_reporting_is_active() {
        assert!(
            ...
            "an unmodified right-click in mouse mode is the application's event, \
             so nermal must not also pop its own menu over it"
        );
    }
    ...
    fn shift_right_click_stays_the_escape_hatch_in_mouse_mode() {
        assert!(
            ...
            "Shift is the documented way to reach nermal's own menu inside a \
             mouse-reporting TUI"
        );
    }
...
mod gpui_tests {
    ...
    fn placed_at(anchor_row: i64) -> crate::terminal::images::PlacedImage {
        use nermal_core::core::kitty_graphics::{Image, WireFormat};
    ...
    fn a_relative_path_links_against_the_panes_own_directory(cx: &mut TestAppContext) {
        let dir = std::env::temp_dir().join(format!("nermal-view-link-{}", std::process::id()));
    ...
    /// itself the divergence: a Windows nermal looking at a *remote* Linux pane
    ...
    fn a_path_the_terminal_wrapped_is_hovered_as_one_link(cx: &mut TestAppContext) {
        let dir = std::env::temp_dir().join(format!("nermal-view-wrap-{}", std::process::id()));
    ...
    fn the_second_column_of_a_wide_character_still_hovers(cx: &mut TestAppContext) {
        let dir = std::env::temp_dir().join(format!("nermal-view-wide-{}", std::process::id()));
    ...
    /// itself, so nermal stays out of it until asked.
    #[gpui::test]
    fn a_path_under_a_full_screen_application_waits_for_the_modifier(cx: &mut TestAppContext) {
        let dir = std::env::temp_dir().join(format!("nermal-view-alt-link-{}", std::process::id()));
        std::fs::create_dir_all(dir.join("scratchpad")).expect("create scratchpad dir");
        ...
        window
            .update(cx, |view, _, cx| {
                ...
                assert!(
                    view.hover_link_at(6, 0, true, cx),
                    "asking for nermal's reading of the screen still gets it"
                );
            })
            .unwrap();
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn title_events_drive_the_tab_title(cx: &mut TestAppContext) {
        ...
        settle(cx);
        window
            .update(cx, |view, _, _| assert_eq!(view.title, "nermal"))
            .unwrap();
    }
    ...
    /// SSH tab in the window read "nermal" until — and only if — the far shell
    ...
    fn allowed_remote_clipboard_image_reaches_the_system_clipboard(cx: &mut TestAppContext) {
        ...
        image::DynamicImage::ImageRgba8(pixel)
            .write_to(&mut std::io::Cursor::new(&mut png), image::ImageFormat::Png)
            .unwrap();
        let write = nermal_core::core::clipboard::ClipboardWrite {
            mime: "image/png".into(),
            data: png.clone(),
            id: Some("copy-1".into()),
        };
        ...
        assert_eq!(
            ...
            nermal_core::core::clipboard::response(Some("copy-1"), "DONE")
        );
    }
    ...
    fn disabled_remote_clipboard_image_is_rejected_without_overwriting(cx: &mut TestAppContext) {
        ...
        assert_eq!(
            reply,
            Some(nermal_core::core::clipboard::response(
                Some("copy-2"),
                "EPERM"
            ))
        );
    ...
    fn shell_vi_mode_prompt_bypasses_the_local_editor(cx: &mut TestAppContext) {
        ...
        panic!("an emacs-mode prompt should re-enable nermal's local editor");
    }
        ...
        window
            .update(cx, |view, window, cx| {
                ...
                assert_eq!(
                    view.cmd.text(),
                    "n",
                    "the first character on the fresh line belongs to nermal's editor"
                );
            })
            .unwrap();
        assert_eq!(
    ...
    fn tab_completion_off_sends_every_tab_to_the_shell(cx: &mut TestAppContext) {
        ...
        window
            .update(cx, |view, window, cx| {
                ...
                view.complete_tab(true, cx);
                assert!(view.completion.is_none(), "no nermal menu while opted out");
        ...
        window
            .update(cx, |view, _, cx| {
                ...
                assert!(
                    ...
                    "no nermal menu while opted out"
                );
    ...
    /// notice that Ctrl-R raises when nermal *wanted* the key and could not have
    /// it.
    #[gpui::test]
    fn prompt_editor_off_hands_over_tab_and_ctrl_r(cx: &mut TestAppContext) {
        ...
        window
            .update(cx, |view, window, cx| {
                ...
                view.on_key_down(&ctrl_r, window, cx);
                assert!(view.completion.is_none(), "no nermal completion menu");
                assert!(view.reverse_search.is_none(), "no nermal history menu");
                assert!(
                    view.integration_notice.is_none(),
    ...
    fn submitted_command_backfills_its_exit_code(cx: &mut TestAppContext) {
        ...
        let marker = format!("nermal_gpui_exit_marker_{}", std::process::id());
        window
            .update(cx, |view, _, cx| {
                view.cmd.set(&marker);
    ...
    fn a_dropped_link_does_not_claim_the_process_exited(cx: &mut TestAppContext) {
        ...
        window
            .update(cx, |view, _, cx| {
                ...
                assert_eq!(view.title, "nermal — disconnected");

                view.set_workspace(None);
                view.handle_event(AlacEvent::Exit, cx);
                assert_eq!(view.title, "nermal — process exited");
            })
            .unwrap();
    }
    ...
    /// which read "nermal — disconnected" no matter whose link died.
    #[gpui::test]
    ...
    fn cmd_backspace_reaches_a_foreground_tui_as_ctrl_u(cx: &mut TestAppContext) {
        ...
        window
            .update(cx, |view, window, cx| {
                assert!(
                    !view.input_active(),
                    "the foreground process, not nermal's editor, owns input"
                );
                view.on_key_down(
                    &KeyDownEvent {
                        keystroke: gpui::Keystroke {
