...
struct ReaderSignals {
    ...
    clipboard_writes: Arc<Mutex<VecDeque<nermal_core::core::clipboard::ClipboardWrite>>>,
...
/// while a pane on a remote `nermal-server` (Linux or macOS only, see
/// [`nermal_core::daemon::install::asset::asset_for_uname`]) or on a native-SSH
...
pub struct PaneWorkspace {
    ...
    /// pane fell back to the bare app name and the tab read "nermal —
    /// disconnected").
...
/// The workspace id a spawn carries, so the pane's shell gets `$nermal_WS` and a
...
impl RemoteTerminal {
    ...
    fn spawn_reader(
        ...
        std::thread::Builder::new()
            .name("nermal-remote-reader".to_string())
            .spawn(move || {
                let ReaderSignals {
                    cwd,
                    shell,
                    ...
                    loop {
                        ...
                        match msg {
                            ...
                            DaemonMsg::Image(frame) => {
                                flush_batch!();
                                if let Some(img) =
                                    nermal_core::core::kitty_graphics::Image::decode_frame_owned(
                                        frame,
                                    )
                                {
                            ...
                            DaemonMsg::DeleteImage(sel) => {
                                flush_batch!();
                                if let Some(del) =
                                    nermal_core::core::kitty_graphics::ImageDelete::decode(&sel)
                                {
                            ...
                            DaemonMsg::ClipboardWrite(frame) => {
                                flush_batch!();
                                if let Some(write) =
                                    nermal_core::core::clipboard::ClipboardWrite::decode_frame(frame)
                                {
                                        ...
                                        .is_ok()
                                    {
                                        ...
                                        let reply = nermal_core::core::clipboard::response(
                                            write.id.as_deref(),
                                            "EBUSY",
                                        );
    ...
    pub fn pop_clipboard_write(&self) -> Option<nermal_core::core::clipboard::ClipboardWrite> {
...
pub(crate) fn notify_desktop_for_pane(title: Option<&str>, body: &str, pane: Option<EntityId>) {
    let summary = sanitize_notification_text(title.unwrap_or("nermal"), NOTIFY_TITLE_MAX);
...
/// Shape: `nermal-pane-<pid>-<leaf>-<seq>`.
///
...
/// whatever process now owns the bundle id — a relaunched nermal, or a second
...
const NOTIFICATION_ID_PREFIX: &str = "nermal-pane-";
...
mod macos_notify {
    ...
    define_class!(
        ...
        #[name = "NermalNotificationDelegate"]
        struct Delegate;
    ...
    fn install_delegate() {
        ...
        ONCE.call_once(|| {
            ...
            // `com.github.nermal` when LaunchServices knows that id (a bundled
            // nermal.app, or a machine that has one installed); when it does not,
            ...
            let _ = notify_rust::set_application("com.github.nermal");
...
mod notification_tests {
    ...
    fn a_foreign_identifier_reveals_nothing() {
        ...
        for id in [
            String::new(),
            "nermal-pane-".into(),
            "nermal-pane-x-1".into(),
            "nermal-pane-42".into(),
            "other-42-1".into(),
            format!("nermal-pane-{pid}"),
            format!("nermal-pane-{pid}-42"),
            format!("nermal-pane-{pid}-x-1"),
        ] {
...
mod replay_tests {
    ...
    fn attach_client(
        pane: &std::sync::Arc<nermal_core::daemon::pane::DaemonPane>,
    ...
    fn type_until_echoed(
        pane: &nermal_core::daemon::pane::DaemonPane,
        line: &[u8],
        term: &RemoteTerminal,
        needle: &str,
    ) -> bool {
    ...
    fn a_pane_re_attached_after_a_switch_gets_its_screen_back() {
        crate::core::config::pin_test_config_dir();
        let pane = nermal_core::daemon::pane::DaemonPane::spawn(
            7711,
...
mod parked_cursor_tests {
    ...
    fn a_routed_pane_reads_a_raw_pty_whatever_the_client_was_built_for() {
        ...
        .unwrap();
        // A remote workspace only ever installs a `nermal-server` on Linux or
...
/// folded out of the stream ([`nermal_core::core::term_modes`]). This is the
...
mod tests {
    ...
    fn a_local_stdio_workspace_routes_to_a_child_process_on_the_pane_dialect() {
        let ws = PaneWorkspace {
            ...
            target: crate::core::session::RemoteTarget::LocalStdio {
                program: "/tmp/nermal-server".into(),
                args: vec!["--stdio".into()],
            },
            spec: None,
            label: None,
        ...
        match &header.target {
            crate::daemon::router::RouteTarget::LocalStdio { program, args } => {
                assert_eq!(program, "/tmp/nermal-server");
    ...
    #[test]
    fn a_local_spawn_carries_its_workspace_so_the_shell_gets_nermal_ws() {
        let ws = "0d4e1a54-0000-4000-8000-000000000001";

        assert_eq!(
            spawn_workspace(Some(ws), &PaneRoute::Local).as_deref(),
            Some(ws),
            "without this the pane's shell has no $nermal_WS and every \
    ...
    fn only_a_dead_daemon_is_worth_starting_one_for() {
        let connect_failed = |kind| -> anyhow::Error {
            anyhow::Error::new(std::io::Error::new(kind, "no listener"))
                .context("connect to daemon at /tmp/nermal.sock")
        };
    ...
    fn the_spawn_directory_answers_until_the_shell_reports_its_own() {
        ...
        term.seed_cwd(Some(PathBuf::from("/repo/nermal")));
        assert_eq!(
            term.foreground_cwd(),
            Some(PathBuf::from("/repo/nermal")),
            "the sidebar can group this pane without waiting for the shell"
        );

        DaemonMsg::Cwd(PathBuf::from("/repo/nermal/crates"))
            .encode(&mut daemon_side)
            .unwrap();
        daemon_side.flush().unwrap();
        assert!(
            poll(Some("/repo/nermal/crates")),
