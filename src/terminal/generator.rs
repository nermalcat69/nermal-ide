...
fn run_uncached(script: &str, cwd: &Path) -> Vec<Parsed> {
    ...
    let out = nermal_core::core::proc::output_within(
...
mod tests {
    ...
    fn cargo_packages_dedupes_by_name() {
        let json = r#"{"packages":[{"name":"nermal","version":"0.9.0"},{"name":"serde","version":"1.0"},{"name":"nermal","version":"0.9.0"}]}"#;
        assert_eq!(
            pairs(&parse_cargo_packages(json)),
            vec![("nermal", Some("0.9.0")), ("serde", Some("1.0"))]
        );
    }
