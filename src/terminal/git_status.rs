...
/// else. See [`nermal_core::core::path_spelling`].
///
...
fn key(host: HostId, path: &Path) -> Cow<'_, Path> {
    nermal_core::core::path_spelling::spelling_on(host, path)
}
...
impl GitStatusCache {
    ...
    /// permanent whenever anything switched branches outside nermal: the overlay
...
mod tests {
    ...
    /// A branch switched outside nermal leaves the cached status naming the old
    ...
    fn a_real_repository_files_its_probe_and_its_diff_under_one_root() {
        use nermal_core::core::git::diff::{DiffRequest, probe_diff};

        let host = nermal_core::host::local::LocalHost::new();
        let dir = std::env::temp_dir().join(format!("nermal-one-root-{}", std::process::id()));
        ...
        // A branch switched outside nermal is what makes this correction the
