...
mod tests {
    ...
    fn a_soft_wrapped_file_path_is_one_link() {
        let dir = std::env::temp_dir().join(format!("nermal-wrap-{}", std::process::id()));
        let make = |rel: &str| {
    ...
    fn a_newline_short_of_the_right_edge_is_not_a_wrap() {
        let dir = std::env::temp_dir().join(format!("nermal-nowrap-{}", std::process::id()));
    ...
    fn angle_brackets_pair_only_when_they_hug_their_contents() {
        for (text, want) in [
            ...
            ("usage: nermal <command> [opts]", "<command>"),
