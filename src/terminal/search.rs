...
/// Answers for paths on the machine nermal itself is running on.
...
///
/// The machine nermal runs on and the machine a pane's paths live on need not
...
impl PathStyle {
    /// The dialect the machine nermal is running on speaks.
...
impl LinkRoots {
    /// Roots on the machine nermal is running on, where `$HOME` means what it
...
/// clients, which meant a Windows nermal could not say what `~` meant in *any*
...
mod tests {
    ...
    fn temp_file(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("nermal-link-test-{}", std::process::id()));
    ...
    fn tilde_expansion_prefers_home_inferred_from_the_pane_cwd() {
        let roots = LinkRoots {
            dirs: vec![PathBuf::from("/Users/alice/clone/nermal")],
            local_home: true,
            style: PathStyle::Posix,
        };
        assert_eq!(
            expand_home("~/clone/nermal/src/main.rs", &roots),
            Some("/Users/alice/clone/nermal/src/main.rs".to_string())
        );
    }
    ...
    /// nermal runs on rather than the one the pane's paths are on — so this only
    ...
    /// The bug this whole [`PathStyle`] exists for: a Windows nermal looking at
    ...
    /// used to reach for the *client's* separator, so a Windows nermal asked a
    ...
    /// client. The rule was gated to Unix ones, so a Windows nermal could not
