...
fn spawn_config_watcher(cx: &mut App) {
    ...
    let handler = move |res: notify::Result<notify::Event>| {
        ...
        // event — see `nermal_core::host::is_content_change`.
        if !nermal_core::host::is_content_change(&event.kind) {
            return;
        }
        let hit = event
            .paths
...
fn forward_open_path_with(
    ...
    dispatch: impl FnOnce(Option<String>) -> std::io::Result<nermal_core::daemon::control::ReplyOk>,
) -> bool {
...
fn forward_open_path(open_path: Option<&std::path::Path>) -> bool {
    use nermal_core::client::ControlClient;
    use nermal_core::daemon::control::{ControlHello, ControlRequest};
    ...
    forward_open_path_with(
        ...
        |path| {
            let hello = ControlHello::host_rpc(
                format!("nermal-app-open-{}", std::process::id()),
                "this computer",
            );
...
fn main() {
    ...
    if let Some(register) = explorer_menu_action_from(&args) {
        let result = if register {
            ...
            // entries, whatever language the user runs nermal in.
        ...
        .any(|arg| arg == std::ffi::OsStr::new("--stop-daemon"))
    {
        ...
        if let Some(dir) = update_install_dir_from(args.iter().cloned()) {
            ...
            nermal_core::daemon::update_guard::hold_for_parent();
    ...
    // Give desktop toasts the nermal icon and name instead of notify-rust's
    ...
    // double-click on the bundle, `open -a nermal` — only reaches this process
    ...
    // tray-resident nermal has: with `show_tray_icon` on, closing the last
    ...
    // back in were ⌘N, the tray's "Show nermal", or quitting and relaunching).
...
mod config_reload_tests {
    ...
    fn running_on(cx: &mut App, config: Config) {
        let dir = std::env::temp_dir().join(format!("nermal-reload-{}", std::process::id()));
