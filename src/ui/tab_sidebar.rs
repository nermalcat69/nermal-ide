...
impl NermalApp {
        ...
        for (group_slot, group_ix) in blocks {
            ...
            for (slot, i) in visible.into_iter().enumerate() {
                ...
                rows.push(row.context_menu(move |menu, window, cx| {
                    NermalApp::tab_context_menu(menu, i, true, &menu_app, window, cx)
                }));
            }
    ...
    /// decided by its tabs' cwds, so "put this tab in nermal" is a request the
...
mod fold_tests {
    ...
    fn drawn(app: &NermalApp, i: usize) -> bool {
    ...
    fn plant_repo(app: &NermalApp, i: usize, cwd: &str, root: &str, cx: &mut gpui::App) {
...
mod tests {
    ...
    fn a_custom_name_is_never_shortened_the_way_a_path_is() {
        let keys = vec![Some(c("work/urgent")), Some(g("/home/u/nermal"))];
        let sections = sidebar_sections(&keys);
        assert_eq!(
            sections.iter().map(|s| s.name.clone()).collect::<Vec<_>>(),
            vec![Some("work/urgent".into()), Some("nermal".into())]
        );
    }
    ...
    fn a_custom_group_never_merges_with_the_repo_it_is_named_after() {
        let keys = vec![Some(g("/w/nermal")), Some(c("nermal"))];
        let sections = sidebar_sections(&keys);
    ...
    fn group_names_disambiguate_only_the_collisions() {
        ...
        assert_eq!(names, vec!["work/app", "fork/app", "nermal"]);
    }
