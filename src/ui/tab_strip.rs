...
/// metadata can distinguish nermal launch defaults from user-authored arguments.
fn shell_spec(shell: &DetectedShell) -> ShellSpec {
    ShellSpec {
        program: shell.program.clone(),
        args: shell.args.clone(),
        args_are_nermal_defaults: shell.args_are_nermal_defaults,
    }
}
...
/// same titles.
pub(crate) use nermal_core::core::tab_view::strip_host_prefix;
...
/// nermal ships integration for reports OSC 7 and no OSC 0 — was listed by the
/// switcher as `~/repo/nermal` and by the strip that owned it as "nermal", the
/// app's own name (#740).
...
pub(crate) fn workspace_avatar(
    ...
    div()
        ...
        .children(dot.map(|rgb| NermalApp::status_dot(rgb, 0, size, cx.theme().popover, false)))
}
...
mod tests {
    ...
    fn short_title_keeps_an_ssh_address_whole() {
        ...
        // is the title nermal's own pwsh integration writes on Windows.
    ...
    fn elide_path_shows_the_whole_deep_path_when_the_budget_allows(cx: &mut TestAppContext) {
        ...
        let path = "E:/work/toolbox/crates/nermal-core/src/client";
        let max = measure_text(&ts, &font, size, path) + 1.;
    ...
    fn configured_shell_arguments_remain_user_authored_in_the_menu() {
        ...
        assert!(!spec.args_are_nermal_defaults);
    }
    ...
    fn strip_tab() -> crate::ui::machine_mirror::TabView {
        crate::ui::machine_mirror::TabView {
            id: nermal_core::core::machine::TabId::new(),
            name: None,
    ...
    fn a_renamed_tab_keeps_its_name_over_every_other_answer() {
        ...
        tab.cwd = Some("/Users/x/repo/nermal".into());
    ...
    /// #740: every shell nermal ships integration for except PowerShell reports
    /// its directory over OSC 7 and never sets a title, which left the tab
    /// reading "nermal" — the app's own name — while the switcher listing the
    ...
    fn a_pane_that_has_only_said_where_it_is_is_named_after_that() {
        ...
        assert_eq!(label_of(&tab, 0, Some(home())), "~/repo/nermal");
        // Through the same shortener as a title, so a deep directory is cut
        // where a deep path in a title would be.
        tab.cwd = Some("/Users/x/repo/nermal/crates/nermal-core/src".into());
        assert_eq!(
            label_of(&tab, 0, Some(home())),
            super::short_title("/Users/x/repo/nermal/crates/nermal-core/src", Some(home())),
        );
    }
    ...
    fn a_tab_named_after_a_directory_says_nothing_more_on_hover_unless_it_was_cut() {
        ...
        tab.cwd = Some("/Users/x/repo/crates/nermal-core/src".into());
        assert_eq!(label_of(&tab, 0, Some(home())), "…/crates/nermal-core/src");
        assert_eq!(
            tooltip_of(&tab, 0, Some(home())).as_deref(),
            Some("~/repo/crates/nermal-core/src")
        );
    ...
    /// of that very same pane. Before this change the strip said "nermal" and
    ...
    fn the_strip_names_a_titleless_pane_exactly_as_the_switcher_does(cx: &mut TestAppContext) {
        ...
        let _stream = app.update_in(&mut vcx, |app, window, cx| {
            ...
            // titled itself — every shell nermal ships integration for except
            // PowerShell.
            view.read(cx)
                .terminal
    ...
    fn a_pane_with_nothing_to_say_falls_back_the_way_it_always_did() {
        ...
        let tab = strip_tab();
        assert_eq!(label_of(&tab, 0, Some(home())), "nermal");
    ...
    /// machine that is has nothing to do with which one nermal is running on: a
