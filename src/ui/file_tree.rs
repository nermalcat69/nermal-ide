...
impl FileTreeState {
    ...
    fn sync_watch(
        ...
        if let Some(sub) = self.watch.clone() {
            ...
            HostOps::run(
                ...
                |app: &mut NermalApp, result: std::io::Result<()>, cx| {
    ...
    fn sync_search(&mut self, query: &str, roots: &[PathBuf], cx: &mut Context<NermalApp>) {
...
impl NermalApp {
    ...
    fn file_tree_begin_edit(
        ...
        let sub = cx.subscribe_in(
            &input,
            window,
            |this: &mut NermalApp, _input, ev, window, cx| match ev {
...
fn dotfiles_menu_item(show_hidden: bool, app: &gpui::WeakEntity<NermalApp>) -> PopupMenuItem {
    let app = app.clone();
...
mod tests {
    ...
    fn the_tree_reads_the_same_listing_out_of_the_host() {
        let host = nermal_core::host::local::LocalHost::new();
        let tmp = std::env::temp_dir().join(format!("nermal-tree-host-{}", std::process::id()));
    ...
    fn a_symlink_cycle_cannot_make_the_search_walk_forever() {
        let host = nermal_core::host::local::LocalHost::new();
        let tmp = std::env::temp_dir().join(format!("nermal-tree-loop-{}", std::process::id()));
        let _ = host.remove(&tmp, true);
...
mod render_idle_gpui_tests {
    ...
    pub(super) fn scratch(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("nermal-idle-{name}-{}", std::process::id()));
    ...
    pub(super) fn rows(app: &Entity<NermalApp>, vcx: &mut VisualTestContext) -> usize {
    ...
    fn fs_event(app: &Entity<NermalApp>, vcx: &mut VisualTestContext, path: &Path) {
        app.update_in(vcx, |app, _, cx| {
            app.file_tree_apply_fs_events(HostId::LOCAL, &HashSet::from([path.to_path_buf()]), cx);
        });
    }

    pub(super) fn settle(app: &Entity<NermalApp>, vcx: &mut VisualTestContext, root: &Path) {
    ...
    fn git(root: &Path, args: &[&str]) -> bool {
        let mut full = vec![
            "-c",
            "user.name=nermal test",
            "-c",
            "user.email=test@nermal.invalid",
            "-c",
