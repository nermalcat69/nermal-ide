...
use gpui::{App, Global};
use nermal_core::daemon::control::ControlClient;
                ...
                match connected {
                    Ok(client) => {
                        ...
                        if restarted {
                            ...
                            crate::ui::tree_sync::on_link_up(cx, nermal_core::host::HostId::LOCAL);
                        }
                    }
...
fn connect_blocking() -> std::io::Result<Arc<ControlClient>> {
    ...
    let sink: nermal_core::daemon::control::EventSink = Box::new(local_event_sink);
    #[cfg(unix)]
    let client = {
        let stream = std::os::unix::net::UnixStream::connect(
            nermal_core::host::server::control_socket_path()?,
        )?;
        ...
        nermal_core::daemon::transport::tune(&stream);
        ControlClient::over_unix(stream, &hello, sink)?
    };
    #[cfg(windows)]
    let client =
        ControlClient::over_tcp(nermal_core::host::server::connect_control()?, &hello, sink)?;
    Ok(Arc::new(client))
}
...
fn dialect_refusal(e: &std::io::Error) -> Option<nermal_core::daemon::control::DialectRefusal> {
    nermal_core::daemon::control::parse_dialect_refusal(&e.to_string())
}

fn local_event_sink(event: nermal_core::daemon::control::ControlEvent) {
    nermal_core::daemon::control::observe_event(nermal_core::host::HostId::LOCAL, event);
}
