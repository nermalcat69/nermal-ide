...
impl NermalApp {
    ...
    /// the one it gets — see [`NermalApp::document_dock_px`].
    ///
...
mod gpui_tests {
    ...
    fn push_tab(app: &mut NermalApp, cx: &mut Context<NermalApp>) {
    ...
    fn window_with(
        ...
        w: f32,
        tabs: usize,
    ) -> (Entity<NermalApp>, VisualTestContext) {
    ...
    fn window(cx: &mut TestAppContext, w: f32) -> (Entity<NermalApp>, VisualTestContext) {
        window_with(cx, w, 1)
    }

    fn dock_px(app: &Entity<NermalApp>, vcx: &mut VisualTestContext) -> Option<f32> {
    ...
    fn tab_layout(app: &Entity<NermalApp>, vcx: &mut VisualTestContext) -> DocumentLayout {
    ...
    fn the_palette_offers_the_active_tabs_layout_not_the_configs(cx: &mut TestAppContext) {
        ...
        let row = |app: &Entity<NermalApp>, vcx: &mut VisualTestContext| {
