...
pub(crate) struct SftpPanelState {
    ...
    ///
    /// An upload is written to `<name>.nermal-upload-<hex>` and renamed into
...
impl NermalApp {
    ...
    /// `<name>.nermal-upload-<hex>` and is renamed into place at the end, so the
...
mod gpui_tests {
    ...
    fn panel(app: &Entity<NermalApp>, vcx: &mut VisualTestContext) -> (bool, RightPanelTab) {
        vcx.update(|_, cx| {
            let app = app.read(cx);
