...
use nermal_core::host::{Host as _, HostId};
...
pub const LOCAL_STDIO_ENV: &str = "nermal_LOCAL_STDIO_SERVER";
...
fn client_hostname() -> String {
    ...
    NAME.get_or_init(|| {
        ...
        nermal_core::core::proc::hide_console(&mut cmd)
            .output()
            .ok()
            .filter(|o| o.status.success())
            .map(|o| String::from_utf8_lossy(&o.stdout).trim().to_string())
            .filter(|s| !s.is_empty())
            .unwrap_or_else(|| "a nermal client".to_string())
    })
    .clone()
}
...
pub fn rows_from_machine(machine: &nermal_core::core::machine::Machine) -> Vec<RemoteWorkspaceRow> {
...
/// layer's own wording, which reads like the far end is not nermal at all. It is —
...
mod tests {
    ...
    fn request() -> InstallRequest {
        InstallRequest {
            ...
            version: "0.9.1".into(),
            asset: "nermal-server-linux-x86_64-musl",
            source_url: "https://example.invalid/v0.9.1/nermal-server".into(),
            remote_path: "/home/me/.local/share/nermal/bin/nermal-server-0.9.1".into(),
    ...
    fn refusal(peer: u32, ours: u32) -> String {
        format!(
            "java answered, but not as a nermal server: control peer (build 26.7.7-nightly) \
    ...
    fn a_pane_and_its_workspace_resolve_to_the_same_machine() {
        ...
        let local = RemoteTarget::LocalStdio {
            program: "/opt/nermal-server".into(),
            args: vec!["--stdio".into()],
        };
    ...
    fn the_mismatch_prompt_names_the_host_and_both_versions() {
        ...
        let m = MismatchedRemoteDaemon {
            host: "me@build-box:22".into(),
            running_version: Some("0.8.0".into()),
            running_exe: Some("/home/me/.local/share/nermal/bin/nermal-server-0.8.0".into()),
            wanted_version: "0.9.1".into(),
        };
    ...
    fn rows_from_the_tree_sort_newest_first_and_derive_names() {
        use nermal_core::core::machine::{Machine, PaneRecord, Tab, Workspace};
