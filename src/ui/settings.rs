...
/// next to it are written. Every locale nermal ships writes these counts the
...
impl NermalApp {
    ...
    pub(crate) fn import_ssh_config_profiles(
        ...
        log::info!(
            "imported {read} alias(es) from {source} ({} file(s) read): {} added, {} updated, \
             {} unchanged; no nermal setting for: [{}]",
            report.files_read,
            stats.added,
            stats.updated,
            stats.unchanged,
    ...
    fn render_shell_group(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        // nermal already knows which shells are installed — it lists them on the
        ...
        // the other. The field stays: a shell nermal did not find still has to be
        // reachable by path.
    ...
    fn render_settings_input(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        // Tab completion and history search are menus nermal opens *inside* its
    ...
    fn render_settings_about(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        let http_proxy_invalid = !http_proxy_value.is_empty()
            && !nermal_core::daemon::install::proxy::is_valid_manual(&http_proxy_value);
...
mod tests {
    ...
    fn the_passphrase_follows_the_first_key_that_is_really_there() {
        let dir = std::env::temp_dir().join(format!("nermal-keyform-{}", std::process::id()));
    ...
    /// The `nermal` CLI exists so scripts and coding agents can drive nermal, so it
    ...
    fn index_titles_match_rendered_row_labels() {
        for title in [
            ...
            "Dim inactive panes",
            "Option (⌥) acts as Meta",
            "Install the nermal command on PATH",
        ] {
            assert!(
                settings_search_entries()
                    .iter()
    ...
    fn group_labels_name_the_file_and_the_app() {
        ...
        assert_eq!(ssh_group_label(""), "In nermal");
        ...
        let app = window
            .update(cx, |root, _, _| {
                root.view()
                    .clone()
                    .downcast::<NermalApp>()
                    .unwrap_or_else(|_| panic!("window root wraps a NermalApp"))
            })
            .unwrap();
