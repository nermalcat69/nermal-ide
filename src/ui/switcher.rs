...
struct TabRow {
    ...
    git: Option<nermal_core::core::git::GitStatus>,
}
...
/// `nermal run` leaves behind, and what `nermal pane ls --all` points the CLI's
...
pub(crate) fn orphan_panes_of(
    listed: Vec<nermal_core::daemon::protocol::PaneInfo>,
    held: &HashSet<u64>,
) -> Vec<OrphanPane> {
    listed
        .into_iter()
...
/// characters, which is what `nermal pane ls --all` prints and enough to line
/// the two listings up. An owner that is no workspace id at all is an older
/// client's own label (`nermal-cli`) and already reads fine.
...
fn default_workspace_name(chosen: Option<&HostChoice>, cx: &App) -> String {
    let host = match chosen {
        None => nermal_core::host::HostId::LOCAL,
...
impl NermalApp {
    ...
    fn refresh_orphan_panes(&mut self, cx: &mut Context<Self>) {
        cx.spawn(async move |this, cx| {
            let listed = cx
                .background_spawn(async move { nermal_core::client::PaneClient::local().list() })
                .await;
            let listed = match listed {
                Ok(listed) => listed,
                Err(e) => {
                    log::warn!(target: "nermal::switcher", "orphan pane listing failed: {e}");
                    return;
                }
            };
        ...
        cx.spawn(async move |this, cx| {
            let listed = cx
                .background_spawn(async move {
                    let client = nermal_core::client::PaneClient::local();
                    client.kill(pane_id)?;
                    client.list()
                })
                .await;
            let listed = match listed {
                Ok(listed) => listed,
                Err(e) => {
                    log::warn!(target: "nermal::switcher", "closing orphan %{pane_id} failed: {e}");
                    return;
                }
            };
            let _ = this.update(cx, |this, cx| {
    ...
    fn switcher_groups(&self, cx: &mut Context<Self>) -> Vec<Group> {
        ...
        // person who ran `nermal new` like nothing happened at all. They open
    ...
    fn tab_rows_for(&self, id: WorkspaceId, cx: &App) -> Vec<TabRow> {
        ...
        let git = |cwd: Option<&str>| -> Option<nermal_core::core::git::GitStatus> {
                ...
                v_flex()
                    ...
                    .child(
                        h_flex()
                            ...
                            // the row ended in `~/repo/025/nermal · …` every
...
mod tests {
    ...
    fn orphan_panes_of_keeps_only_live_panes_no_workspace_holds() {
        use nermal_core::daemon::protocol::PaneInfo;
        ...
        fn info(pane_id: u64, alive: bool) -> PaneInfo {
            PaneInfo {
                ...
                osc_title: None,
                alive,
                owner: Some("nermal-cli".to_string()),
            }
        }
    ...
    fn a_tab_of_another_window_is_named_the_way_a_local_one_would_be() {
        ...
        let mut view = crate::ui::machine_mirror::TabView {
            ...
            cwd: Some("/Users/x/repo/nermal".to_string()),
        ...
        view.osc_title = Some("user@host:~/repo/025/nermal".to_string());
        assert_eq!(
            tab_view_label(&view, 0, None),
            crate::ui::tab_strip::short_title("user@host:~/repo/025/nermal", None),
        ...
        assert_eq!(
            tab_view_label(&view, 0, None),
            crate::ui::tab_strip::short_title("/Users/x/repo/nermal", None),
