...
/// The bindings that were already in the keymap before nermal put its own
...
pub fn init(cx: &mut App) {
    ...
    // closing the last window retires nermal to the tray instead of quitting —
    ...
    // is for. `NermalApp`'s own listener still wins wherever there is a window:
...
/// Replaces the keymap with the full set: the bindings nermal inherited, then
...
/// `init` bound them in, and gpui reads the map back to front, so nermal's
...
/// paste chord every Windows and Linux desktop trains its users on; nermal
...
fn authored_entry(action: &str) -> Option<(CommandGroup, String)> {
    ...
    Some(match action {
        ...
        "About" => (
            CommandGroup::Application,
            t(L10nKey::CmdAboutnermal).to_string(),
        ),
        "CheckForUpdates" => (
            CommandGroup::Application,
        ...
        "Quit" => (
            CommandGroup::Application,
            t(L10nKey::CmdQuitnermal).to_string(),
        ),
...
mod gpui_tests {
    ...
    #[gpui::test]
    fn a_rebuild_keeps_the_bindings_nermal_did_not_install(cx: &mut TestAppContext) {
        cx.update(|cx| {
            ...
            assert!(
                !inherited.is_empty(),
                "gpui-component binds backspace before nermal touches the keymap"
            );
