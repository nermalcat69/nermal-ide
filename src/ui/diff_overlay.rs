...
impl NermalApp {
    ...
    ///
    /// Not [`NermalApp::segmented_on`]: that one is a bordered track, which is
...
/// The de-duplication sets on `NermalApp` are keyed by `(HostId, PathBuf)`, so
...
mod overlay_gpui_tests {
    ...
    fn a_second_source_over_one_directory_is_a_second_overlay(cx: &mut TestAppContext) {
        ...
        let cwd = std::path::PathBuf::from("/no/such/nermal/repo");
    ...
    fn one_file_snapshot(source: DiffSource) -> DiffSnapshot {
        ...
        DiffSnapshot {
            root: std::path::PathBuf::from("/no/such/nermal/repo"),
            source,
            branch: "main".into(),
            files: vec![FileDiff {
                path: "a.rs".into(),
    ...
    fn show(app: &Entity<NermalApp>, vcx: &mut VisualTestContext, source: DiffSource) {
        let cwd = std::path::PathBuf::from("/no/such/nermal/repo");
...
///
/// The way in is ordinary: switch branches anywhere outside nermal — another
...
/// `nermal_core::core::path_spelling` should lift this, as a change that can
...
mod render_idle_gpui_tests {
    ...
    fn a_long_patch_builds_only_the_rows_on_screen(cx: &mut TestAppContext) {
        ...
        let root = std::env::temp_dir().join(format!("nermal-diff-rows-{}", std::process::id()));
    ...
    fn an_overlay_over_a_stale_branch_reaches_render_idle(cx: &mut TestAppContext) {
        let root = std::env::temp_dir().join(format!("nermal-diff-idle-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).unwrap();
        let root = std::fs::canonicalize(&root).unwrap();
        git(&root, &["init", "--quiet"]);
        ...
        // The cache as a branch switch outside nermal leaves it: a branch this
...
mod selection_gpui_tests {
    ...
    fn window(cx: &mut TestAppContext) -> (Entity<NermalApp>, VisualTestContext) {
    ...
    fn copied(app: &Entity<NermalApp>, vcx: &mut VisualTestContext) -> Option<String> {
        app.update_in(vcx, |this, _, cx| this.copy_diff_selection(cx));
        vcx.update(|_, cx| cx.read_from_clipboard().and_then(|item| item.text()))
    }

    fn selection(app: &Entity<NermalApp>, vcx: &mut VisualTestContext) -> Option<DiffSelection> {
