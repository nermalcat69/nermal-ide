...
//!
//! All of it is app-level, hanging off `NermalApp.scm` rather than off a tab.
...
use nermal_core::core::git::log::{Commit, CommitFile, CommitPage, GraphScope};
use nermal_core::core::git::status::HeadState;
...
pub(crate) struct ScmPanelState {
    ...
    /// built by `Default` alongside the rest of `NermalApp`.
