...
use nermal_core::core::git::diff::CommitLabel;
use nermal_core::core::git::log::{Commit, CommitFile, RefKind};
use nermal_core::core::git::status::DecoStatus;
...
impl NermalApp {
    ...
    ///
    /// `theme.accent` is a neutral surface tint in nermal rather than the brand
...
mod tests {
    use super::*;
    use nermal_core::core::git::diff::FileStatus;
...
/// `nermal_core::core::path_spelling` does, in a change that can show these
...
mod detail_gpui_tests {
    ...
    fn settle(
        ...
        done: impl Fn(&NermalApp, &gpui::App) -> bool,
    ) {
    ...
    fn paths(app: &Entity<NermalApp>, vcx: &mut VisualTestContext) -> Vec<String> {
    ...
    fn a_seeded_detail_only_asks_for_the_files(cx: &mut TestAppContext) {
        ...
        let mut seed = nermal_core::core::git::log::load_commit(
            &*nermal_core::host::local::LocalHost::new(),
            &root,
            &head,
        )
    ...
    fn a_detail_from_another_repository_gives_the_body_back(cx: &mut TestAppContext) {
        ...
        app.update_in(&mut vcx, |app, window, cx| {
            let stranger = RepoKey {
                host: HostId::LOCAL,
                root: PathBuf::from("/no/such/nermal/repo"),
            };
