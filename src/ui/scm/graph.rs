...
impl NermalApp {
    ...
    fn scm_load_graph(&mut self, repo: &RepoKey, cx: &mut Context<Self>) {
        ...
        crate::ui::host_ops::HostOps::run(
            host,
            cx,
            move |h| nermal_core::core::git::log::load_page(h, &root, &query, want),
            move |this, page, cx| {
        ...
        h_flex()
            ...
            .context_menu({
                ...
                move |menu, _window, cx| {
                    ...
                    NermalApp::graph_row_context_menu(menu, &repo, &oid, danger, &app)
                }
            })
            .into_any_element()
    }
...
impl NermalApp {
    ...
    ) -> impl Fn(PopupMenu, &mut Window, &mut Context<PopupMenu>) -> PopupMenu + 'static + use<>
    {
        ...
        move |menu, _window, _cx| {
            ...
            let pick = |app: &gpui::WeakEntity<NermalApp>, next: GraphScope| {
                let app = app.clone();
...
impl NermalApp {
    ...
    fn graph_row_context_menu(
        ...
        let op = |app: &gpui::WeakEntity<NermalApp>, repo: &RepoKey, build: fn(String) -> GitOp| {
            let app = app.clone();
            let repo = repo.clone();
...
mod tests {
    ...
    fn repo() -> RepoKey {
        RepoKey {
            host: HostId::LOCAL,
            root: PathBuf::from("/tmp/nermal-graph-test"),
        }
    }
    ...
    fn everything_past_the_cap_lands_in_the_overflow_column() {
        let cap = 4;
        for lane in 4u16..=nermal_core::core::git::log::MAX_LANES {
    ...
    fn switching_repositories_drops_the_previous_page(cx: &mut TestAppContext) {
        ...
        let other = RepoKey {
            host: HostId::LOCAL,
            root: PathBuf::from("/tmp/nermal-graph-test-other"),
        };
        app.update(&mut vcx, |app, cx| {
            app.scm.graph.requested = GRAPH_PAGE * 3;
            app.scm.graph.page = Some(Arc::new(empty_page()));
    ...
    fn commit_named(summary: &str, author: &str, oid: &str) -> Commit {
        use nermal_core::core::git::log::{OffsetTs, Signature};
        let who = Signature {
...
mod render_idle_gpui_tests {
    ...
    fn scratch(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("nermal-graph-{name}-{}", std::process::id()));
