...
use nermal_core::core::git::diff::MAX_RENDERED_FILES;
use nermal_core::core::git::ops::GitOp;
...
/// nermal is `.appearance(false)`; a hairline here, and an accent ring on top of
...
impl NermalApp {
    ...
    /// 0.20)` in nermal, so a "primary" button is a mid-grey block sitting in a
    ...
    fn scm_repo_root(
        ...
        crate::ui::host_ops::HostOps::run(
            ...
            move |this, out, cx| {
                ...
                // from it on that box. See `nermal_core::core::path_spelling`.
                let root = out
                    .as_deref()
                    .and_then(|s| s.lines().next())
                    .map(str::trim)
                    .filter(|l| !l.is_empty())
                    .map(|l| nermal_core::core::path_spelling::spelling_on_buf(id, l));
...
mod tests {
    ...
    use nermal_core::core::git::status::{ConflictKind, EntryKind, RepoPath};
...
mod render_idle_gpui_tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("nermal-scm-{name}-{}", std::process::id()));
    ...
    fn scm_panel_on(
        ...
        root: &Path,
        until: impl Fn(&NermalApp, &gpui::App) -> bool,
    ) -> (
        Entity<NermalApp>,
        VisualTestContext,
    ...
    /// `nermal_core::core::path_spelling` does should lift this, in a change
