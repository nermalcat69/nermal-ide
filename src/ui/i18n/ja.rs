...
pub fn translate_ja(key: L10nKey) -> Option<&'static str> {
    Some(match key {
        ...
        L10nKey::Close => "閉じる",
        L10nKey::QuitStopServerTitle => "nermal を終了してサーバーを停止しますか？",
        L10nKey::QuitStopServerBody => {
            "nermal を終了してバックグラウンドサーバーを停止します。シェルで実行中のものはすべて終了します。タブとレイアウトは次回起動時に新しいシェルで開きます。（ウィンドウを閉じるだけならトレイに退避し、シェルは動き続けます）"
        }
        ...
        L10nKey::SettingsLanguageDesc => "nermal の表示言語を選択します",
        ...
        L10nKey::ExplorerMenuOpenIn => "nermal で開く",
        L10nKey::ExplorerMenuOpenHere => "ここで nermal を開く",
        L10nKey::SettingsCustomThemesIntro => {
            "テーマを複製して色を編集するか、nermal の YAML テーマや iTerm2 の .itermcolors をテーマフォルダに置いてください"
        }
        ...
        L10nKey::SettingsInnermal => "nermal 内",
        ...
        L10nKey::SettingsTypeAddressToConnect => {
            "アドレスを入力するとすぐに接続できます。nermal はあとで保存するか尋ねます"
        }
        ...
        L10nKey::SettingsImportAliasesDesc => {
            "ファイルを再読み込みして新しい項目を追加します。ここでの編集は nermal が保存します — ファイル自体には書き込まれません"
        }
        ...
        L10nKey::SettingsImportIgnored => {
            "nermal に設定のないオプションが {count} 件あり、ファイルに残されています: {options}"
        }
        ...
        L10nKey::SettingsStartInDesc => {
            "新しいシェルの開始場所: nermal の起動ディレクトリ、ホームフォルダ、または固定パス"
        }
        ...
        L10nKey::SettingsPromptIntro => {
            "シェルプロンプトでの nermal 独自のエディターとメニュー。オフにするとその分がシェルに渡されます"
        }
        L10nKey::SettingsPromptEditor => "プロンプトエディター",
        L10nKey::SettingsPromptEditorDesc => {
            "シェルプロンプトで入力する行を nermal が編集します — 選択、取り消し、下のメニュー。オフにするとシェル自身の行エディター（ZLE、readline、fish）に戻ります"
        }
        ...
        L10nKey::SettingsTabCompletionDesc => {
            "プロンプトで Tab を押すと nermal の補完メニューが開きます。オフの場合、Tab はシェル自身の補完に渡されます"
        }
        L10nKey::SettingsHistorySearch => "履歴検索",
        L10nKey::SettingsHistorySearchDesc => {
            "プロンプトで ⌃R を押すと nermal のファジー履歴メニューが開きます。オフなら ⌃R はシェルへ — 逆方向検索や、そこでバインドしたもの（fzf、percol）"
        }
        ...
        L10nKey::SettingsAgentsIntroDesc => {
            "フックにより、これらのエージェントを実行するペインの状態（作業中 / 待機中 / 完了）がタブバーに表示されます。nermal 内でのみ有効"
        }
        ...
        L10nKey::SettingsStartupWindowDesc => "nermal 起動時のウィンドウ状態",
        L10nKey::SettingsRememberWindowSize => "ウィンドウサイズと位置を記憶",
        L10nKey::SettingsRememberWindowSizeDesc => {
            "nermal が最後に終了したときのサイズと位置で開き直します。オフならデフォルトサイズで中央に開きます"
        }
        ...
        L10nKey::SettingsDefaultTerminalDesc => {
            "nermal を Unix 実行ファイル、SSH リンク、man ページリンク用の macOS のデフォルトターミナルにします。nermal はフォルダとスクリプトも開けますが、Finder のフォルダハンドラは置き換えません。独自のターミナルを指定するアプリはこの設定を無視することがあります。"
        }
        L10nKey::SettingsDefaultTerminalSet => "デフォルトのターミナルに設定",
        L10nKey::SettingsDefaultTerminalSetSuccess => {
            "nermal を対応するターミナルファイルとリンクのデフォルトハンドラに設定しました。"
        }
        L10nKey::SettingsDefaultTerminalSetFailed => {
            "nermal をデフォルトのターミナルに設定できませんでした: {error}"
        }
        ...
        L10nKey::SettingsUpdateReadyNextLaunch => "次回 nermal を起動したときに適用されます。",
        ...
        L10nKey::SettingsDaemonStaleDesc => {
            "nermal はその場で更新されました。アプリは新しく、ペインはまだ以前のビルドのサーバーが処理しています。再起動すると新しいビルドに切り替わり、ペインで動いているプロセスはすべて終了します。急ぐ必要はなく、ペインが空いているときにどうぞ"
        }
        L10nKey::UpdateDialogTitle => "アップデートがあります",
        L10nKey::UpdateDialogDetail => {
            "nermal {version} が利用できます（現在 {current}）。インストールするとアプリが再起動します。バックグラウンドサーバーは動いたままなので、ペインの中身は残ります"
        }
        L10nKey::UpdateDialogDetailWindows => {
            "nermal {version} が利用できます（現在 {current}）。インストールするとアプリとバックグラウンドサービスが再起動します。ペインのプロセスは終了し、タブとレイアウトは新しいシェルで復元されます"
        }
        L10nKey::UpdateDialogDetailManual => {
            "nermal {version} が利用できます（現在 {current}）。{hint}"
        }
        ...
        L10nKey::UpdateDialogNeedsElevation => {
            "nermal は全ユーザー向けにインストールされているため、インストール前に Windows の管理者承認が一度求められます。nermal 自体が管理者権限で実行されることはありません"
        }
        ...
        L10nKey::SettingsUpdateUnsupportedMacos => {
            "この nermal は書き込み可能な nermal.app バンドルにないため、自分自身を置き換えられません。「アプリケーション」へ移動するか、リリースページから更新してください"
        }
        ...
        L10nKey::SettingsUpdateUnsupportedWindows => {
            "この nermal は認識可能な Inno Setup 版でもポータブル ZIP 版でもないため、自動更新できません。リリースページを開いて手動で更新してください"
        }
        L10nKey::SettingsUpdateWindowsAllUsers => {
            "nermal はすべてのユーザー向けにインストールされており、置き換えには管理者権限が必要ですが、nermal は自ら昇格を要求しません。リリースページからインストーラーを実行して更新してください"
        }
        ...
        L10nKey::SettingsUpdateMissingChecksums => {
            "このリリースには checksums.txt がないため、nermal は自動インストールを行いません"
        }
        ...
        L10nKey::SettingsCommandLineDesc => {
            "同梱の nermal コマンドを PATH に入れ、スクリプトやエージェントが任意のターミナルから nermal を操作できるようにします（ペイン内ではどちらでも動きます）。自分でビルドした nermal を優先したい場合はオフに。次回起動時に有効"
        }
        L10nKey::SettingsInstallCliOnPath => "`nermal` コマンドを PATH にインストール",
        ...
        L10nKey::SettingsAppHttpProxyDesc => {
            "nermal 自身の更新チェックとダウンロードにのみ使用し、ペインで実行中のプログラムには影響しません。空欄ならシステムのプロキシに従います"
        }
        ...
        L10nKey::SettingsSearchCommandLineToolKeywords => {
            "cli nermal パス シェル コマンド インストール シンボリックリンク ターミナル iterm エージェント スクリプト command line tool"
        }
        ...
        L10nKey::FileTreeNewEntryTooltip => "新しいファイルまたはフォルダ",
        ...
        L10nKey::EditorCloseAllTabs => "すべてのタブを閉じる",
        ...
        L10nKey::EditorAutoSaveOn => "自動保存: オン",
        L10nKey::EditorAutoSaveOff => "自動保存: オフ",
        ...
        L10nKey::PanelNetworkTitle => "ネットワーク",
        L10nKey::PanelNetworkEmpty => "アクティブなネットワーク接続はありません",
        L10nKey::PanelNetworkConnected => "接続済み",
        L10nKey::PanelNetworkDisconnected => "切断",
        L10nKey::PanelNetworkReceived => "受信",
        L10nKey::PanelNetworkSent => "送信",
        L10nKey::PanelNetworkDownloadSpeed => "ダウンロード",
        L10nKey::PanelNetworkUploadSpeed => "アップロード",
        ...
        L10nKey::DiffTruncatedBudget => {
            "内容は読み込まれていません — nermal の Diff 予算を超えています。ターミナルで `git diff` を実行してください"
        }
        ...
        L10nKey::DiffBudgetAndCap => "nermal の予算とファイルごとの上限",
        L10nKey::DiffBudget => "nermal の予算",
        ...
        L10nKey::RemoteInstallTitle => "「{machine}」に nermal サーバーをインストールしますか？",
        L10nKey::RemoteInstallDetail => {
            "nermal はサーバーバイナリを {machine} に書き込み、{machine} でワークスペースをホストできるようにします。{machine} 上の他のものには触れず、sudo も使いません。\n\n{path_label}\u{2003}{path}\n{version_label}\u{2003}{version}\n{size_label}\u{2003}{size}\n{from_label}\u{2003}{from}\n{sha_label}\u{2003}{sha256}\n\n{silent_upgrades}"
        }
        ...
        L10nKey::RemoteMismatchTitle => "「{machine}」上の nermal サーバーを更新しますか？",
        ...
        L10nKey::RemoteServerOutdated => {
            "{machine} の nermal サーバーが古く（{build}）、この nermal からは通信できません。更新すると接続できます"
        }
        L10nKey::RemoteServerTooNew => {
            "{machine} の nermal サーバー（{build}）は、この nermal より新しいバージョンです。このコンピューターの nermal を更新するか、向こうのサーバーを対応するものに置き換えてください"
        }
        L10nKey::RemoteDaemonStartFailed => {
            "nermal のローカルサーバーを起動できませんでした: {error}"
        }
        L10nKey::RemoteDaemonUnreachable => {
            "nermal のローカルサーバーに到達できませんでした: {error}"
        }
        L10nKey::RemoteDaemonTooOld => {
            "このマシンのデーモンは古いビルドのため、{machine} 上のサーバーを再起動できません。nermal を終了（デーモンも停止します）して開き直し、再試行してください"
        }
        ...
        L10nKey::RemoteHostNotnermal => {
            "{machine} は応答しましたが、nermal サーバーとしては応答しませんでした: {error}"
        }
        L10nKey::RemoteWorkspaceListFailed => {
            "{machine} に接続しましたが、ワークスペースの一覧を取得できませんでした: {error}"
        }
        L10nKey::RemoteServerRestartFailed => {
            "{machine} 上の nermal サーバーを再起動できませんでした: {error}"
        }
        L10nKey::RemoteNoRouteToHost => "nermal は {machine} に到達する手段を失いました",
        ...
        L10nKey::SwitcherStartingServer => "nermal のサーバーを起動中…",
        L10nKey::SwitcherDownloadingServerWithTotal => {
            "nermal のサーバーをダウンロード中… {done} / {total}"
        }
        L10nKey::SwitcherDownloadingServerNoTotal => "nermal のサーバーをダウンロード中… {done}",
        L10nKey::SwitcherCopyingServer => "nermal のサーバーをコピー中… {done} / {total}",
        ...
        L10nKey::CmdRightPanelNetwork => "右パネル: ネットワーク",
        ...
        L10nKey::CmdAboutnermal => "nermal について",
        ...
        L10nKey::CmdQuitnermal => "nermal を終了",
        L10nKey::CmdQuitnermalSubtitle => "サーバーを停止し、実行中のすべてのシェルを終了",
        ...
        L10nKey::AppForkNoCommand => "nermal には {name} 用のフォークコマンドがありません",
        L10nKey::AppForkLocalOnly => {
            "{name} のセッションはローカルペインからしかフォークできません"
        }
        L10nKey::AppForkNoSessionId => {
            "nermal はこのペインで {name} のセッション ID を確認できていません — 設定 → エージェントでフックをインストールしてください"
        }
        ...
        L10nKey::AppAgentHooksNoHomeDir => {
            "nermal はこのコンピュータのホームディレクトリを特定できなかったため、インストール先がありません"
        }
        ...
        L10nKey::AppAgentHooksNonermalHooks => {
            "nermal のフックが見つからないため、削除するものはありません"
        }
        ...
        L10nKey::SettingsShellDetected => "nermal が見つけたシェル",
        ...
        L10nKey::SettingsDaemonStaleDescInPlace => {
            "nermal はその場で更新されました。アプリは新しく、ペインはまだ前のビルドで動いています。サーバーは停止せずに新しいビルドへ置き換えられるので、シェルはそのまま引き継がれます。nermal 内蔵の SSH クライアントを使うペインだけは例外で、その接続は閉じられ、開き直しが必要です"
        }
        L10nKey::AppRestartServerBodyInPlace => {
            "サーバーは停止せずに自分自身をこのビルドへ置き換えます。シェルは動いたままで、ウィンドウはすぐに再接続します。nermal 内蔵の SSH クライアントを使うペインだけは例外で、その接続は閉じられ、開き直しが必要です"
        }
        ...
        L10nKey::SettingsPerPaneHistoryDescription => {
            "上キーでたどるのは、全ペインが混ざったものではなくこのペインで実行したコマンドです。新しいペインは既存の履歴から始まり、追加分は閉じるときに書き戻されます。対象は nermal が設定できる bash と zsh のペインで、独自の引数で起動したシェルはそのままです"
        }
        ...
        L10nKey::IntegrationNoticeNotEngaged => {
            "このペインでは nermal シェル統合が有効になっておらず、インライン補完と Ctrl+R メニューは使えません。よくある原因は、独自の引数で起動したシェル、PTY ラッパー、未対応のシェルです。"
        }
        ...
        L10nKey::AppMenuDocumentation => "nermal ドキュメント",
        ...
        L10nKey::TrayShownermal => "nermal を表示",
        ...
        L10nKey::TabMenuOpenFolder => "フォルダーを開く…",
...
pub fn translate_variant_ja(key: L10nKey, branch: &'static str) -> Option<&'static str> {
    let res = match (key, branch) {
        ...
        (L10nKey::SettingsImportIgnored, "zero") => {
            "ファイル内のすべてのオプションに nermal 側の設定があります"
        }
        (L10nKey::SettingsImportIgnored, "one") => {
            "nermal に設定のないオプションが 1 件あり、ファイルに残されています: {options}"
        }
