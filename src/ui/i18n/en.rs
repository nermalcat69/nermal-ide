...
pub fn translate_en(key: L10nKey) -> &'static str {
    match key {
        ...
        L10nKey::QuitStopServerBody => {
            "This quits nermal and stops the background server; anything running in your shells is terminated. Your tabs and layout reopen with fresh shells next launch. (Closing the window only retires nermal to the tray — the shells keep running.)"
        }
        ...
        L10nKey::SettingsLanguageDesc => "Choose the language used for the nermal interface.",
        ...
        L10nKey::ExplorerMenuOpenIn => "Open in nermal",
        L10nKey::ExplorerMenuOpenHere => "Open nermal here",
        L10nKey::SettingsCustomThemesIntro => {
            "Duplicate a theme to edit its colors, or drop a nermal YAML theme or iTerm2 .itermcolors file in the themes folder."
        }
        ...
        L10nKey::SettingsInnermal => "In nermal",
        ...
        L10nKey::SettingsTypeAddressToConnect => {
            "Type an address to connect now — nermal offers to save it afterwards."
        }
        ...
        L10nKey::SettingsImportAliasesDesc => {
            "Re-reads the file and adds anything new. Edits you make here are stored by nermal — the file itself is never written."
        }
        ...
        L10nKey::SettingsImportIgnored => {
            "{count} options have no setting in nermal and were left in the file: {options}"
        }
        ...
        L10nKey::SettingsStartInDesc => {
            "What a fresh shell starts in: nermal's launch directory, your home folder, or a fixed path."
        }
        ...
        L10nKey::SettingsPromptIntro => {
            "nermal's own editor and menus at the shell prompt. Turn one off to hand that much back to the shell."
        }
        L10nKey::SettingsPromptEditor => "Prompt editor",
        L10nKey::SettingsPromptEditorDesc => {
            "nermal edits the line you type at the shell prompt: selection, undo, and the menus below. Off hands the prompt back to the shell's own editor — ZLE, readline, fish."
        }
        ...
        L10nKey::SettingsTabCompletionDesc => {
            "Tab at the prompt opens nermal's completion menu. When off, Tab goes to the shell's own completion instead."
        }
        L10nKey::SettingsHistorySearch => "History search",
        L10nKey::SettingsHistorySearchDesc => {
            "⌃R at the prompt opens nermal's fuzzy history menu. Off sends ⌃R to the shell — its own reverse-i-search, or whatever you bound there (fzf, percol)."
        }
        ...
        L10nKey::SettingsAgentsIntroDesc => {
            "Hooks give panes running these agents live status (working / waiting / done) in the tab bar. Only inside nermal."
        }
        ...
        L10nKey::SettingsStartupWindowDesc => "Window state when nermal launches.",
        L10nKey::SettingsRememberWindowSize => "Remember window size & position",
        L10nKey::SettingsRememberWindowSizeDesc => {
            "Reopen at the size and position the window had when nermal last quit. Off opens centered at the default size."
        }
        ...
        L10nKey::SettingsDefaultTerminalDesc => {
            "Make nermal the macOS default terminal for Unix executables, SSH links, and man-page links. nermal can also open folders and scripts, but does not replace Finder's folder handler. Apps that choose their own terminal may ignore this setting."
        }
        L10nKey::SettingsDefaultTerminalSet => "Set as Default Terminal",
        L10nKey::SettingsDefaultTerminalSetSuccess => {
            "nermal is now the default handler for supported terminal files and links."
        }
        L10nKey::SettingsDefaultTerminalSetFailed => {
            "Could not set nermal as the default terminal: {error}"
        }
        ...
        L10nKey::SettingsUpdateReadyNextLaunch => {
            "It will be installed the next time you start nermal."
        }
        ...
        L10nKey::SettingsDaemonStaleDesc => {
            "nermal was updated in place: the app is new, your panes are still served by the old build. Restarting the server picks up the new one and ends everything running in your panes. No hurry — do it when they're idle."
        }
        L10nKey::UpdateDialogTitle => "Update available",
        L10nKey::UpdateDialogDetail => {
            "nermal {version} is available — you're on {current}. Installing restarts the app; the background server keeps running, so your panes survive."
        }
        L10nKey::UpdateDialogDetailWindows => {
            "nermal {version} is available — you're on {current}. Installing restarts the app and the background service: processes in your panes are ended, and your tabs and layout come back with fresh shells."
        }
        L10nKey::UpdateDialogDetailManual => {
            "nermal {version} is available — you're on {current}. {hint}"
        }
        ...
        L10nKey::UpdateDialogNeedsElevation => {
            "nermal is installed for all users, so Windows asks for administrator approval once before installing. nermal itself never runs elevated."
        }
        ...
        L10nKey::SettingsUpdateUnsupportedMacos => {
            "This copy is not in a writable nermal.app bundle, so it cannot replace itself. Move nermal to Applications, or open the release page to update."
        }
        ...
        L10nKey::SettingsUpdateWindowsAllUsers => {
            "nermal is installed for all users, so replacing it needs administrator rights that nermal will not ask for itself. Open the release page and run the installer to update."
        }
        ...
        L10nKey::SettingsUpdateMissingChecksums => {
            "The release has no checksums.txt, so nermal refuses to install it automatically."
        }
        ...
        L10nKey::SettingsCommandLineDesc => {
            "Put the bundled nermal command on your PATH, so scripts and agents can drive nermal from any terminal — inside a pane it works either way. Turn off to keep your own build unshadowed. Applies at next launch."
        }
        L10nKey::SettingsInstallCliOnPath => "Install the nermal command on PATH",
        ...
        L10nKey::SettingsAppHttpProxyDesc => {
            "Used only for nermal's update checks and downloads, not for programs in your panes. Empty follows the system proxy."
        }
        ...
        L10nKey::SettingsSearchCommandLineToolKeywords => {
            "command line tool cli nermal path shell command install symlink terminal iterm agent script"
        }
        ...
        L10nKey::FileTreeNewEntryTooltip => "New File or Folder",
        ...
        L10nKey::EditorCloseAllTabs => "Close All Tabs",
        ...
        L10nKey::EditorAutoSaveOn => "Auto-save: on",
        L10nKey::EditorAutoSaveOff => "Auto-save: off",
        ...
        L10nKey::PanelUsageGpu => "GPU",
        L10nKey::PanelUsageByProcess => "By process",
        L10nKey::PanelNetworkTitle => "Network",
        L10nKey::PanelNetworkEmpty => "No active network connections",
        L10nKey::PanelNetworkConnected => "Connected",
        L10nKey::PanelNetworkDisconnected => "Disconnected",
        L10nKey::PanelNetworkReceived => "Received",
        L10nKey::PanelNetworkSent => "Sent",
        L10nKey::PanelNetworkDownloadSpeed => "Download",
        L10nKey::PanelNetworkUploadSpeed => "Upload",
        ...
        L10nKey::PanelPortsUnsupported => "That machine's nermal-server is too old to list ports.",
        ...
        L10nKey::DiffTruncatedBudget => {
            "Body not loaded — past nermal's diff budget. Run git diff in the terminal for this file."
        }
        ...
        L10nKey::DiffBudgetAndCap => "nermal's budget and the per-file cap",
        L10nKey::DiffBudget => "nermal's budget",
        ...
        L10nKey::RemoteNoConnectionDetails => {
            "This window is a workspace on {machine}, but nermal has no connection details for it — check its SSH profile or ~/.ssh/config entry still exists."
        }
        L10nKey::RemoteThisComputer => "this computer",
        L10nKey::RemoteProfileGone => "deleted profile",
        L10nKey::RemoteRestartTitle => "Restart nermal's server on \"{machine}\"?",
        ...
        L10nKey::RemoteReplaceBody => {
            "nermal will install a matching server on {machine} and start it.\n\
             \n\
             Every session running on {machine} ends, including any this window is not \
             connected to."
        }
        L10nKey::RemoteRestartFailedTitle => "nermal's server on \"{machine}\" was not restarted",
        ...
        L10nKey::RemoteInstallTitle => "Install nermal's server on \"{machine}\"?",
        L10nKey::RemoteInstallDetail => {
            "nermal will write its server binary to {machine} so this machine can host \
        ...
        L10nKey::RemoteMismatchTitle => "Update nermal's server on \"{machine}\"?",
        ...
        L10nKey::RemoteServerOutdated => {
            "{machine} is running an old nermal server ({build}) that this copy of nermal \
             cannot talk to. Update it to connect."
        }
        L10nKey::RemoteServerTooNew => {
            "{machine} is running a newer nermal server ({build}) than this copy of nermal. \
             Update nermal on this computer, or replace the server there with a matching one."
        }
        L10nKey::RemoteDaemonStartFailed => "nermal's local server could not be started: {error}",
        L10nKey::RemoteDaemonUnreachable => "could not reach nermal's local server: {error}",
        L10nKey::RemoteDaemonTooOld => {
            "this machine's daemon is an older build and cannot restart the server on {machine}. Quit nermal (that stops the daemon), open it again, and retry."
        }
        ...
        L10nKey::RemoteHostNotnermal => "{machine} answered, but not as a nermal server: {error}",
        ...
        L10nKey::RemoteServerRestartFailed => {
            "could not restart nermal's server on {machine}: {error}"
        }
        L10nKey::RemoteNoRouteToHost => "nermal no longer has a way to reach {machine}",
        ...
        L10nKey::SwitcherStartingServer => "Starting nermal's server…",
        L10nKey::SwitcherDownloadingServerWithTotal => {
            "Downloading nermal's server… {done} / {total}"
        }
        L10nKey::SwitcherDownloadingServerNoTotal => "Downloading nermal's server… {done}",
        L10nKey::SwitcherCopyingServer => "Copying nermal's server… {done} / {total}",
        ...
        L10nKey::CmdRightPanelNetwork => "Right Panel: Network",
        ...
        L10nKey::CmdAboutnermal => "About nermal",
        ...
        L10nKey::CmdQuitnermal => "Quit nermal",
        L10nKey::CmdQuitnermalSubtitle => "stops the server; every running shell ends",
        ...
        L10nKey::ConfigQuarantinedStartup => {
            "config.json could not be parsed. nermal is on default settings and kept the file's contents beside it as config.json.corrupt. Fix it and nermal reloads; until then, Settings changes are not saved."
        }
        L10nKey::ConfigQuarantinedReload => {
            "The edited config.json could not be parsed. nermal kept the settings it is running on and set the file's contents aside as config.json.corrupt. Fix it and nermal reloads; saving a setting first overwrites it."
        }
        L10nKey::ConfigUnreadableStartup => {
            "config.json could not be read. nermal is on default settings and left the file exactly as it is. Fix its permissions or contents and nermal reloads; until then, Settings changes are not saved."
        }
        L10nKey::ConfigUnreadableReload => {
            "config.json could not be read. nermal kept the settings it is running on and left the file exactly as it is. Fix its permissions or contents and nermal reloads; saving a setting first overwrites it."
        }
        ...
        L10nKey::AppForkNoCommand => "nermal has no fork command for {name}",
        L10nKey::AppForkLocalOnly => "{name} sessions can only be forked from a local pane",
        L10nKey::AppForkNoSessionId => {
            "nermal hasn't seen a {name} session id in this pane — install its hooks in Settings → Agents"
        }
        ...
        L10nKey::AppAgentHooksNoHomeDir => {
            "nermal could not work out this computer's home directory, so there is nowhere to install to."
        }
        ...
        L10nKey::AppAgentHooksNonermalHooks => "No nermal hooks found; nothing to remove",
        ...
        L10nKey::SettingsShellDetected => "Shells nermal found",
        ...
        L10nKey::SettingsDaemonStaleDescInPlace => {
            "nermal was updated in place: the app is new, your panes still run on the old build. The server can swap itself for the new one without stopping, so your shells carry straight over. Panes on nermal's built-in SSH client are the exception — those close and need reopening."
        }
        L10nKey::AppRestartServerBodyInPlace => {
            "The server swaps itself for this build in place: your shells keep running, and the window reconnects a moment later. Panes on nermal's built-in SSH client are the exception — those close and need reopening."
        }
        ...
        L10nKey::SettingsPerPaneHistoryDescription => {
            "Up walks through what you ran in this pane, not every pane interleaved. A new pane starts from your existing history and writes back what it adds when it closes. Applies to bash and zsh panes nermal can set up; a shell started with your own arguments is left alone."
        }
        ...
        L10nKey::IntegrationNoticeNotEngaged => {
            "nermal shell integration hasn't engaged in this pane, so inline completion and the Ctrl+R menu are unavailable. Usual causes: a shell you started with your own arguments, a PTY wrapper, or an unsupported shell."
        }
        ...
        L10nKey::TrayTooltipAgents => "nermal — {parts}",
        ...
        L10nKey::AppMenuNewFile => "New File",
        ...
        L10nKey::AppMenuDocumentation => "nermal Documentation",
        ...
        L10nKey::TrayShownermal => "Show nermal",
