...
pub fn translate_zh(key: L10nKey) -> Option<&'static str> {
    Some(match key {
        ...
        L10nKey::EditorEmptyExploreHint => "您可以通过左侧边栏浏览文件并进行编辑",
        ...
        L10nKey::QuitStopServerBody => {
            "这会退出 nermal 并停止后台 server，shell 里正在跑的东西都会被终止。标签页和布局会保留，下次启动时以全新的 shell 打开。（只关窗口的话应用收进托盘，shell 继续跑。）"
        }
        ...
        L10nKey::SettingsLanguageDesc => "选择 nermal 界面使用的语言。",
        ...
        L10nKey::ExplorerMenuOpenIn => "在 nermal 中打开",
        L10nKey::ExplorerMenuOpenHere => "在此处打开 nermal",
        L10nKey::SettingsCustomThemesIntro => {
            "复制一个主题即可在此编辑颜色，或把 nermal YAML 主题、iTerm2 .itermcolors 文件放进主题文件夹。"
        }
        ...
        L10nKey::SettingsInnermal => "在 nermal 中",
        ...
        L10nKey::SettingsTypeAddressToConnect => "输入地址即可立刻连接，之后 nermal 会提示保存。",
        ...
        L10nKey::SettingsImportAliasesDesc => {
            "重新读取文件并添加新内容。你在这里做的编辑由 nermal 保存——不会写入该文件本身。"
        }
        ...
        L10nKey::SettingsImportIgnored => {
            "有 {count} 个选项在 nermal 中没有对应设置，仍留在文件里：{options}"
        }
        ...
        L10nKey::SettingsPromptIntro => {
            "shell 提示符处的 nermal 自带编辑器与菜单。关闭某项即可把这部分交还给 shell。"
        }
        L10nKey::SettingsPromptEditor => "提示符编辑器",
        L10nKey::SettingsPromptEditorDesc => {
            "由 nermal 编辑你在 shell 提示符上敲的这一行：选择、撤销，以及下面这些菜单。关闭后交还给 shell 自己的行编辑器——ZLE、readline、fish。"
        }
        ...
        L10nKey::SettingsTabCompletionDesc => {
            "在提示符按 Tab 打开 nermal 的补全菜单。关闭后 Tab 交由 shell 自身的补全处理。"
        }
        L10nKey::SettingsHistorySearch => "历史搜索",
        L10nKey::SettingsHistorySearchDesc => {
            "在提示符按 ⌃R 打开 nermal 的模糊历史菜单。关闭后 ⌃R 交给 shell——它自带的反向搜索，或你绑定的其它功能（fzf、percol）。"
        }
        ...
        L10nKey::SettingsAgentsIntroDesc => {
            "hook 让跑这些 agent 的窗格在标签栏实时显示状态（进行中 / 等待中 / 已完成）。仅在 nermal 内生效。"
        }
        ...
        L10nKey::SettingsStartupWindowDesc => "nermal 启动时的窗口状态。",
        L10nKey::SettingsRememberWindowSize => "记住窗口大小与位置",
        L10nKey::SettingsRememberWindowSizeDesc => {
            "以 nermal 上次退出时窗口的大小和位置重新打开。关闭时以默认大小居中打开。"
        }
        ...
        L10nKey::SettingsDefaultTerminalDesc => {
            "将 nermal 设为 Unix 可执行文件、SSH 链接和 man 页面链接的 macOS 默认终端。nermal 仍可打开文件夹和脚本，但不会替换 Finder 的文件夹处理程序。自行指定终端的应用可能不会遵循此设置。"
        }
        L10nKey::SettingsDefaultTerminalSet => "设为默认终端",
        L10nKey::SettingsDefaultTerminalSetSuccess => {
            "nermal 已成为受支持终端文件和链接的默认处理程序。"
        }
        L10nKey::SettingsDefaultTerminalSetFailed => "无法将 nermal 设为默认终端：{error}",
        ...
        L10nKey::SettingsUpdateReadyNextLaunch => "下次启动 nermal 时会自动装上。",
        ...
        L10nKey::SettingsDaemonStaleDesc => {
            "nermal 是原地升级的：界面已是新版，pane 还由旧版 server 托管。重启 server 换成新版，代价是 pane 里正在跑的进程全部结束。不急，挑 pane 空闲时再重启。"
        }
        L10nKey::UpdateDialogTitle => "有可用更新",
        L10nKey::UpdateDialogDetail => {
            "nermal {version} 已发布，你现在是 {current}。安装会重启应用；后台 server 不动，pane 里的东西都还在。"
        }
        L10nKey::UpdateDialogDetailWindows => {
            "nermal {version} 已发布，你现在是 {current}。安装会重启应用和后台 server：pane 里的进程会被结束，标签页和布局以全新的 shell 恢复。"
        }
        L10nKey::UpdateDialogDetailManual => "nermal {version} 已发布，你现在是 {current}。{hint}",
        ...
        L10nKey::UpdateDialogNeedsElevation => {
            "nermal 是为所有用户安装的，安装前 Windows 会请求一次管理员批准。nermal 本身不会以管理员身份运行。"
        }
        ...
        L10nKey::SettingsUpdateUnsupportedMacos => {
            "当前副本不在可写的 nermal.app 包里，无法自我替换。请把 nermal 移到“应用程序”，或打开发布页面更新。"
        }
        ...
        L10nKey::SettingsUpdateWindowsAllUsers => {
            "nermal 是为所有用户安装的，替换需要管理员权限，而 nermal 不会自行提权。请打开发布页面，自行运行安装程序更新。"
        }
        ...
        L10nKey::SettingsUpdateMissingChecksums => {
            "该版本缺少 checksums.txt，因此 nermal 拒绝自动安装。"
        }
        ...
        L10nKey::SettingsCommandLineDesc => {
            "把自带的 nermal 命令加入 PATH，让脚本和 agent 能从任意终端驱动 nermal——在 nermal 窗格内两种情况都可用。自己构建的 nermal 不想被遮蔽就关掉。下次启动生效。"
        }
        L10nKey::SettingsInstallCliOnPath => "将 `nermal` 命令安装到 PATH",
        ...
        L10nKey::SettingsAppHttpProxyDesc => {
            "仅用于 nermal 自身的更新检查和下载，不影响面板中运行的程序。留空则跟随系统代理。"
        }
        ...
        L10nKey::SettingsSearchCommandLineToolKeywords => {
            "命令行工具 cli nermal 路径 shell 命令 安装 符号链接 terminal command line tool"
        }
        ...
        L10nKey::FileTreeNewEntryTooltip => "新建文件或文件夹",
        ...
        L10nKey::EditorCloseAllTabs => "关闭所有标签页",
        ...
        L10nKey::EditorAutoSaveOn => "自动保存：开",
        L10nKey::EditorAutoSaveOff => "自动保存：关",
        ...
        L10nKey::PanelNewCategory => "新建分类",
        L10nKey::PanelSearchTitle => "搜索",
        L10nKey::PanelSearching => "搜索中…",
        L10nKey::PanelSearchSummary => "在 {files} 个文件中找到 {matches} 处匹配",
        L10nKey::PanelSearchTruncated => "已提前停止",
        L10nKey::PanelSearchEmpty => "无结果",
        L10nKey::PanelReplaceAll => "全部替换",
        L10nKey::PanelUsageTitle => "资源占用",
        L10nKey::PanelUsageLoading => "测量中…",
        L10nKey::PanelUsageCpu => "CPU",
        L10nKey::PanelUsageMemory => "内存",
        L10nKey::PanelUsageProcesses => "进程数",
        L10nKey::PanelUsageGpu => "GPU",
        L10nKey::PanelUsageByProcess => "按进程",
        L10nKey::PanelNetworkTitle => "网络",
        L10nKey::PanelNetworkEmpty => "没有活动的网络连接",
        L10nKey::PanelNetworkConnected => "已连接",
        L10nKey::PanelNetworkDisconnected => "已断开",
        L10nKey::PanelNetworkReceived => "已接收",
        L10nKey::PanelNetworkSent => "已发送",
        L10nKey::PanelNetworkDownloadSpeed => "下载",
        L10nKey::PanelNetworkUploadSpeed => "上传",
        L10nKey::PanelNetworkNoRate => "—",
        L10nKey::SettingsSourceControl => "源代码管理",
        L10nKey::SettingsScmPostCommit => "提交后",
        L10nKey::SettingsScmPostCommitDesc => "提交按钮接下来会做什么，适用于所有工作区。",
        L10nKey::PanelActivitySubtitle => "活动",
        L10nKey::PanelKillProcess => "终止进程",
        L10nKey::PanelKillProcessConfirm => "终止进程 {pid}？其中未保存的内容将会丢失。",
        ...
        L10nKey::PanelPortsUnsupported => "对端的 nermal-server 太旧，列不出端口。",
        ...
        L10nKey::DiffTruncatedBudget => {
            "内容未加载——已超出 nermal 的 diff 预算。在终端运行 git diff 查看此文件。"
        }
        ...
        L10nKey::DiffBudgetAndCap => "nermal 的预算和单文件上限",
        L10nKey::DiffBudget => "nermal 的预算",
        ...
        L10nKey::RemoteNoConnectionDetails => {
            "此窗口是 {machine} 上的工作区，但 nermal 没有它的连接信息了——检查其 SSH 配置或 ~/.ssh/config 条目是否还在。"
        }
        L10nKey::RemoteThisComputer => "本机",
        L10nKey::RemoteProfileGone => "已删除的配置",
        L10nKey::RemoteRestartTitle => "重启“{machine}”上的 nermal server？",
        L10nKey::RemoteRestartBody => {
            "这会结束 {machine} 上的所有 shell，包括此窗口没显示的。工作区和布局会保留，并以全新的 shell 恢复。"
        }
        L10nKey::RemoteReplaceBody => {
            "nermal 会在 {machine} 上安装匹配的 server 并启动它。\n\
             \n\
             {machine} 上运行的所有会话都会结束，包括此窗口未连接的会话。"
        }
        L10nKey::RemoteRestartFailedTitle => "“{machine}”上的 nermal server 未被重启",
        ...
        L10nKey::RemoteInstallTitle => "在“{machine}”上安装 nermal server？",
        L10nKey::RemoteInstallDetail => {
            "nermal 会将其 server 二进制文件写入 {machine}，以便本机可以在那里托管\
        ...
        L10nKey::RemoteMismatchTitle => "更新“{machine}”上的 nermal server？",
        ...
        L10nKey::RemoteServerOutdated => {
            "{machine} 上的 nermal server 太旧（{build}），当前这份 nermal 连不上它。\
             更新它才能连接。"
        }
        L10nKey::RemoteServerTooNew => {
            "{machine} 上的 nermal server（{build}）比当前这份 nermal 还新。\
             请更新本机的 nermal，或把那边的 server 替换成匹配的版本。"
        }
        L10nKey::RemoteDaemonStartFailed => "无法启动 nermal 本地 server：{error}",
        L10nKey::RemoteDaemonUnreachable => "无法连接到 nermal 本地 server：{error}",
        L10nKey::RemoteDaemonTooOld => {
            "本机的 nermal 守护进程版本较旧，无法重启 {machine} 上的 server。请退出 nermal（这会停止守护进程）再打开，然后重试。"
        }
        ...
        L10nKey::RemoteHostNotnermal => "{machine} 已响应，但并非作为 nermal server：{error}",
        L10nKey::RemoteWorkspaceListFailed => "已连接到 {machine}，但其工作区列表获取失败：{error}",
        L10nKey::RemoteServerRestartFailed => "无法重启 {machine} 上的 nermal server：{error}",
        L10nKey::RemoteNoRouteToHost => "nermal 已无法到达 {machine}",
        ...
        L10nKey::SwitcherStartingServer => "正在启动 nermal server…",
        L10nKey::SwitcherDownloadingServerWithTotal => "正在下载 nermal server… {done} / {total}",
        L10nKey::SwitcherDownloadingServerNoTotal => "正在下载 nermal server… {done}",
        L10nKey::SwitcherCopyingServer => "正在复制 nermal server… {done} / {total}",
        ...
        L10nKey::SwitcherFormFolder => "文件夹",
        L10nKey::SwitcherFormChooseFolder => "选择文件夹…",
        L10nKey::SwitcherFormFolderRequiredHint => "选择一个文件夹以创建工作区",
        ...
        L10nKey::CmdRightPanelAgents => "右侧面板：代理",
        L10nKey::CmdRightPanelSearch => "右侧面板：搜索",
        L10nKey::CmdRightPanelUsage => "右侧面板：资源占用",
        L10nKey::CmdRightPanelNetwork => "右侧面板：网络",
        ...
        L10nKey::CmdAboutnermal => "关于 nermal",
        ...
        L10nKey::CmdQuitnermal => "退出 nermal",
        L10nKey::CmdQuitnermalSubtitle => "停止服务；结束所有运行中的 shell",
        ...
        L10nKey::ConfigQuarantinedStartup => {
            "config.json 无法解析。nermal 正以默认设置运行，原内容已保留为旁边的 config.json.corrupt。修好后会自动重载；在此之前，设置里的更改不会保存。"
        }
        L10nKey::ConfigQuarantinedReload => {
            "修改后的 config.json 无法解析。nermal 保留了当前在用的设置，文件内容另存为旁边的 config.json.corrupt。修好后会自动重载；在此之前保存设置会覆盖它。"
        }
        L10nKey::ConfigUnreadableStartup => {
            "config.json 读取失败。nermal 正以默认设置运行，文件原样保留。修好权限或内容后会自动重载；在此之前，设置里的更改不会保存。"
        }
        L10nKey::ConfigUnreadableReload => {
            "config.json 读取失败。nermal 保留了当前在用的设置，文件也原样保留。修好权限或内容后会自动重载；在此之前保存设置会覆盖它。"
        }
        ...
        L10nKey::AppForkNoCommand => "nermal 没有用于 {name} 的 fork 命令",
        L10nKey::AppForkLocalOnly => "{name} 会话只能从本地窗格 fork",
        L10nKey::AppForkNoSessionId => {
            "nermal 尚未在此窗格中看到 {name} 的会话 ID——请在设置 → Agents 中安装其 hook"
        }
        ...
        L10nKey::AppAgentHooksNoHomeDir => {
            "nermal 无法确定这台计算机的主目录，因此没有可安装的位置。"
        }
        ...
        L10nKey::AppAgentHooksNonermalHooks => "没有找到 nermal 钩子，无需移除",
        ...
        L10nKey::SettingsShellDetected => "nermal 检测到的 shell",
        ...
        L10nKey::SettingsDaemonStaleDescInPlace => {
            "nermal 是原地更新的：应用是新的，面板还跑在旧版上。server 可以不停机就换成新版，shell 直接延续下来。用 nermal 内置 SSH 客户端的面板除外——那些连接会断开，需要重新打开。"
        }
        L10nKey::AppRestartServerBodyInPlace => {
            "后台 server 会原地把自己换成当前这个版本：shell 继续运行，窗口稍后自动连回去。用 nermal 内置 SSH 客户端的面板除外——那些连接会断开，需要重新打开。"
        }
        ...
        L10nKey::SettingsPerPaneHistoryDescription => {
            "上方向键翻的是这个面板里跑过的命令，而不是所有面板混在一起。新面板从已有历史开始，关闭时把新增的写回去。只对 nermal 能接管的 bash 和 zsh 面板生效；用你自己参数启动的 shell 不受影响。"
        }
        ...
        L10nKey::IntegrationNoticeNotEngaged => {
            "此窗格的 nermal shell 集成没生效，内联补全和 Ctrl+R 菜单不可用。常见原因：用自己参数启动的 shell、PTY 包装器，或不受支持的 shell。"
        }
        ...
        L10nKey::AppMenuDocumentation => "nermal 文档",
        ...
        L10nKey::TrayShownermal => "显示 nermal",
...
pub fn translate_variant_zh(key: L10nKey, branch: &'static str) -> Option<&'static str> {
    let res = match (key, branch) {
        ...
        (L10nKey::SettingsImportIgnored, "zero") => "文件里的每个选项在 nermal 中都有对应设置。",
        (L10nKey::SettingsImportIgnored, "one") => {
            "有 1 个选项在 nermal 中没有对应设置，仍留在文件里：{options}"
        }
