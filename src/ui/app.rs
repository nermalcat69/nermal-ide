...
// nermal-core next to the validation: a local, narrower pair used to push a
...
/// of the set fills.
///
/// Every icon nermal draws itself sits on the same optical bound — 3.4..20.6 of
...
impl Tab {
    ...
    pub(crate) fn from_tree(tree: &nermal_core::core::machine::Tab, pane: Pane) -> Self {
        Self {
            pane,
    ...
    ///
    /// [`TabView`](nermal_core::core::tab_view::TabView) is how a tab looks to
    /// someone who is *not* the window showing it — the switcher listing
    /// another window's workspace, `nermal tab ls` on the far side of a socket.
    ...
    /// a directory through
    /// [`TabView::label`](nermal_core::core::tab_view::TabView::label), so the
    ...
    pub(crate) fn label_view(
        ...
        nermal_core::core::tab_view::TabView,
        ...
        let leaf = leaf.read(cx);
        let view = nermal_core::core::tab_view::TabView {
            id: self.tree_id.get(),
            name,
            ...
            // name nermal chose for it deliberately, and `stated_title` hands
...
pub(crate) struct Renaming {
    ...
    pub(crate) tab: nermal_core::core::machine::TabId,
...
pub struct NermalApp {
    ...
    pending_tab: Option<nermal_core::core::machine::TabId>,
    ...
    pub(crate) tab_merge: Cell<
        Option<(
            nermal_core::core::machine::TabId,
    ...
    /// finishing is what completes it (see `NermalApp::finish_connect`).
...
pub(crate) enum CloseTarget {
    Tab(nermal_core::core::machine::TabId),
    Pane,
}
...
impl NermalApp {
    ...
    fn prompt_daemon_version_mismatch(window: &mut Window, cx: &mut Context<Self>) {
        ...
        let answer = window.prompt(
            ...
            Some(&detail),
            &[t(L10nKey::CmdQuitnermal), t(L10nKey::RestartServer)],
            cx,
        );
        ...
        let (tabs, active) = match session {
            None => match new_terminal(
                ...
                Err(e) => {
                    ...
                    // everything, and it used to be what they saw when nermal
    ...
    pub(crate) fn detach_workspace(&self, cx: &mut App) {
            ...
            && crate::ui::tree_sync::workspace_is_disposable(cx, self.workspace)
        {
            crate::ui::tree_sync::fire_workspace_op(cx, self.workspace, |ws| {
                nermal_core::daemon::control::ControlRequest::WorkspaceRemove { workspace: ws }
            });
    ...
    /// by reading that window's `NermalApp` back out of the registry. Called from
    ...
    /// writer (see [`NermalApp::remember_focused_leaf`]) — it stays because the
    ...
    fn tab_landing_rect(
        &self,
        id: nermal_core::core::machine::TabId,
        window: &Window,
    ) -> Option<(
        crate::ui::pane_drag::DropZone<gpui::EntityId>,
        Bounds<Pixels>,
    ...
    fn merge_tab(
        &mut self,
        source: nermal_core::core::machine::TabId,
    ...
    fn deliver_agent_prompt(&mut self, prompt: &str, window: &mut Window, cx: &mut Context<Self>) {
        let Some(target) = self.agent_target_leaf(cx) else {
            crate::terminal::notify_desktop(Some("nermal"), t(L10nKey::AppNoRunningCodingAgent));
            return;
        };
    ...
    fn send_selection_to_agent(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let Some(selection) = selection else {
            crate::terminal::notify_desktop(Some("nermal"), t(L10nKey::AppNothingSelected));
            return;
        };
    ...
    fn send_git_diff_to_agent(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let Some((host, cwd)) = target else {
            crate::terminal::notify_desktop(Some("nermal"), t(L10nKey::AppPaneNoKnownDirectory));
            return;
        };
        crate::ui::host_ops::HostOps::run_in(
            host,
    ...
    fn commit_http_proxy(&mut self, cx: &mut Context<Self>) {
        ...
        if !value.is_empty() && !nermal_core::daemon::install::proxy::is_valid_manual(&value) {
            cx.notify();
            return;
        }
    ...
    fn commit_working_directory_path(&mut self, cx: &mut Context<Self>) {
        ...
        // my project" reads as a nermal bug rather than a typo (#601). Refuse
    ...
    /// lives in `nermal-core` and cannot reach `src/ui/i18n` — it used to hand
    ...
    fn agent_hooks_outcome_msg(outcome: &crate::core::agent_hooks::HookOutcome) -> String {
        ...
        match outcome {
            ...
            O::NonermalHooks => t(L10nKey::AppAgentHooksNonermalHooks).to_string(),
        }
    }
...
fn pane_free_for(
    ...
    match alive.get(&id) {
        ...
        Some(Some(recorded)) => {
            ...
            // stamping its own name — `nermal` before this release wrote a
            // literal "nermal-cli" — and refusing on it strands every pane the
    ...
    for st in &session.tabs {
        ...
        tabs.push(Tab {
            ...
            tree_id: std::cell::Cell::new(
                st.tree_id
                    .unwrap_or_else(nermal_core::core::machine::TabId::new),
            ),
            last_used: std::cell::Cell::new(0),
            focus_origin: Default::default(),
        });
...
mod tests {
    ...
    fn a_start_in_path_saves_only_when_it_names_a_real_directory() {
        ...
        let file = std::env::temp_dir().join("nermal-wd-saveable-probe");
    ...
    fn shell_arguments_round_trip_between_field_text_and_argv() {
        for case in [
            ...
            &[r"--dir", r"C:\Users\me"],
            &[r"--dir", r"C:\Program Files\nermal"],
            &["--tag", "#1"],
        ] {
            let argv = argv(case);
            let field_text = join_shell_args(&argv);
            assert_eq!(
                split_shell_args(&field_text).expect("nermal's own refill must parse"),
                argv,
    ...
    fn unbalanced_quotes_are_refused_not_shredded() {
        ...
        assert!(split_shell_args("--name 'nermal").is_err());
...
pub(crate) mod test_window {
    ...
    pub(crate) fn harness(cx: &mut TestAppContext) -> (Entity<NermalApp>, VisualTestContext) {
...
mod ssh_rebuild_gpui_tests {
    ...
    use crate::ui::pane::{Pane, PaneSlot};
    use gpui::TestAppContext;
    use nermal_core::core::machine::{LayoutDelta, PaneNode, Tab as TreeTab};
...
mod keybinding_gpui_tests {
    ...
    fn begin_capture(app: &Entity<NermalApp>, vcx: &mut VisualTestContext, action: &str) {
...
mod restart_server_gpui_tests {
    ...
    async fn settling_after_a_restart_does_not_lease_the_window_twice(cx: &mut TestAppContext) {
        ...
        NermalApp::settle_after_restart(app.downgrade(), Ok(()), &mut cx.to_async()).await;
...
mod shell_menu_gpui_tests {
    ...
    fn harness(cx: &mut TestAppContext) -> (Entity<NermalApp>, VisualTestContext) {
        crate::core::config::pin_test_config_dir();
        cx.executor().allow_parking();
        cx.update(|cx| {
            gpui_component::init(cx);
...
// `NermalApp::render` declares the active tab's panes displayed each frame and
...
mod displayed_gpui_tests {
    ...
    fn pane_id(app: &super::NermalApp, tab: usize) -> gpui::EntityId {
        app.tabs[tab]
            .pane
            .first_leaf()
...
mod new_window_action_tests {
    ...
    /// nermal to the tray rather than quitting it: the process is alive, the
    /// menu bar is still nermal's, and there is nothing on screen. A listener
    /// that lives only on `NermalApp`'s render root reaches nothing there, and
