...
/// The smallest window nermal still holds its shape in.
///
...
impl WindowRegistry {
    ...
    pub fn open_windows(cx: &mut App) -> Vec<(WorkspaceId, WeakEntity<NermalApp>)> {
        Self::sweep(cx);
        cx.global::<Self>()
            .windows
            .iter()
    ...
    pub fn app_in(cx: &mut App, window: &Window) -> Option<gpui::Entity<NermalApp>> {
        Self::sweep(cx);
    ...
    pub fn app_for(cx: &mut App, workspace: WorkspaceId) -> Option<WeakEntity<NermalApp>> {
        Self::sweep(cx);
        cx.global::<Self>()
            .windows
            .iter()
...
pub fn open_at_tab(cx: &mut App, workspace: WorkspaceId, tab: nermal_core::core::machine::TabId) {
...
pub fn open_workspace_at(
    ...
    let mut created: Option<gpui::Entity<NermalApp>> = None;
    let opened = cx.open_window(options, |window, cx| {
        let app = cx.new(|cx| match initial_cwd.clone() {
            Some(cwd) => NermalApp::for_workspace_at(workspace, Some(cwd), window, cx),
            None => NermalApp::for_workspace(workspace, window, cx),
        });
...
/// its id, and no other window would do. Everything else is `nermal [PATH]`,
...
/// windowless entry point, so a tray-resident nermal comes back to the workspace
/// it retired with instead of claiming a fresh one.
pub fn quick_connect_from_url(cx: &mut App, ssh: nermal_core::core::ssh_profile::QuickConnect) {
...
/// to disagree: "Open in nermal" from Explorer, or `nermal <PATH>`, skipped the
...
/// Brings the UI back when macOS relaunches a nermal that is already running.
///
/// AppKit only asks when it found no visible window, and that is a state
/// nermal lives in on purpose: closing the last window with the tray icon on
...
fn window_options(cx: &mut App, workspace: Option<WorkspaceId>) -> WindowOptions {
    ...
    WindowOptions {
        ...
        // The title bar above is nermal's own, so the compositor must not add a
...
mod tests {
    ...
    fn a_window_asks_the_compositor_for_client_side_decorations(cx: &mut gpui::TestAppContext) {
        // #679: nermal draws its own title bar, so the request must say so —
    ...
    fn a_request_carrying_a_path_restores_the_layout_and_brings_the_path_along(
        ...
        // The Explorer "Open in nermal" case with no window up: the folder is
    ...
    fn a_delete_reads_its_kill_list_before_the_removal_blanks_the_mirror(
        ...
        use nermal_core::core::machine::{Machine, PaneRecord, Tab, Workspace as TreeWorkspace};
    ...
    fn forgetting_a_workspace_never_tells_its_machine(cx: &mut gpui::TestAppContext) {
        ...
        use nermal_core::core::machine::{Machine, Workspace as TreeWorkspace};

        // `WorkspaceStore::remove` saves; a test has no business writing the
        // real views.
        let _ = nermal_core::core::config::set_config_dir(
            std::env::temp_dir().join(format!("nermal-windows-test-{}", std::process::id())),
        );
        cx.update(|cx| {
