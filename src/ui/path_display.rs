...
/// The directory `~` stands for on the machine nermal is running on, or `None`
/// when it won't say.
///
...
///
/// The rule itself lives in [`nermal_core::core::path_spelling`], next to the
...
pub(crate) fn native_separators(path: &Path) -> Cow<'_, Path> {
    nermal_core::core::path_spelling::native_separators(path)
}
...
/// no link, or one whose shell has ssh'd somewhere nermal never spoke to. It
...
mod tests {
    ...
    fn native_separators_rewrites_forward_slashes_to_backslashes_on_windows() {
        ...
        assert_eq!(
            native_separators(Path::new("D:/code/nermal\\skills")),
            Path::new("D:\\code\\nermal\\skills")
        );
        assert_eq!(
            native_separators(Path::new("D:/code/nermal")),
            Path::new("D:\\code\\nermal")
        );
    }
    ...
    fn native_separators_is_a_no_op_off_windows() {
        ...
        for p in ["/home/u/nermal", "C:\\Users\\dev", "mixed/path\\here"] {
            let got = native_separators(Path::new(p));
            assert_eq!(got.as_ref(), Path::new(p), "{p:?}");
