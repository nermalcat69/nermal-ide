...
fn capped(text: &str) -> String {
    ...
    format!(
        "{}\n[… truncated by nermal — ask for the rest if needed]",
        &text[..end]
    )
}
...
mod tests {
    ...
    fn selection_prompt_embeds_the_text_and_cwd() {
        let p = build_selection_prompt("error[E0308]: mismatched types", Some("/work/nermal"))
            .expect("non-empty selection builds");
        assert!(p.contains("error[E0308]"));
        assert!(p.contains("/work/nermal"));
        assert!(p.contains("```"));
    ...
    fn oversized_context_is_truncated_with_a_note() {
        ...
        assert!(p.contains("truncated by nermal"));
    }
