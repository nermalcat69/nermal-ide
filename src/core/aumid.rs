...
//! touches at most the single per-user `nermal.lnk` that Inno's default install
//! owns anyway — never a second Start Menu entry beside an all-users install
//! (which would show "nermal" twice and outlive the uninstaller), and never
...
pub(crate) const AUMID: &str = "com.github.nermal";
...
fn decide() -> Result<Decision, String> {
    ...
    // An elevated install owns `%ProgramData%\...\nermal.lnk`, which we cannot
    // rewrite unelevated. A per-user twin beside it would list "nermal" twice in
...
/// `target[\<triple>]\{debug,release}\nermal-app.exe`. `CACHEDIR.TAG` is the
...
mod tests {
    ...
    fn start_menu_shortcut_lives_under_programs() {
        ...
        assert_eq!(lnk.file_name().and_then(|n| n.to_str()), Some("nermal.lnk"));
        assert_eq!(
            lnk.parent()
    ...
    fn cargo_layouts_are_recognized_as_build_output() {
        for exe in [
            r"C:\src\nermal\target\debug\nermal-app.exe",
            r"C:\src\nermal\target\release\nermal-app.exe",
            r"C:\src\nermal\target\x86_64-pc-windows-msvc\release\nermal-app.exe",
            r"C:\src\nermal\TARGET\Debug\nermal-app.exe",
        ] {
            assert!(
    ...
    fn a_written_shortcut_reads_back_as_ours() {
        let dir = std::env::temp_dir().join(format!("nermal-aumid-{}", std::process::id()));
        std::fs::create_dir_all(&dir).expect("create temp dir");
        let lnk = dir.join("nermal.lnk");
    ...
    /// what `decide()` chose and pops a real toast, which should carry the nermal
    ...
    fn sends_a_branded_toast() {
        ...
        crate::terminal::notify_desktop(Some("nermal"), "AUMID toast test");
