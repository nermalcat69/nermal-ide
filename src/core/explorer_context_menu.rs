...
//!
//! The registry is user-visible system state, so nermal never writes these verbs
...
//!
//! Both verbs invoke the GUI-subsystem `nermal-app.exe` directly so Explorer never
...
const DIRECTORY_KEY: &str = r"Software\Classes\Directory\shell\nermal";
const BACKGROUND_KEY: &str = r"Software\Classes\Directory\Background\shell\nermal";
...
impl Location {
    ...
    ///
    /// Explorer reads this string from the registry, not from nermal, so what is
...
fn replace_entry_with(
    ...
    // The verb root belongs exclusively to nermal. Replacing it at the operation
...
/// Remove only the two Explorer verb trees owned by nermal.
...
/// without this a user who switches nermal to Chinese keeps English entries in
...
fn application_path() -> Result<PathBuf> {
    let app = std::env::current_exe().context("locating the running nermal application")?;
    app.parent()
        .context("the running nermal application has no parent directory")?;
    Ok(app)
}
...
mod windows {
    ...
    fn create_key(path: &str) -> Result<RegistryKey> {
        ...
        if code != ERROR_SUCCESS {
            return Err(io_error("creating the nermal Explorer registry key", code));
        }
        Ok(RegistryKey(key))
    }
    ...
    fn set_string(key: &RegistryKey, name: Option<&OsStr>, value: &OsStr) -> Result<()> {
        ...
        let byte_len = u32::try_from(value.len() * 2)
            .context("the nermal Explorer registry value is too long")?;
        ...
        if code == ERROR_SUCCESS {
            ...
            Err(io_error("writing a nermal Explorer registry value", code))
        }
    }
    ...
    fn delete_tree(path: &str) -> Result<()> {
        let path = wide(OsStr::new(path));
        // SAFETY: `path` is a live, NUL-terminated UTF-16 string. Only nermal's
        ...
        match code {
            ...
            other => Err(io_error("removing the nermal Explorer registry key", other)),
        }
    }
    ...
    /// The verb key, or `None` when nermal's menu is not installed.
    ///
    ...
    fn open_existing(path: &str) -> Result<Option<RegistryKey>> {
        ...
        match code {
            ...
            other => Err(io_error("opening the nermal Explorer registry key", other)),
        }
    }
...
mod tests {
    ...
    fn registration_targets_both_directory_surfaces() {
        ...
        let app = Path::new(r"C:\Program Files\nermal\nermal-app.exe");
        let [directory, background] = registrations(app);

        assert_eq!(directory.location.key(), DIRECTORY_KEY);
        assert_eq!(directory.location.label(), "Open in nermal");
        assert_eq!(directory.icon, app.as_os_str());
        assert_eq!(
            directory.command,
            r#""C:\Program Files\nermal\nermal-app.exe" --open-path "%1""#
        );

        assert_eq!(background.location.key(), BACKGROUND_KEY);
        assert_eq!(background.location.label(), "Open nermal here");
        assert_eq!(background.icon, app.as_os_str());
        assert_eq!(
            background.command,
            r#""C:\Program Files\nermal\nermal-app.exe" --open-path "%V""#
        );
    }

    /// The entries Explorer shows were English whatever language nermal ran in,
    ...
    fn the_verb_labels_follow_the_ui_language() {
        ...
        assert_eq!(Location::Directory.label(), "在 nermal 中打开");
        assert_eq!(Location::Background.label(), "在此处打开 nermal");

        crate::ui::i18n::set_locale("ja-JP");
        assert_eq!(Location::Directory.label(), "nermal で開く");
        assert_eq!(Location::Background.label(), "ここで nermal を開く");

        crate::ui::i18n::set_locale("en");
        assert_eq!(Location::Directory.label(), "Open in nermal");
        assert_eq!(Location::Background.label(), "Open nermal here");
    }

    #[test]
    fn commands_quote_even_paths_without_spaces() {
        assert_eq!(
            quoted_open_command(Path::new(r"C:\nermal\nermal-app.exe"), "%1"),
            r#""C:\nermal\nermal-app.exe" --open-path "%1""#
        );
    }
