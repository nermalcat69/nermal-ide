...
pub(super) struct LastLayout {
    ...
    /// The wrap width text was last laid out against. No longer read: both
    /// call sites that used to re-apply it (`set_soft_wrap`, `set_input_bounds`)
    /// now measure the input's current width directly instead, since this
    /// snapshot goes stale the moment the container resizes or wrap toggles
    /// off. Kept on the struct for whatever else may construct/inspect a
    /// `LastLayout` wholesale.
    #[allow(dead_code)]
...
impl InputState {
    ...
    pub fn set_soft_wrap(&mut self, wrap: bool, _: &mut Window, cx: &mut Context<Self>) {
        ...
        self.soft_wrap = wrap;
        if wrap {
            // The input's own current width, not `last_layout.wrap_width` —
            // that field is a snapshot from whenever wrapped lines were last
            // computed, which goes stale the moment the container resizes
            // while wrap is off (or was never computed at all yet). Turning
            // wrap on used to re-wrap against that old width instead of the
            // one the input is actually drawn at right now, so text wrapped
            // as if the pane were still whatever size it was the last time
            // wrapping ran — cut off or not reflowing until an unrelated
            // resize forced a fresh measurement.
            let wrap_width = self.input_bounds.size.width;
    ...
    pub(super) fn set_input_bounds(&mut self, new_bounds: Bounds<Pixels>, cx: &mut Context<Self>) {
        ...
        if self.last_layout.is_some() && wrap_width_changed {
            let wrap_width = if !self.soft_wrap {
                // None to disable wrapping (will use Pixels::MAX)
                None
            } else {
                // The just-measured `new_bounds` width, not the stale
                // `last_layout.wrap_width` this used to re-apply — see
                // `set_soft_wrap`'s comment for why that snapshot is wrong
                // the moment the container's actual width has changed, which
                // `wrap_width_changed` just confirmed it has.
