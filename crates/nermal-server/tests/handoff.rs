...
//!
//! The proof is a variable. `nermal_kept=…` lives in that shell's memory and
...
use nermal_core::daemon::protocol::{DaemonMsg, ShellSpec, WinSize};
...
impl Daemon {
    fn start() -> Daemon {
        ...
        let child = Command::new(Self::binary())
            ...
            .env("nermal_CONTROL_SOCK", dir.path().join("control.sock"))
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
            .expect("start nermal-server --daemon");
    ...
    fn binary() -> PathBuf {
        PathBuf::from(env!("CARGO_BIN_EXE_nermal-server"))
    }
...
fn collect_until(session: &mut nermal_core::client::PaneSession, marker: &[u8]) -> Vec<u8> {
...
fn a_handoff_keeps_the_process_the_pty_and_the_shell_that_is_on_it() {
    ...
    session
        .input(b"nermal_kept=survivor; echo nermal_before_$nermal_kept\r")
        .expect("the shell takes input");
    collect_until(&mut session, b"nermal_before_survivor");
    drop(session);
    ...
    let replayed = collect_until(&mut session, b"nermal_before_survivor");
    assert!(
        windows_contain(&replayed, b"nermal_before_survivor"),
    ...
    // is unambiguous.
    session
        .input(b"echo nermal_after_$nermal_kept\r")
        .expect("the shell still takes input");
    collect_until(&mut session, b"nermal_after_survivor");
...
fn a_handoff_to_something_that_will_not_exec_leaves_the_daemon_serving() {
    ...
    session
        ...
        .expect("bound the stream reads");
    session
        .input(b"echo nermal_still_here\r")
        .expect("the shell takes input");
    collect_until(&mut session, b"nermal_still_here");

    let err = panes
        .hand_off(Path::new("/nonexistent/nermal-that-is-not-there"))
    ...
    session
        .input(b"echo nermal_unharmed\r")
        .expect("the pane still has its shell");
    collect_until(&mut session, b"nermal_unharmed");
    assert_eq!(
        session.pane_id(),
        pane_id,
