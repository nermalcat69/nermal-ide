...
/// The one config dir every test here shares, pinned into `nermal_core` on
...
fn pinned_dir() -> &'static Path {
    ...
    DIR.get_or_init(|| {
        sweep_dead_runs();
        let dir = std::env::temp_dir().join(format!("nermal-reap-{}", std::process::id()));
...
/// Removes `nermal-reap-<pid>` directories whose creating test run is gone —
...
fn sweep_dead_runs() {
    ...
    for entry in entries.flatten() {
        ...
        let Some(pid) = name
            .to_str()
            .and_then(|n| n.strip_prefix("nermal-reap-"))
...
fn spawn_daemon_from(exe: &Path, dir: &Path) -> Child {
    Command::new(exe)
        ...
        .arg("--config-dir")
        .arg(dir)
        .env("nermal_DATA_DIR", dir)
        .env("nermal_CONTROL_SOCK", dir.join("control.sock"))
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
...
fn await_ready(dir: &Path) {
    ...
    while PaneClient::at(&endpoint).version().is_err() || !dir.join("daemon.pid").exists() {
        assert!(
            ...
            "nermal-server did not open its endpoint within {READY_WITHIN:?}"
        );
...
fn a_seat_holder_with_no_pidfile_is_still_found_and_reaped() {
    ...
    let child = spawn_daemon_from(Path::new(env!("CARGO_BIN_EXE_nermal-server")), dir);
    let pid = child.id();
    await_ready(dir);
    assert_eq!(
    ...
    let second = spawn_daemon_from(Path::new(env!("CARGO_BIN_EXE_nermal-server")), dir);
...
/// `reap_stranded` is the road the GUI's `ensure_running` and `nermal server
...
fn reap_stranded_clears_a_seat_holder_with_no_pidfile() {
    ...
    nermal_core::daemon::spawn::reap_stranded();
...
fn a_daemon_running_from_a_deleted_executable_is_still_ours_to_reap() {
    ...
    let copied = bin_dir.join("nermal-server");
    std::fs::copy(env!("CARGO_BIN_EXE_nermal-server"), &copied).unwrap();
