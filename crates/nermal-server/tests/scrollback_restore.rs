...
use nermal_core::core::machine::PaneSeed;
use nermal_core::daemon::control::{ControlHello, ControlRequest, ReplyOk};
use nermal_core::daemon::protocol::{ClientMsg, DaemonMsg, RestoreFrom, ShellSpec, WinSize};
use nermal_core::daemon::transport;
...
const MARKER: &[u8] = b"nermal_screen_kept";
  ...
  "panes": [
    ...
    fn start(&self) -> Running {
        let child = Command::new(env!("CARGO_BIN_EXE_nermal-server"))
            ...
            .arg("--config-dir")
            .arg(self.path())
            .env("nermal_DATA_DIR", self.path())
            .env("nermal_CONTROL_SOCK", self.path().join("control.sock"))
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
