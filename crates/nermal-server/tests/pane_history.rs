...
impl Daemon {
    ...
    fn with_home(bashrc: &str, history: &str) -> Daemon {
        ...
        let child = Command::new(env!("CARGO_BIN_EXE_nermal-server"))
            .arg("--daemon")
            .arg("--config-dir")
            .arg(dir.path())
            .env("HOME", dir.path())
...
fn a_pane_writes_its_own_history_and_hands_it_back_when_it_closes() {
    ...
    session
        .input(b"echo nermal_ran_here\r")
        .expect("the shell takes input");
    collect_until(&mut session, b"nermal_ran_here");
    ...
    assert!(
        !daemon.global_history().contains("nermal_ran_here"),
...
/// The snippet seeds with `tail … > "$nermal_HISTFILE"`, and a redirection
...
fn a_small_histfilesize_cannot_swallow_the_merge_back() {
    ...
    session
        .input(b"echo nermal_survives_truncation\r")
        .expect("the shell takes input");
    collect_until(&mut session, b"nermal_survives_truncation");
...
fn two_panes_do_not_see_each_others_commands() {
    ...
    first
        .input(b"echo nermal_first_pane\r")
        .expect("the first shell takes input");
    collect_until(&mut first, b"nermal_first_pane");
    second
        .input(b"echo nermal_second_pane\r")
        .expect("the second shell takes input");
    collect_until(&mut second, b"nermal_second_pane");
    ...
    wait_until(
        || {
            daemon
                ...
                .is_some_and(|h| h.contains("nermal_first_pane"))
        },
    ...
    wait_until(
        || {
            daemon
                ...
                .is_some_and(|h| h.contains("nermal_second_pane"))
        },
