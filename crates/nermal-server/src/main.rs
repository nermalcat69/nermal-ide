...
const USAGE: &str = "\
nermal-server — the nermal session daemon, headless

USAGE:
    nermal-server --daemon [--config-dir <dir>]
    nermal-server --stdio [--serve | --bridge] [--control-sock <path>]
    nermal-server --stdio --pane [--config-dir <dir>]
    nermal-server agent-hook <agent> <event>
...
fn main() -> ExitCode {
    ...
    if args.first().map(String::as_str) == Some("agent-hook") {
        if let (Some(agent), Some(event)) = (args.get(1), args.get(2)) {
            nermal_core::core::agent_hooks::run_agent_hook(agent, event);
        }
        return ExitCode::SUCCESS;
    }

    if args.iter().any(|a| a == "--version" || a == "-V") {
        println!("nermal-server {}", env!("CARGO_PKG_VERSION"));
        return ExitCode::SUCCESS;
    }
    if args
        .iter()
        .any(|a| a == nermal_core::daemon::install::PROTOCOL_FLAG)
    {
        println!(
            "{}",
            nermal_core::daemon::install::RemoteProtocol::of_this_build().to_line()
        );
        return ExitCode::SUCCESS;
    }
    ...
    nermal_core::core::crash::install("server");
    nermal_core::core::logfile::install("server");

    if args.iter().any(|a| a == "--stdio") {
        return match run_stdio(&args) {
            Ok(()) => ExitCode::SUCCESS,
            Err(e) => {
                eprintln!("nermal-server: stdio session ended with error: {e}");
                ExitCode::FAILURE
            }
        };
    }
    ...
    eprint!("nermal-server: nothing to do without --daemon or --stdio\n\n{USAGE}");
    ExitCode::FAILURE
}

fn run_daemon() -> ExitCode {
    if let Err(e) = nermal_core::daemon::server::run_daemon() {
        eprintln!("nermal-server: daemon exited with error: {e}");
        ...
        match upstream {
            ...
            None => {
                ...
                server::serve_with(
                    link,
                    LocalHost::shared(),
                    nermal_core::daemon::server::control_services(),
                )
            }
        }
    }
...
fn bridge_panes() -> io::Result<()> {
    use nermal_core::daemon::{spawn, transport};
...
fn bridge(upstream: std::os::unix::net::UnixStream) -> io::Result<()> {
    ...
    let feeder = std::thread::Builder::new()
        .name("nermal-stdio-bridge-in".into())
        .spawn(move || {
