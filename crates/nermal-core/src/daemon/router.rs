...
async fn drive(local: Stream, header: &RouteHeader) -> io::Result<()> {
    ...
    // and last until nermal itself restarts.
...
async fn restart_server(
    ...
    match (&header.target, action) {
        ...
        _ => Err(anyhow::anyhow!(
            "restarting nermal's server is only supported for machines it serves, not {}",
            header.describe()
        )),
    }
}
...
mod tests {
    ...
    async fn a_wsl_route_refuses_a_distro_name_that_could_be_an_option() {
        for header in [
            ...
            RouteHeader {
                ...
                server_command: Some("nermal-server --stdio".to_string()),
                channel: RouteChannel::Control,
                action: RouteAction::Forward,
            },
        ] {
    ...
    fn an_unopenable_link_fails_the_route() {
        ...
        let (client, daemon_side) = UnixStream::pair().unwrap();
        let header = RouteHeader::local_stdio("nermal-no-such-binary-anywhere", &[]);
        let routed = std::thread::spawn(move || RemoteRouter::route(daemon_side, &header));
    ...
    fn a_request() -> InstallRequest {
        InstallRequest {
            ...
            source_url: "https://example.invalid/nermal-server".into(),
            remote_path: "/home/me/.local/share/nermal/bin/nermal-server-26.7.5".into(),
