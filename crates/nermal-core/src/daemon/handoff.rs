...
fn anonymous_file() -> std::io::Result<std::fs::File> {
    ...
    for attempt in 0..8 {
        let path = dir.join(format!("nermal-handoff-{}-{attempt}", std::process::id()));
...
mod tests {
    ...
    fn carried(id: u64, fd: RawFd, output: &[u8]) -> Carried {
        Carried {
            ...
            master_fd: fd,
            child_pid: 4242,
            integration_dir: Some(PathBuf::from("/tmp/nermal-int")),
            size: size(),
    ...
    fn a_failed_exec_puts_close_on_exec_back_on_every_descriptor() {
        ...
        let err = take_over(
            Path::new("/nonexistent/nermal-that-cannot-exec"),
