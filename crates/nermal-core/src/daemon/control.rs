...
/// server's filename (`nermal-server-c{control}p{protocol}`), so moving it also
...
pub enum ControlRequest {
    ...
    GuiOpen {
        ...
        /// what `nermal [PATH]` wants, and what a workspace the CLI just made
...
impl ControlClient {
        ...
        W: Write + Send + 'static,
    {
        ...
        let reader = std::thread::Builder::new()
            .name("nermal-control-reader".into())
    ...
    pub fn call_with_deadline(
        ...
        log::debug!(target: "nermal::control", "#{req_id} {req:?}");
...
mod tests {
    ...
    fn other_connect_failures_are_not_dialect_refusals() {
        for other in [
            ...
            "the remote nermal-server did not start: exit status 127",
