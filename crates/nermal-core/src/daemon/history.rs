...
//! like, long after the pane's environment was decided. nermal's integration
...
//!
//! A shell with no nermal integration is simply not covered: nothing sets the
...
pub const FILE_ENV: &str = "nermal_HISTFILE";

/// Shells whose integration snippet knows what to do with [`FILE_ENV`].
///
/// The list is exactly the shells that get a `HISTFILE` *and* a nermal snippet to
...
mod tests {
    ...
    fn the_sweep_keeps_only_the_panes_that_still_exist() {
        ...
        let dir = std::env::temp_dir().join(format!("nermal-sweeptest-{}", std::process::id()));
