...
const DAEMON_EXE_STEMS: [&str; 3] = ["nermal-app", "nermal-server", "nermal"];
...
pub fn ensure_running() -> anyhow::Result<()> {
    ...
    // post-install "Launch nermal" click — where the launch deserves its
    // daemon, not an error.
    #[cfg(windows)]
    {
        ...
        while crate::daemon::update_guard::held() {
            if Instant::now() >= deadline {
                anyhow::bail!(
                    "a nermal update is being installed right now; the daemon will return \
...
/// level: that is where the locked images (nermal-app.exe, OpenConsole.exe,
...
fn spawn_detached() -> anyhow::Result<()> {
    ...
    if windows::redirection_trust_enforced() {
        ...
        match windows::spawn_detached_with_clean_parent(&exe, &args) {
            ...
            // instead of refusing to start nermal at all.
            Err(error) => log::warn!(
...
///
/// Public because `nermal-cli` launches the headless `nermal-server` the same way,
...
mod windows_spawn_tests {
    ...
    const INNER_ENV: &str = "nermal_REDIRECTION_TRUST_INNER";
    const CLEAN_PARENT_ENV: &str = "nermal_REDIRECTION_TRUST_CLEAN_PARENT";
    const PROBE_RESULT_ENV: &str = "nermal_REDIRECTION_TRUST_PROBE_RESULT";
    const TEST_NAME: &str =
    ...
    fn daemon_spawn_does_not_inherit_redirection_trust() {
        ...
        let root = std::env::temp_dir().join(format!(
            "nermal-redirection-trust-{}-{unique}",
            std::process::id()
        ));
        ...
        assert!(
            ...
            "nermal must detect the enforcing policy before selecting the alternate spawn path"
        );
        ...
        assert_eq!(
            verdict.trim(),
            "CLEAN_OK",
            "a nermal daemon child must drop the inherited policy and retain access to user-created junctions"
        );
    }
}
...
mod exe_name_tests {
    ...
    fn windows_matches_daemon_names_case_insensitively() {
        assert!(is_reapable_daemon_name("nermal-APP.EXE"));
        assert!(is_reapable_daemon_name("nermal-Server"));
        assert!(is_reapable_daemon_name("nermal"));
    }
    ...
    fn the_deleted_marker_is_not_part_of_a_process_name() {
        assert_eq!(
            strip_deleted_marker("/opt/nermal/nermal-server (deleted)".into()),
            "/opt/nermal/nermal-server"
        );
        assert_eq!(
            strip_deleted_marker("/opt/nermal/nermal-server".into()),
            "/opt/nermal/nermal-server"
        );
        // Only the trailing marker is the kernel's.
        assert_eq!(
            strip_deleted_marker("/tmp/x (deleted)/nermal-server".into()),
            "/tmp/x (deleted)/nermal-server"
        );
    }
    ...
    fn strip_exe_suffix_only_strips_a_trailing_exe() {
        assert_eq!(strip_exe_suffix("nermal-app.exe"), "nermal-app");
        assert_eq!(strip_exe_suffix("nermal-app.EXE"), "nermal-app");
        assert_eq!(strip_exe_suffix("nermal-app"), "nermal-app");
...
mod tests {
    ...
    fn connect_to_stale_socket_path_fails() {
        let dir = std::env::temp_dir().join(format!("nermal-spawn-test-{}", std::process::id()));
