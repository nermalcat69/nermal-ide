...
struct ChosenShell {
    program: String,
    args: Vec<String>,
    args_are_nermal_defaults: bool,
}

fn has_custom_args(chosen: Option<&ChosenShell>) -> bool {
    chosen.is_some_and(|c| !c.args.is_empty() && !c.args_are_nermal_defaults)
}
...
fn choose_shell(
    ...
    spawn_override
        .map(|s| ChosenShell {
            program: s.program,
            args: s.args,
            args_are_nermal_defaults: s.args_are_nermal_defaults,
        })
        .or_else(|| {
...
fn build_spawn_config(
    ...
    let shell = configured.as_ref().map(|c| ShellSpec {
        ...
        args: c.args.clone(),
        args_are_nermal_defaults: c.args_are_nermal_defaults,
    });
...
///
/// `resolved_program` is what nermal *decided* to run before shell integration
...
fn initial_working_directory(cwd: Option<PathBuf>) -> Option<PathBuf> {
    ...
    // mistyped path reads as a nermal bug (#601). Settings refuses to save one
...
const TERM_PROGRAM_NAME: &str = "nermal";
...
const nermal_CONFIG_DIR_ENV: &str = "nermal_CONFIG_DIR";
const nermal_PANE_ENV: &str = "nermal_PANE";
const nermal_WS_ENV: &str = "nermal_WS";
...
/// shell nermal was told to run *is* the user's shell, so that is what we name.
///
...
fn pane_environment(
    ...
    if let Some(ws) = workspace {
        env.push((nermal_WS_ENV.to_string(), ws.to_string()));
    }
    if let Some(dir) = config_dir_env() {
        env.push((nermal_CONFIG_DIR_ENV.to_string(), dir));
    }
...
struct PaneState {
    ...
    /// is every pane on a remote `nermal-server` — there the controlling client
    ...
    /// shell's — and that is the proof that the far side runs nermal's shell
...
fn drain_then_report(
    ...
    std::thread::Builder::new()
        .name("nermal-daemon-pane-pty-close".to_string())
...
impl DaemonPane {
    ...
    fn spawn_reader(
        ...
        std::thread::Builder::new()
            .name("nermal-daemon-pane-reader".to_string())
            .spawn(move || {
                ...
                let trace = std::env::var("nermal_TRACE").is_ok_and(|v| !v.is_empty() && v != "0");
    ...
    fn spawn_exit_monitor(
        ...
        if handle.is_null() {
            ...
            std::thread::Builder::new()
                .name("nermal-daemon-pane-exit-fallback".to_string())
                .spawn(move || {
        ...
        std::thread::Builder::new()
            .name("nermal-daemon-pane-exit-monitor".to_string())
            .spawn(move || {
...
fn join_bounded(handle: JoinHandle<()>, timeout: Duration) -> bool {
    ...
    if std::thread::Builder::new()
        .name("nermal-daemon-pane-join".to_string())
        .spawn(move || {
            let _ = handle.join();
            let _ = tx.send(());
        })
...
mod tests {
    ...
    fn initial_working_directory_skips_paths_that_are_not_directories() {
        ...
        let file = real.join("nermal-iwd-probe");
    ...
    fn only_user_authored_args_block_shell_integration() {
        let chosen = |args: Vec<&str>, nermal: bool| ChosenShell {
            program: r"C:\Program Files\Git\bin\bash.exe".to_string(),
            args: args.into_iter().map(str::to_string).collect(),
            args_are_nermal_defaults: nermal,
        };
...
\x1b[?2004hframe one\x1b[Hframe two"
    ...
    fn at_prompt_covers_prompt_draw_gap_across_chunks() {
        ...
        let chunk = s.feed(
            b"\x1b]133;A\x07\x1b]7;file://host/repo/nermal\x07\r\nnermal git:(main) \xe2\x9e\x9c ",
        );
        assert!(
    ...
    fn sentinel_events_drive_agent_session_state() {
        ...
        let mut sniffer = OscSniffer::new();
        let stream = concat!(
            "\x1b]777;notify;nermal://cli-agent;",
    ...
    fn probed_cwd_keeps_the_shells_spelling_for_a_symlinked_path() {
        let tmp = std::env::temp_dir().join(format!("nermal-cwd-{}", std::process::id()));
    ...
    fn probed_cwd_matching_the_reported_one_says_nothing() {
        let mut st = test_state(true);
        st.cwd = Some(PathBuf::from("/Users/alice/dev/nermal"));
        let (tx, rx) = mpsc::channel();
        st.subscriber = Some(tx);
    ...
    fn attach_replays_initial_cwd_even_before_shell_reports_osc7() {
        let mut st = test_state(true);
        st.cwd = Some(PathBuf::from("/Users/alice/clone/nermal"));
        ...
        assert!(
            matches!(rx.try_recv(), Ok(DaemonMsg::Cwd(p)) if p == PathBuf::from("/Users/alice/clone/nermal"))
        );
    }
    ...
    fn a_spec_granted_clipboard_permission_survives_a_re_attach() {
        ...
        // `nermal-server` — is exactly as permitted as its controller says.
        let mut st = test_state(true);
        let (tx, _rx) = mpsc::channel();
        attach_subscriber_with_permissions(&mut st, tx, true, false);
        assert!(st.allow_remote_clipboard_write);
    ...
    fn reader_poll_applies_the_probed_cwd() {
        ...
        let handle = DaemonPane::spawn_reader(
            ...
            ForegroundProbes {
                remote: Box::new(|| None),
                agent: Box::new(|| None),
                cwd: Box::new(|| Some(PathBuf::from("/Users/alice/dev/nermal"))),
            },
            Arc::new(DeathReporter::new(|| {})),
        );
        handle.join().unwrap();
        ...
        assert!(
            matches!(sub_rx.try_recv(), Ok(DaemonMsg::Cwd(p)) if p == PathBuf::from("/Users/alice/dev/nermal"))
        );
    }
    ...
    fn pane_environment_advertises_the_terminal_under_the_standard_names() {
        ...
        assert_eq!(env.get("TERM_PROGRAM").map(String::as_str), Some("nermal"));
        assert_eq!(
            env.get("TERM_PROGRAM_VERSION").map(String::as_str),
            Some(version)
        );
        assert_eq!(
            env.get(crate::core::agent_hooks::nermal_ENV_MARKER)
                .map(String::as_str),
            Some(version)
        );
        assert_eq!(
    ...
    fn pane_environment_hands_the_shell_its_own_address() {
        ...
        match config_dir_env() {
            ...
            None => assert!(
                !env.contains_key(nermal_CONFIG_DIR_ENV),
                "no resolvable config dir must not inject an empty nermal_CONFIG_DIR"
            ),
        }
        ...
        .collect();
        assert!(
            !unfiled.contains_key(nermal_WS_ENV),
    ...
    #[cfg(unix)]
    #[test]
    fn a_configured_shell_variable_outranks_the_shell_nermal_launched() {
        // Same precedence contract as TERM_PROGRAM: nermal describes the pane,
    ...
    fn the_shell_we_announce_is_the_argv_we_are_about_to_exec() {
        ...
        assert_eq!(
            ...
            "/opt/homebrew/bin/fish",
            "a builder with an argv has already overruled whatever nermal resolved"
        );
    ...
    #[test]
    fn spawned_shell_carries_the_nermal_marker() {
        let cmd = build_shell_command(None, &Some(PathBuf::from("/tmp")), 42, Some("ws-main"))
            .expect("build default shell command")
            .0;
        let nermal = cmd
            .get_env(crate::core::agent_hooks::nermal_ENV_MARKER)
            .and_then(|v| v.to_str());
        assert_eq!(
            nermal,
            Some(env!("CARGO_PKG_VERSION")),
            "the daemon must inject nermal into every spawned shell"
        );
        assert_eq!(
            cmd.get_env(nermal_PANE_ENV).and_then(|v| v.to_str()),
            Some("42"),
            "the daemon must tell every spawned shell which pane it is"
        );
        assert_eq!(
            cmd.get_env(nermal_WS_ENV).and_then(|v| v.to_str()),
            Some("ws-main"),
            "the daemon must tell every spawned shell which workspace filed it"
        );
        if let Some(dir) = config_dir_env() {
            assert_eq!(
                cmd.get_env(nermal_CONFIG_DIR_ENV).and_then(|v| v.to_str()),
                Some(dir.as_str()),
    ...
    fn the_two_endpoints_are_siblings_derived_from_one_config_dir() {
        ...
        // and sit in ~/.local/share/nermal as `daemon.sock`, the same basename the
        ...
        let dir = std::path::Path::new("/tmp/nermal-endpoint-test");
