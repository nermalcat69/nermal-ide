...
# --- nermal shell integration (zsh) ---
if [[ -o interactive ]] && [[ -z "$nermal_SHELL_INTEGRATION" ]]; then
  export nermal_SHELL_INTEGRATION=1
  ...
  # first line is read.
  if [[ -n "$nermal_HISTFILE" ]]; then
    if [[ ! -e "$nermal_HISTFILE" ]]; then
      ...
      if [[ -n "$HISTFILE" ]]; then
        builtin printf '%s\n' "$HISTFILE" > "$nermal_HISTFILE.origin" 2>/dev/null
        if [[ -r "$HISTFILE" ]]; then
          command tail -n 5000 "$HISTFILE" > "$nermal_HISTFILE" 2>/dev/null
        fi
      fi
      [[ -e "$nermal_HISTFILE" ]] || : > "$nermal_HISTFILE" 2>/dev/null
      command wc -c < "$nermal_HISTFILE" 2>/dev/null | command tr -d ' \n' \
        > "$nermal_HISTFILE.seed" 2>/dev/null
    fi
    HISTFILE="$nermal_HISTFILE"
  ...
  __nermal_osc() { builtin printf '\e]%s\a' "$1"; }
  ...
  # the Esc widget name.
  __nermal_report_edit_mode() {
    if [[ "$(builtin bindkey -lL main)" == *viins* ]]; then
      __nermal_osc "133;V;1"
    else
      __nermal_osc "133;V;0"
    fi
  }
  ...
  __nermal_report_cwd() { builtin printf '\e]7;file://%s%s\a' "${HOST:-localhost}" "${PWD//\%/%25}"; }

  # D (command finished + its exit code) gets its own hook, *prepended* to
  # precmd_functions rather than bundled into __nermal_precmd below: the app only
  ...
  __nermal_precmd_d() {
    local ret=$?
    if [[ -n "$__nermal_cmd_active" ]]; then
      __nermal_osc "133;D;$ret"
      unset __nermal_cmd_active
    fi
  }
  ...
  __nermal_preexec() {
    ...
    cmd=${cmd//$'\r'/%0D}
    cmd=${cmd//$'\n'/%0A}
    __nermal_osc "133;C;$cmd"
  }

  autoload -Uz add-zsh-hook
  add-zsh-hook precmd __nermal_precmd
  add-zsh-hook preexec __nermal_preexec
  ...
  precmd_functions=(__nermal_precmd_d $precmd_functions)
  ...
  __nermal_restore_zdotdir() {
    ZDOTDIR=${nermal_USER_ZDOTDIR:-$HOME}
    add-zsh-hook -d precmd __nermal_restore_zdotdir
    unfunction __nermal_restore_zdotdir
  }
  add-zsh-hook precmd __nermal_restore_zdotdir
fi
# --- end nermal shell integration ---
"#;

const FISH_INTEGRATION: &str = r#"
# --- nermal shell integration (fish) ---
...
if status is-interactive; and test -z "$nermal_SHELL_INTEGRATION"
  set -gx nermal_SHELL_INTEGRATION 1

  function __nermal_osc
    printf '\e]%s\a' $argv[1]
  end

  function __nermal_report_edit_mode
  ...
  function __nermal_preexec --on-event fish_preexec
    set -g __nermal_cmd_active 1
  ...
  function __nermal_precmd --on-event fish_prompt
    set -l ret $status
    if set -q __nermal_cmd_active
      __nermal_osc "133;D;$ret"
      set -e __nermal_cmd_active
    end
    __nermal_report_cwd
    __nermal_report_edit_mode
    __nermal_osc "133;A"
  ...
  functions -c fish_prompt __nermal_original_fish_prompt
  function fish_prompt
    __nermal_original_fish_prompt
    __nermal_osc "133;B"
  end
end
# --- end nermal shell integration ---
...
# --- nermal shell integration (bash) ---
if [[ $- == *i* ]] && [[ -z "$nermal_SHELL_INTEGRATION" ]]; then
  export nermal_SHELL_INTEGRATION=1
  ...
  if [[ "$OSTYPE" == msys* || "$OSTYPE" == cygwin* ]]; then
    ...
    __nermal_report_cwd() { builtin printf '\e]7;file://%s%s\a' "${HOSTNAME:-localhost}" "${PWD//\%/%25}"; }
  fi
  ...
  precmd_functions=(__nermal_precmd_d "${precmd_functions[@]}")
  precmd_functions+=(__nermal_precmd)
  preexec_functions+=(__nermal_preexec)
fi
# --- end nermal shell integration ---
"#;

const POWERSHELL_INTEGRATION: &str = r#"
# --- nermal shell integration (PowerShell) ---
if (-not $env:nermal_SHELL_INTEGRATION) {
  $env:nermal_SHELL_INTEGRATION = '1'

  $global:__nermalEsc = [char]0x1b
  $global:__nermalBel = [char]0x07
  # Whatever prompt the user's profile settled on; we call through to it.
  $global:__nermalOrigPrompt = $function:prompt
  # Gates the D marker so the first prompt (no command yet) emits no bogus exit.
  $global:__nermalCmdActive = $false
  ...
  $global:__nermalUser = [Environment]::UserName
  $global:__nermalHost = [Environment]::MachineName
  $global:__nermalHome = if ($HOME) { $HOME.Replace('\', '/').TrimEnd('/') } else { '' }
  ...
  function global:prompt {
    ...
    if ($global:__nermalCmdActive) {
      ...
      Write-Host -NoNewline "$($global:__nermalEsc)]133;D;$code$($global:__nermalBel)"
    }
    ...
    if ($PWD.Provider.Name -eq 'FileSystem') {
      ...
      Write-Host -NoNewline "$($global:__nermalEsc)]7;file://$($global:__nermalHost)$p$($global:__nermalBel)"
      ...
      # so without this every nermal tab running PowerShell stays generic, on every
      # platform. Forward slashes (so nermal's tab-label parser can take the last
      ...
      if ($global:__nermalHome -and ($titlePath -eq $global:__nermalHome -or
          $titlePath.StartsWith($global:__nermalHome + '/'))) {
        $titlePath = '~' + $titlePath.Substring($global:__nermalHome.Length)
      }
      Write-Host -NoNewline "$($global:__nermalEsc)]0;$($global:__nermalUser)@$($global:__nermalHost):$titlePath$($global:__nermalBel)"
    }
    ...
    $base = & $global:__nermalOrigPrompt
    if ($base -is [array]) { $base = $base -join [char]0x0a }
    $global:LASTEXITCODE = $lastExit

    "$($global:__nermalEsc)]133;A$($global:__nermalBel)$base$($global:__nermalEsc)]133;B$($global:__nermalBel)"
  }
  ...
  if ((Test-Path Function:\PSConsoleHostReadLine) -and -not $global:__nermalReadLineWrapped) {
    $global:__nermalReadLineWrapped = $true
    $global:__nermalOrigReadLine = $function:global:PSConsoleHostReadLine
    function global:PSConsoleHostReadLine {
      $line = & $global:__nermalOrigReadLine
      if (-not [string]::IsNullOrWhiteSpace($line)) {
        $global:__nermalCmdActive = $true
        # Carry the submitted line on the C mark (nermal extension): the daemon
        ...
        try {
          ...
          Write-Host -NoNewline "$($global:__nermalEsc)]133;C;$cmd$($global:__nermalBel)"
        } catch {
          Write-Host -NoNewline "$($global:__nermalEsc)]133;C$($global:__nermalBel)"
        }
      }
      $line
    }
...
# --- nermal shell integration (nushell) ---
...
if (($env.nermal_SHELL_INTEGRATION? | default "") == "") {
  $env.nermal_SHELL_INTEGRATION = "1"

  # nermal restores the user's own config.nu at this placeholder — `source` is
  ...
  $env.config.hooks.pre_prompt = ($env.config.hooks.pre_prompt | default [] | append {||
    ...
    let __nermal_osc = {|__nermal_payload| print -n $"(ansi osc)($__nermal_payload)(char bel)" }
    if ($env.__nermal_cmd_active? | default false) {
      hide-env __nermal_cmd_active
      do $__nermal_osc $"133;D;($env.LAST_EXIT_CODE? | default 0)"
    }
    ...
    let __nermal_path = if (($nu.os-info.name? | default '') == 'windows') {
      ($env.PWD | str replace -a '\' '/' | str replace -a '%' '%25')
    } else {
      ($env.PWD | str replace -a '%' '%25')
    }
    let __nermal_path = if ($__nermal_path | str starts-with '/') { $__nermal_path } else { '/' + $__nermal_path }
    do $__nermal_osc $"7;file://($env.COMPUTERNAME? | default 'localhost')($__nermal_path)"
    do $__nermal_osc "133;A"
  })
  ...
  if ((($env.config.prompt_indicator? | default null) | describe) != 'nothing') {
    let __nermal_orig_indicator = $env.config.prompt_indicator
    $env.config.prompt_indicator = {||
      let __nermal_ind = match ($__nermal_orig_indicator | describe) {
        'closure' => (do $__nermal_orig_indicator)
        _ => $__nermal_orig_indicator
      }
      $"($__nermal_ind)(ansi osc)133;B(char bel)"
    }
  }
}
# --- end nermal shell integration ---
...
fn zsh_redirectors() -> [(&'static str, String); 4] {
    let redirect = |name: &str, tail: &str| {
        format!(
            "__nermal_ztmp=$ZDOTDIR\n\
             if [[ -n \"$nermal_USER_ZDOTDIR\" ]]; then ZDOTDIR=$nermal_USER_ZDOTDIR; else unset ZDOTDIR; fi\n\
             [[ -f \"${{ZDOTDIR:-$HOME}}/{name}\" ]] && source \"${{ZDOTDIR:-$HOME}}/{name}\"\n\
             {tail}ZDOTDIR=$__nermal_ztmp\n\
             unset __nermal_ztmp\n"
        )
    };
    [
        (
            ".zshenv",
            redirect(".zshenv", "export nermal_USER_ZDOTDIR=${ZDOTDIR:-$HOME}\n"),
        ),
...
const ZDOTDIR_PREFIX: &str = "nermal-zdotdir-";
...
fn real_user_zdotdir() -> Option<String> {
    if let Ok(z) = std::env::var("nermal_USER_ZDOTDIR") {
        if !z.is_empty() {
            return Some(z);
        }
    }
...
fn setup_zsh() -> Option<Injection> {
    ...
    if let Some(user_zdotdir) = real_user_zdotdir() {
        env.insert("nermal_USER_ZDOTDIR".to_string(), user_zdotdir);
    }
...
fn setup_nushell_with(user_config: Option<&Path>) -> Option<Injection> {
    let dir = throwaway_dir("nermal-nu-")?;
...
fn nushell_config_script_with(user_config: Option<&Path>) -> String {
    ...
    NUSHELL_INTEGRATION.replace("__nermal_SOURCE_USER_CONFIG__", &source)
}
...
/// nermal keeps its own config (`~/.config/nermal` on macOS too); nu follows the
...
fn platform_config_dir() -> Option<PathBuf> {
    ...
    // same directory in every environment nermal can spawn a pane in.
...
# necessary because nermal spawns bash non-login (see shell_integration.rs) so
...
__nermal_bashrc=0
source() {{ case "${{1-}}" in */.bashrc|.bashrc) __nermal_bashrc=1;; esac; builtin source "$@"; }}
.() {{ case "${{1-}}" in */.bashrc|.bashrc) __nermal_bashrc=1;; esac; builtin . "$@"; }}
...
unset -f source .
if [[ $__nermal_bashrc == 0 && -f ~/.bashrc ]]; then source ~/.bashrc; fi
unset __nermal_bashrc
{BASH_INTEGRATION}"#
    )
}
...
fn setup_bash() -> Option<Injection> {
    let dir = throwaway_dir("nermal-bashrc-")?;
...
const WSL_RCFILE_ENV: &str = "nermal_RC";
...
const WSL_ZDOTDIR_ENV: &str = "nermal_ZDOTDIR";
...
/// user's own `.bashrc` down with it, because nermal starts it non-login so that
...
fn wsl_exec_script() -> String {
    format!(
        concat!(
            r#"case "${{SHELL:-}}" in "#,
            r#"*/bash) [ -r "${{nermal_RC:-}}" ] && exec "$SHELL" --rcfile "$nermal_RC" -i; "#,
            r#"exec "$SHELL" -l ;; "#,
            r#"*/zsh) [ -n "${{nermal_ZDOTDIR:-}}" ] && [ -r "$nermal_ZDOTDIR/.zshrc" ] "#,
            r#"&& export ZDOTDIR="$nermal_ZDOTDIR"; exec "$SHELL" -l ;; "#,
...
///
/// `nermal_USER_ZDOTDIR` is conspicuously absent, and that is the point.
...
fn setup_wsl(args: &[String]) -> Option<Injection> {
    ...
    let dir = throwaway_dir("nermal-wslrc-")?;
...
pub fn setup(program: Option<&str>, args: &[String], has_custom_args: bool) -> Option<Injection> {
    ...
    // files run. Arguments nermal's own detection supplied (Git Bash's `-i -l`,
    ...
    injection
        .env
        .insert("nermal_SHELL_INTEGRATION".to_string(), String::new());
...
pub mod remote {
    ...
    pub const PROBE_COMMAND: &str = "echo __nermal_shell; echo $SHELL";

    const PROBE_MARKER: &str = "__nermal_shell";

    const HEREDOC: &str = "__nermal_RC_EOF__";
    ...
    fn write_file(out: &mut String, name: &str, body: &str) {
        out.push_str(&format!(
            "command cat > \"$__nermal_d/{name}\" <<'{HEREDOC}'\n{}\n{HEREDOC}\n",
    ...
    fn zsh_bootstrap(shell_path: &str) -> String {
        ...
        out.push_str("__nermal_d=${TMPDIR:-/tmp}/nermal-zdotdir-$$\n");
        out.push_str("command mkdir -p \"$__nermal_d\" 2>/dev/null\n");
        ...
        for (name, contents) in zsh_redirectors() {
            ...
            guard.push_str(&format!("[ -s \"$__nermal_d/{name}\" ] && "));
        }
        out.push_str(&format!(
            "{guard}export ZDOTDIR=\"$__nermal_d\" nermal_RM_DIR=\"$__nermal_d\"\n"
        ));
    ...
    fn bash_bootstrap(shell_path: &str) -> String {
        ...
        let mut out = String::new();
        out.push_str("__nermal_d=${TMPDIR:-/tmp}/nermal-bashrc-$$\n");
        out.push_str("command mkdir -p \"$__nermal_d\" 2>/dev/null\n");
        write_file(
            &mut out,
            "bashrc",
...
# --- nermal remote cleanup (zsh) ---
if [[ -n "$nermal_RM_DIR" ]]; then
  typeset -g __nermal_rm_dir=$nermal_RM_DIR
  unset nermal_RM_DIR
  __nermal_rm_rcdir() {
    [[ -n "$__nermal_rm_dir" ]] || return 0
    command rm -rf -- "$__nermal_rm_dir"
    unset __nermal_rm_dir
  }
  autoload -Uz add-zsh-hook
  add-zsh-hook precmd __nermal_rm_rcdir
fi
# --- end nermal remote cleanup ---
"#;

    const BASH_CLEANUP_HOOK: &str = r#"
# --- nermal remote cleanup (bash) ---
if [[ -n "$nermal_RM_DIR" ]]; then
  __nermal_rm_dir=$nermal_RM_DIR
  unset nermal_RM_DIR
  __nermal_rm_rcdir() {
    [[ -n "$__nermal_rm_dir" ]] || return 0
    command rm -rf -- "$__nermal_rm_dir"
  ...
  precmd_functions+=(__nermal_rm_rcdir)
fi
# --- end nermal remote cleanup ---
"#;
    ...
    mod tests {
        ...
        fn probe_declines_anything_that_isnt_a_shell_we_know() {
            assert_eq!(parse_probe("__nermal_shell\n$SHELL\n"), None);
            assert_eq!(parse_probe("__nermal_shell\n\n"), None);
            assert_eq!(parse_probe("__nermal_shell\n/bin/ksh\n"), None);
        ...
        fn zsh_bootstrap_gates_zdotdir_on_every_redirector_landing() {
            ...
            for name in [".zshenv", ".zprofile", ".zshrc", ".zlogin"] {
                assert!(
                    script.contains(&format!("[ -s \"$__nermal_d/{name}\" ] &&")),
            ...
            assert!(export < exec);
            assert!(script.contains("__nermal_report_cwd"));
        }
        ...
        fn bash_bootstrap_forces_a_non_login_shell_through_the_rcfile() {
            ...
            assert!(script.contains("exec '/bin/bash' --rcfile \"$__nermal_d/bashrc\" -i"));
...
mod tests {
    ...
    fn is_our_zdotdir_matches_only_our_prefix() {
        assert!(is_our_zdotdir("/tmp/nermal-zdotdir-1234-0"));
        assert!(is_our_zdotdir("nermal-zdotdir-x"));
        assert!(!is_our_zdotdir("/home/alice/.config/zsh"));
        assert!(!is_our_zdotdir("/tmp/other-zdotdir"));
        assert!(!is_our_zdotdir(""));
        assert!(!is_our_zdotdir("/tmp/not-nermal-zdotdir-1"));
    }
    ...
    fn wsl_setup_never_contacts_the_distro() {
        let args: Vec<String> = vec![
            "--distribution".into(),
            "nermal-no-such-distro-exists".into(),
            "--cd".into(),
            "~".into(),
        ];
        ...
        assert_eq!(
            ...
            &[
                ...
                "nermal-no-such-distro-exists".to_string(),
                "--cd".to_string(),
                "~".to_string()
            ]
        );
    ...
    fn the_wsl_bootstrap_carries_integration_for_fish_not_just_bash() {
        ...
        assert!(script.contains(
            r#"*/bash) [ -r "${nermal_RC:-}" ] && exec "$SHELL" --rcfile "$nermal_RC" -i; exec "$SHELL" -l ;;"#
        ));
    ...
    fn the_wsl_bootstrap_carries_integration_for_zsh_too() {
        ...
        assert!(
            !script.contains("nermal_USER_ZDOTDIR"),
    ...
    fn wsl_integration_env_writes_what_each_shell_reads_and_names_it_for_the_distro() {
        let dir = throwaway_dir("nermal-wsltest-").expect("temp dir");
        let env = wsl_integration_env(&dir, None).expect("integration files");

        let rcfile = PathBuf::from(env.get(WSL_RCFILE_ENV).expect("nermal_RC"));
        assert_eq!(std::fs::read_to_string(&rcfile).unwrap(), bash_rcfile());

        let zdotdir = PathBuf::from(env.get(WSL_ZDOTDIR_ENV).expect("nermal_ZDOTDIR"));
        for (name, contents) in zsh_redirectors() {
            assert_eq!(
        ...
        assert_eq!(env.get("WSLENV").unwrap(), "nermal_RC/p:nermal_ZDOTDIR/p");
        assert!(!env.contains_key("nermal_USER_ZDOTDIR"));
    ...
    fn the_wsl_bootstrap_picks_the_arm_that_matches_the_distro_s_shell() {
        ...
        let dir = throwaway_dir("nermal-wslarm-").expect("temp dir");
    ...
    fn wslenv_preserves_the_users_own_entries() {
        assert_eq!(
            wslenv_with(Some("MYVAR/p:OTHER"), &["nermal_RC/p"]),
            "MYVAR/p:OTHER:nermal_RC/p"
        );
    ...
    fn bash_rcfile_path_uses_forward_slashes_on_windows() {
        let rendered = bash_path(Path::new(
            r"C:\Users\a\AppData\Local\Temp\nermal-bashrc-1-0\bashrc",
        ));
        if cfg!(windows) {
            assert_eq!(
                rendered,
                "C:/Users/a/AppData/Local/Temp/nermal-bashrc-1-0/bashrc"
            );
        }
        assert_eq!(
            bash_path(Path::new("/tmp/nermal-bashrc-1-0/bashrc")),
            "/tmp/nermal-bashrc-1-0/bashrc"
        );
    }
    ...
    fn zsh_redirectors_source_user_files_and_append_integration() {
        ...
        for (name, body) in &files {
            assert!(
                body.contains("$nermal_USER_ZDOTDIR"),
        ...
        let zshrc = &files[2].1;
        assert!(zshrc.contains("__nermal_precmd"));
        assert!(zshrc.contains("133;A"));
        assert!(!files[0].1.contains("__nermal_precmd"));
    }
    ...
    fn zsh_redirectors_point_zdotdir_at_the_real_dir_only_while_sourcing() {
        for (name, body) in zsh_redirectors() {
            ...
            let restore = body
                .rfind("ZDOTDIR=$__nermal_ztmp")
                .expect("restores our dir");
            let unset = body
                .rfind("unset __nermal_ztmp")
    ...
    fn zshenv_recaptures_a_user_relocated_zdotdir() {
        ...
        let recapture = zshenv
            .find("export nermal_USER_ZDOTDIR=${ZDOTDIR:-$HOME}")
            .unwrap();
        let restore = zshenv.rfind("ZDOTDIR=$__nermal_ztmp").unwrap();
        assert!(
        ...
        for (name, body) in &files[1..] {
            assert!(
                !body.contains("export nermal_USER_ZDOTDIR"),
                "{name} must not re-export nermal_USER_ZDOTDIR"
            );
        }
    }
    ...
    fn zsh_integration_restores_real_zdotdir_after_startup() {
        assert!(ZSH_INTEGRATION.contains("__nermal_restore_zdotdir"));
        assert!(ZSH_INTEGRATION.contains("ZDOTDIR=${nermal_USER_ZDOTDIR:-$HOME}"));
        assert!(
            ZSH_INTEGRATION.contains("add-zsh-hook -d precmd __nermal_restore_zdotdir"),
    ...
    fn bashrc_sourcings(profile: Option<(&str, &str)>) -> Option<usize> {
        let home = throwaway_dir("nermal-rcfile-home-")?;
    ...
    fn a_profile_that_never_forwards_still_gets_bashrc_once() {
        for profile in [
            Some((".bash_profile", "export nermal_PROFILE=1\n")),
            Some((".profile", "export nermal_PROFILE=1\n")),
    ...
    fn every_integration_guards_install_on_empty_sentinel() {
        for (shell, body) in [
            ...
            assert!(
                body.contains(r#"-z "$nermal_SHELL_INTEGRATION""#),
        ...
        assert!(
            !FISH_INTEGRATION.contains("set -q nermal_SHELL_INTEGRATION"),
    ...
    fn d_emitter_is_prepended_ahead_of_user_precmd_hooks() {
        assert!(
            ZSH_INTEGRATION.contains("precmd_functions=(__nermal_precmd_d $precmd_functions)"),
            "zsh must prepend the D emitter (add-zsh-hook can only append)"
        );
        assert!(
            BASH_INTEGRATION
                .contains(r#"precmd_functions=(__nermal_precmd_d "${precmd_functions[@]}")"#),
    ...
    fn setup_fish_injects_startup_command_without_files() {
        ...
        assert!(inj.args[1].contains("__nermal"));
    ...
    fn setup_dispatches_by_shell_and_sets_sentinel() {
        ...
        assert_eq!(
            inj.env.get("nermal_SHELL_INTEGRATION").map(String::as_str),
            Some("")
        );
        ...
        assert!(inj.env.contains_key("nermal_SHELL_INTEGRATION"));
    ...
    fn powershell_integration_emits_every_osc_133_mark_and_cwd() {
        ...
        assert!(s.contains("if (-not $env:nermal_SHELL_INTEGRATION)"));
        ...
        assert!(s.contains("$global:__nermalOrigPrompt = $function:prompt"));
        assert!(s.contains("& $global:__nermalOrigPrompt"));
    ...
    fn powershell_integration_sets_an_osc_title() {
        let s = POWERSHELL_INTEGRATION;
        assert!(s.contains("]0;$($global:__nermalUser)@$($global:__nermalHost):"));
    ...
    fn powershell_integration_reads_identity_platform_neutrally() {
        ...
        assert!(s.contains("$global:__nermalHome = if ($HOME)"));
    }
    ...
    fn powershell_home_abbreviation_requires_a_path_separator() {
        let s = POWERSHELL_INTEGRATION;
        assert!(s.contains("$titlePath.StartsWith($global:__nermalHome + '/')"));
        assert!(s.contains("$titlePath -eq $global:__nermalHome"));
    }
    ...
    fn throwaway_dir_is_unique_per_call() {
        let a = throwaway_dir("nermal-test-").expect("dir a");
        let b = throwaway_dir("nermal-test-").expect("dir b");
        assert_ne!(a, b);
        assert!(
            a.file_name()
                .unwrap()
                .to_string_lossy()
                .starts_with("nermal-test-")
        );
    ...
    fn nushell_integration_emits_every_osc_133_mark_and_cwd() {
        ...
        assert!(s.contains("__nermal_cmd_active"));
        assert!(s.contains("hide-env __nermal_cmd_active"));
        assert!(s.contains("prompt_indicator?"));
        assert!(
            s.contains(r##"($env.nermal_SHELL_INTEGRATION? | default "") == """##),
    ...
    /// platform convention there, which is *not* where nermal keeps its own
    /// config (`~/.config/nermal`). Reading `~/.config/nushell` on a Mac finds
    ...
    fn nushell_config_dir_is_what_nu_itself_reports() {
        ...
        let xdg_absolute = std::env::temp_dir().join("nermal-nu-xdg");
        ...
        for xdg in cases {
            ...
            assert_eq!(
                ours, theirs,
                "nermal and nu disagree on the config dir for XDG_CONFIG_HOME={xdg:?}"
            );
        }
    }

    /// End to end over a real pty: a user who *has* a config.nu must still
    /// have it after nermal points `nu --config` at the wrapper. `--config`
    ...
    fn nushell_restores_the_user_config_and_still_reports_the_prompt_cycle() {
        ...
        std::fs::write(
            &user_config,
            "$env.config = { show_banner: false }\n$env.nermal_USER_CONFIG_LOADED = \"yes\"\n",
        )
        ...
        let text = prompt_cycle_over_pty(
            ...
            b"print $\"probe=($env.nermal_USER_CONFIG_LOADED? | default 'MISSING')\"; 1 / 0\r",
            Some(home.path()),
        );
        ...
        // Assert on nermal's *own* marks, not on the substring `133;A`: stock
        // Nushell emits its own OSC 133 cycle alongside these, so a bare
        // substring passes even when the wrapper never ran. nermal's hooks
        ...
        assert!(
            ...
            "nermal's own OSC 7 must be among the reports; got:\n{text}"
        );
        assert_eq!(
            last_osc7(&text),
