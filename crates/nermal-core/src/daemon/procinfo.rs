...
mod tests {
    ...
    fn a_quick_probe_comes_back_whole() {
        ...
        assert!(String::from_utf8_lossy(&out.stdout).contains("nermal"));
    }
    ...
    fn a_missing_probe_is_an_error_and_not_an_empty_answer() {
        let cmd = std::process::Command::new("nermal-no-such-probe-b0rk");
        assert!(
...
mod windows_tests {
    ...
    fn listening_ports_finds_the_pane_tree_s_socket_and_only_its_tree_s() {
        ...
        let stamp = format!("nermal-ports-{}", std::process::id());
