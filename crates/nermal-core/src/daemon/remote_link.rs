...
pub const DEFAULT_REMOTE_SERVER_CMD: &str = "nermal-server --stdio";
...
const ENV_MARKER: &str = "__nermal_env__";

pub const REMOTE_ENV_PROBE: &str = concat!(
    "sh -c 'printf \"__nermal_env__ %s\\n\" ",
    "\"sock=${nermal_CONTROL_SOCK-}\" \"cfg=${nermal_CONFIG_DIR-}\" ",
    "\"xdg=${XDG_RUNTIME_DIR-}\" ",
...
pub fn remote_control_socket(env: &RemoteEnv) -> Option<String> {
    ...
    // opens its control socket in the config dir — `$nermal_CONFIG_DIR` if the
    // remote sets one, otherwise `$HOME/.config/nermal`. This has to mirror
    ...
    let dir = match env.config_dir.as_deref().filter(|d| !d.is_empty()) {
        ...
        None => {
            ...
            posix_join(&posix_join(home, ".config"), "nermal")
        }
    };
...
mod tests {
    ...
    fn only_a_pane_link_asks_for_the_pane_socket() {
        let server = "/home/me/.local/share/nermal/bin/nermal-server-26.7.6";
        assert_eq!(
    ...
    fn the_entry_falls_back_exactly_when_streamlocal_cannot_be_used() {
        let cmd = "nermal-server --stdio";
        assert_eq!(
            choose_entry(Some("/run/user/1000/nermal/daemon.sock"), true, cmd),
            RemoteEntry::StreamLocal {
                socket: "/run/user/1000/nermal/daemon.sock".into()
            }
        );
        assert_eq!(
            choose_entry(Some("/run/user/1000/nermal/daemon.sock"), false, cmd),
    ...
    fn the_env_probe_survives_a_chatty_remote() {
        let out = "Welcome to Ubuntu!\n\
                   __nermal_env__ sock=\n\
                   __nermal_env__ xdg=/run/user/1000\n\
                   __nermal_env__ home=/home/me\n\
                   __nermal_env__ tmp=\n\
                   You have mail.\n";
    ...
    fn the_remote_socket_path_matches_the_servers_own_order() {
        ...
        assert_eq!(
            ...
            Some("/tmp/mine.sock"),
            "an explicit $nermal_CONTROL_SOCK outranks everything"
        );
        ...
        let explicit_cfg = RemoteEnv {
            config_dir: Some("/home/me/.config/nermal".into()),
            xdg_runtime_dir: Some("/run/user/1000".into()),
            home: Some("/home/me".into()),
            ..RemoteEnv::default()
        };
        assert_eq!(
            ...
            Some("/home/me/.config/nermal/control.sock"),
            "an explicit $nermal_CONFIG_DIR names the directory"
        );
        ...
        assert_eq!(
            ...
            Some("/home/me/.config/nermal/control.sock"),
            "with no $nermal_CONFIG_DIR the remote server uses $HOME/.config/nermal"
        );
