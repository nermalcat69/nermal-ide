...
const BIN_DIR: &str = "/home/me/.local/share/nermal/bin";
const BINARY: &str = "/home/me/.local/share/nermal/bin/nermal-server-c3p4";
const TEMP_BASE: &str = "/home/me/.local/share/nermal/bin/.nermal-server-c3p4.tmp";

fn temp() -> String {
    unique_temp(TEMP_BASE)
}

const SERVER_BYTES: &[u8] = b"\x7fELF...a static musl nermal-server, pretend it is 6 MB";
...
impl FakeRemote {
    ...
    fn with_legacy_install(self, version: &str) -> (Self, String) {
        let path = format!("{BIN_DIR}/nermal-server-{version}");
        ...
        if let Some(exe) = cmd.strip_suffix(&format!(" {PROTOCOL_FLAG}")) {
            ...
            return match self.speaks.lock().unwrap().get(exe) {
                ...
                None => Ok(ExecOutput {
                    status: Some(1),
                    stdout: String::new(),
                    stderr: "nermal-server: nothing to do without --daemon or --stdio".into(),
                }),
            };
        }
...
fn first_install_runs_all_six_steps() {
    ...
    assert_eq!(
        release.fetched(),
        vec![
            format!("https://github.com/nermalcat69/nermal-ide/releases/download/v{VERSION}/checksums.txt"),
            format!(
                "https://github.com/nermalcat69/nermal-ide/releases/download/v{VERSION}/{ASSET_LINUX_X86_64}"
            ),
        ]
    );
}
...
fn a_daemon_that_died_at_startup_fails_at_once_and_in_its_own_words() {
    let remote = FakeRemote::new().dying_at_startup(
        "1",
        "nermal-server: control listener unavailable: Permission denied (os error 13)\n",
    );
    remote.preinstall(BINARY, 0o755);
    let release = FakeRelease::new();
    let user = FakeUser::declining();
...
fn a_daemon_that_stood_down_cleanly_is_not_a_failed_start() {
    let remote = FakeRemote::new()
        .standing_down("nermal-server: another server already serves this config dir; exiting\n");
    remote.preinstall(BINARY, 0o755);
    let release = FakeRelease::new();
    let user = FakeUser::declining();
...
fn a_daemon_that_never_answers_names_the_probe_and_the_log() {
    let remote = FakeRemote::new().hanging_at_startup("nermal-server: still opening the tree\n");
    remote.preinstall(BINARY, 0o755);
    let release = FakeRelease::new();
    let user = FakeUser::declining();
...
fn restart_replaces_the_running_daemon() {
    let remote = FakeRemote::new()
        ...
        .serving(&format!("{BIN_DIR}/nermal-server-26.7.4"));
    remote.preinstall(BINARY, 0o755);
    let release = FakeRelease::new();
    let user = FakeUser::declining();
...
fn a_restart_with_nothing_to_start_leaves_the_running_daemon_alone() {
    ...
    let old = format!("{BIN_DIR}/nermal-server-c2p3");
...
fn the_launch_command_detaches_and_closes_every_stream() {
    let binary = "/home/me/.local/share/nermal/bin/nermal-server-26.7.5";
...
fn remote_paths_are_shell_quoted() {
    ...
    assert_eq!(
        shell_quote("/home/my box/nermal-server"),
        "'/home/my box/nermal-server'"
    );
...
fn the_launch_command_quotes_its_binary() {
    let binary = "/home/me/a b/nermal-server-1.0.0";
    let cmd = launch_command(binary, &StartupLog::for_binary(binary));
    assert!(cmd.contains("'/home/me/a b/nermal-server-1.0.0'"), "{cmd}");
    assert!(
        cmd.contains("'/home/me/a b/nermal-server-1.0.0.startup.log'"),
...
fn the_running_exe_probe_cannot_fail_the_command() {
    ...
    assert!(TERMINATE_RUNNING_COMMAND.contains("*/nermal-server-*"));
}
...
fn a_bundle_is_used_instead_of_downloading() {
    let dir = std::env::temp_dir().join(format!("nermal-bundle-src-{}", std::process::id()));
...
fn a_bundle_that_lacks_the_asset_does_not_fall_back_to_the_network() {
    let dir = std::env::temp_dir().join(format!("nermal-bundle-empty-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn discover_falls_back_to_release_when_bundled_is_missing() {
    let dir = std::env::temp_dir().join(format!("nermal-bundle-discover-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn discover_uses_bundled_when_it_is_present() {
    let dir = std::env::temp_dir().join(format!(
        "nermal-bundle-discover-present-{}",
        std::process::id()
    ));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
const OTHER_EXE: &str = "/home/me/.local/share/nermal/bin/nermal-server-26.7.9-nightly.20260801";
        ...
        while_changing_the_server(&sshd.conn, || {
            ...
            pane = Some(std::thread::spawn(move || {
                *conn.proved_server() = Some(ProvedServer {
                    binary: "/home/me/.nermal/bin/proved-mid-change".to_string(),
                    mismatch: None,
                });
