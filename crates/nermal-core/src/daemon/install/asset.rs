...
pub const ASSET_LINUX_X86_64: &str = "nermal-server-linux-x86_64-musl";
pub const ASSET_LINUX_AARCH64: &str = "nermal-server-linux-aarch64-musl";
...
pub const ASSET_MACOS_X86_64: &str = "nermal-server-macos-x86_64";
pub const ASSET_MACOS_AARCH64: &str = "nermal-server-macos-aarch64";
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::UnsupportedSystem { raw } => write!(
                f,
                "a remote nermal workspace needs a Linux or macOS host; this machine reports \
                 `uname -sm` = {raw:?}"
            ),
            Self::UnknownMachine { raw } => write!(
                f,
                "no nermal-server is published for this architecture (`uname -sm` = {raw:?}); \
...
pub fn binary_name(control: u32, protocol: u32) -> String {
    format!("nermal-server-c{control}p{protocol}")
}
...
pub fn dialect_from_path(path: &str) -> Option<(u32, u32)> {
    ...
    let (control, protocol) = name.strip_prefix("nermal-server-c")?.split_once('p')?;
...
mod tests {
    ...
    fn asset_names_are_the_ones_the_release_workflow_publishes() {
        assert_eq!(ASSET_LINUX_X86_64, "nermal-server-linux-x86_64-musl");
        assert_eq!(ASSET_LINUX_AARCH64, "nermal-server-linux-aarch64-musl");
        assert_eq!(ASSET_MACOS_X86_64, "nermal-server-macos-x86_64");
        assert_eq!(ASSET_MACOS_AARCH64, "nermal-server-macos-aarch64");
    ...
    fn remote_paths_are_posix_and_named_by_dialect() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/home/me/.local/share/nermal/bin",
            ]
        );
        assert!(
    ...
    fn trailing_slash_on_home_is_absorbed() {
        assert_eq!(
            ...
            "/root/.local/share/nermal/bin/nermal-server-c1p1"
        );
        assert_eq!(remote_paths("/", 1, 1).bin_dir, "/.local/share/nermal/bin");
    }

    #[test]
    fn dialects_are_recoverable_from_an_install_path() {
        assert_eq!(
            dialect_from_path("/home/me/.local/share/nermal/bin/nermal-server-c3p4"),
            Some((3, 4))
        );
        assert_eq!(dialect_from_path("nermal-server-c12p30"), Some((12, 30)));
        assert_eq!(dialect_from_path("/usr/bin/nermal-server"), None);
        assert_eq!(dialect_from_path("/bin/bash"), None);
        assert_eq!(dialect_from_path("nermal-server-c3"), None);
        assert_eq!(dialect_from_path("nermal-server-cxpy"), None);
    }

    #[test]
    fn legacy_version_named_binaries_carry_no_dialect() {
        for legacy in [
            "/home/me/.local/share/nermal/bin/nermal-server-26.7.4",
            "nermal-server-26.7.6-nightly.20260727",
            "nermal-server-0.1.0",
            "/usr/local/bin/nermal-server-",
        ] {
