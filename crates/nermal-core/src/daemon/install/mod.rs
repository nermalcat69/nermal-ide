    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingBundled { asset, searched } => write!(
                f,
                "this build of nermal does not ship a Linux server binary, so it cannot \
            ...
            Self::Declined { host, path } => write!(
                f,
                "installing nermal-server at {path} on {host} was not confirmed; nothing was written"
            ),
            ...
            Self::DialectMismatch {
                ...
                write!(
                    f,
                    "this build needs a nermal-server speaking control v{} and protocol v{}, \
...
impl<'a> Installer<'a> {
    ...
    /// Whether this machine has never had a nermal server on it — the question
    /// the consent prompt turns on.
    ///
    /// A launch leaves `nermal-server-c8p6.startup.log` and `.startup.exit`
    /// beside the binary, and both begin the same way it does. Counting one as
    /// a server would let someone who deleted the binary to uninstall nermal get
    ...
    fn is_first_install(&self, paths: &RemotePaths) -> bool {
        let a_server = |name: &String| {
            name.starts_with("nermal-server-")
    ...
    /// Stop whatever nermal-server is running over there and start the one this
    /// build speaks to.
    ///
    /// Which is not the same binary: the path is named after *our* dialect
    /// (`nermal-server-c{control}p{protocol}`), while the kill matches any
    /// `nermal-server-*` — see [`TERMINATE_RUNNING_COMMAND`]. When
...
/// `nermal-server-c7p6` — which is why the fallback stays a fallback and `/proc`
/// keeps first refusal.
///
...
const RUNNING_EXE_COMMAND: &str = r#"if [ -d /proc ]; then for p in /proc/[0-9]*; do e=$(readlink "$p/exe" 2>/dev/null) || continue; case "$e" in */nermal-server-*) printf '%s' "${e% (deleted)}"; break;; esac; done; else ps -xwwo pid=,comm= 2>/dev/null | while read -r pid e; do case "$e" in */nermal-server-*) printf '%s' "$e"; break;; esac; done; fi; true"#;

const TERMINATE_RUNNING_COMMAND: &str = r#"if [ -d /proc ]; then for p in /proc/[0-9]*; do e=$(readlink "$p/exe" 2>/dev/null) || continue; case "$e" in */nermal-server-*) kill -TERM "${p#/proc/}" 2>/dev/null; break;; esac; done; else ps -xwwo pid=,comm= 2>/dev/null | while read -r pid e; do case "$e" in */nermal-server-*) kill -TERM "$pid" 2>/dev/null; break;; esac; done; fi; true"#;
...
/// open a tab on a host nermal was already connected to and already serving. The
...
pub fn ensure_remote_server_labeled(conn: &Arc<SshConnection>, host: &str) -> io::Result<String> {
    ...
    if let Some(known) = slot.as_ref() {
        log::debug!(
            "remote {host}: nermal-server was already proved at {} on this connection",
            known.binary,
        );
    }
