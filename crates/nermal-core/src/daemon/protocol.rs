...
pub enum PortProbe {
    ...
    /// `nermal procs`, not for the panel: it names the tool and what went wrong.
    Unavailable(String),
}
...
pub enum AuthPromptKind {
    ...
    HostKeyUnknown {
        ...
        /// `nermal-server`, either of which may be an older build; an unknown
...
mod tests {
    ...
    /// workspace's panes are answered for by whatever `nermal-server` is
    ...
    /// prompt is decoded by whatever GUI or `nermal-server` is at the other end,
    ...
    /// crosses both daemon↔GUI and GUI↔remote `nermal-server`, so it has to
