//! The one spelling nermal stores a path on **this** machine in.
//!
...
/// The spelling nermal stores a local path in, so that two of them naming one
...
mod tests {
    ...
    fn off_windows_both_are_the_identity() {
        ...
        for p in ["/home/u/nermal", r"C:\Users\dev", r"mixed/path\here"] {
            let got = local_spelling(Path::new(p));
            assert_eq!(got.as_ref(), Path::new(p), "{p:?}");
