...
//! it does not own, `nermal tab ls` on the other side of a socket — has only
...
/// the paths that arrive here are not all `/…` or `~/…`: nermal's own PowerShell
...
mod tests {
    ...
    fn a_host_prefix_is_only_cut_when_a_path_follows_it() {
        ...
        assert_eq!(
            ...
            "C:/src",
            "nermal's own pwsh title names a drive when the cwd is off the home drive"
        );
