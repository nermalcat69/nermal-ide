...
pub const nermal_ENV_MARKER: &str = "nermal";
...
pub fn run_agent_hook(agent: &str, event: &str) {
    detach_console();
    if std::env::var_os(nermal_ENV_MARKER).is_none() {
        return;
    }
...
fn write_to_controlling_tty(bytes: &[u8]) -> bool {
    ...
    let shell = ancestors.iter().copied().find(|&pid| {
        procs
            ...
            .is_some_and(|n| is_nermal_host_exe(&n))
    });
...
fn is_nermal_host_exe(name: &str) -> bool {
    matches!(name, "nermal-app.exe" | "nermal-server.exe" | "nermal.exe")
}
...
impl HookAgent {
    ...
    fn target_path(self, target: &HookTarget) -> PathBuf {
        match self {
            ...
            HookAgent::Goose => {
                target.under_home(&[".agents", "plugins", "nermal", "hooks", "hooks.json"])
            }
...
impl<'a> HookTarget<'a> {
    ...
    /// Build a local target for a known nermal hook runner.
    ///
    ...
    /// `nermal-app` it would launch instead of comparing configs to `nermal`.
...
pub enum HookOutcome {
    ...
    NothingInstalled,
    /// The config is there, but holds no nermal hooks.
    NonermalHooks,
}
...
const OWNED_FILE_STEM_JSON: &str = "nermal.json";
const OWNED_FILE_STEM_JS: &str = "nermal.js";
...
/// leave the pane on "working" forever and `nermal wait` would only ever time
...
/// before every close and holds the tray and `nermal wait` on a turn long over.
...
fn hook_map_state(
    ...
    for (hook_event, nermal_event) in events {
        let ours = root
            .get("hooks")
        ...
        match ours {
            Some(cmd) => {
                any = true;
                if cmd != target.hook_command(agent, nermal_event) {
                    complete = false;
                }
            }
            None => complete = false,
...
fn hook_map_install(
    ...
    let marker = agent.marker();
    for (hook_event, nermal_event) in events {
        let command = target.hook_command(agent, nermal_event);
        let entries = hooks
            .as_object_mut()
            .unwrap()
            .entry(*hook_event)
...
fn hook_map_uninstall(
    ...
    if removed == 0 {
        return Ok(HookOutcome::NonermalHooks);
    }
    target.write(path, serde_json::to_string_pretty(&root)?.as_bytes())?;
    Ok(HookOutcome::Removed)
}

/// The nermal command an entry in a hook map carries, if it is one of ours.
///
...
fn toml_hooks_state(
    ...
    let complete = marked.len() == events.len()
        && events.iter().all(|(hook_event, nermal_event)| {
            let command = target.hook_command(agent, nermal_event);
...
fn toml_hooks_install(
    ...
    for (hook_event, nermal_event) in events {
        ...
        entry["command"] = toml_edit::value(target.hook_command(agent, nermal_event));
        list.push(entry);
    }
...
fn owned_file_install(
    ...
    {
        return Err(anyhow::anyhow!(
            "{} exists but wasn't written by nermal; not touching it",
            path.display()
        ));
    }
...
fn owned_file_uninstall(
    ...
    if !contents.contains(marker) {
        return Err(anyhow::anyhow!(
            "{} wasn't written by nermal; not touching it",
            path.display()
        ));
    }
    target.host.remove(path, false)?;
    // Take the directories nermal generated with it, innermost first, stopping at
    // the one named after nermal. `remove` is not recursive, so a directory still
    // holding someone else's file simply survives the attempt. Goose nests one
    // level deeper than the rest (`.../nermal/hooks/hooks.json`), which is why
    // this walks rather than checking a single parent.
    let mut dir = path.parent();
    while let Some(d) = dir {
        if d.file_name().is_some_and(|n| n == "nermal") {
...
fn opencode_plugin_js(target: &HookTarget) -> Option<String> {
    ...
    .ok()?;
    Some(format!(
        r#"// nermal agent-hook opencode bridge — generated by nermal, do not edit.
// Bridges OpenCode plugin events onto `nermal agent-hook opencode <event>`,
// which is inert outside nermal (gated on the nermal env var).
export const NermalPresence = async ({{ $ }}) => {{
  if (!process.env["nermal"]) return {{}}
  const cmd = {prefix}
  let sessionId = ""
  let announced = ""
...
fn pi_extension_ts(target: &HookTarget, agent: HookAgent) -> Option<String> {
    ...
    Some(format!(
        r#"/* nermal agent-hook {slug} bridge — generated by nermal, do not edit. */
...
mod tests {
    ...
    #[test]
    fn every_nermal_daemon_host_takes_the_console_fast_path() {
        for name in ["nermal-app.exe", "nermal-server.exe", "nermal.exe"] {
    ...
    fn the_new_hook_agents_target_the_paths_their_clis_read() {
        ...
        for (agent, want) in [
            ...
            (
                HookAgent::Goose,
                "/home/me/.agents/plugins/nermal/hooks/hooks.json",
            ),
        ...
        let dir = std::env::temp_dir().join(format!("nermal-new-hooks-{}", std::process::id()));
    ...
    fn uninstalling_goose_takes_its_generated_plugin_dirs_with_it() {
        let root = std::env::temp_dir().join(format!("nermal-goose-test-{}", std::process::id()));
        let plugins = root.join("plugins");
        let plugin = plugins.join("nermal");
        ...
        assert!(!plugin.exists(), "and the nermal plugin dir above it");
    ...
    fn marker_detection_matches_our_entries_only() {
        let ours = serde_json::json!({
            "hooks": [{ "type": "command", "command": "\"/x/nermal\" agent-hook claude stop" }]
        });
        ...
        let flat = serde_json::json!({
            "matcher": "^bash$",
            "command": "\"/x/nermal\" agent-hook crush prompt-submit"
        });
        assert_eq!(
            marker_command(&flat, "agent-hook crush"),
            Some("\"/x/nermal\" agent-hook crush prompt-submit")
        );
    ...
    fn remote_paths_are_built_in_the_remote_machine_s_spelling() {
        ...
        for (agent, expected) in [
            ...
            (HookAgent::TraeCode, "/home/me/.trae/cli/hooks.json"),
            (HookAgent::Copilot, "/home/me/.copilot/hooks/nermal.json"),
            (
                HookAgent::OpenCode,
            ...
            (
                HookAgent::OhMyPi,
                "/home/me/.omp/agent/extensions/nermal/index.ts",
            ),
            (HookAgent::Kimi, "/home/me/.kimi-code/config.toml"),
            (HookAgent::QoderCLI, "/home/me/.qoder/settings.json"),
            (HookAgent::Crush, "/home/me/.config/crush/crush.json"),
    ...
    fn the_hook_command_names_the_binary_on_that_machine() {
        ...
        let target = HookTarget::remote(&*host, PathBuf::from("/home/me"));
        let dialect = crate::daemon::install::RemoteProtocol::of_this_build();
        let name = format!("nermal-server-c{}p{}", dialect.control, dialect.protocol);
        assert_eq!(
            target.hook_command(HookAgent::Claude, "stop"),
            format!("\"/home/me/.local/share/nermal/bin/{name}\" agent-hook claude stop")
        );
    ...
    fn a_remote_install_round_trips_through_the_host() {
        let dir = std::env::temp_dir().join(format!("nermal-remote-hooks-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let host = FakeRemote::shared();
    ...
    fn owned_file_contents_carry_marker_and_exe() {
        ...
        assert!(opencode.contains(r#"process.env["nermal"]"#));
        ...
        for (agent, slug, package) in [
            ...
            assert!(bridge.contains(r#"process.env["nermal"]"#));
    ...
    fn owned_file_round_trip_and_ownership_guard() {
        let dir = std::env::temp_dir().join(format!("nermal-owned-test-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("nermal.json");
    ...
    fn qoder_config_dir_controls_local_hook_lifecycle() {
        const CASE_ENV: &str = "nermal_TEST_QODER_CONFIG_CASE";
        const ROOT_ENV: &str = "nermal_TEST_QODER_CONFIG_ROOT";
    ...
    fn crush_installs_a_flat_hook_and_preserves_user_entries() {
        let host = FakeRemote::shared();
        let base = std::env::temp_dir().join(format!("nermal-crush-hooks-{}", std::process::id()));
        ...
        let ours: Vec<&serde_json::Value> = entries
            ...
            .collect();
        assert_eq!(ours.len(), 1, "exactly one nermal entry after two installs");
        assert_eq!(
    ...
    fn crush_global_config_controls_local_hook_lifecycle() {
        const CASE_ENV: &str = "nermal_TEST_CRUSH_CONFIG_CASE";
        const ROOT_ENV: &str = "nermal_TEST_CRUSH_CONFIG_ROOT";
        let Ok(case) = std::env::var(CASE_ENV) else {
            // Each case gets its own environment, without changing the one
            // shared by the other tests or touching the user's settings.
            for case in ["override", "empty", "unset"] {
    ...
    fn install_is_idempotent_and_preserves_user_hooks() {
        let dir = std::env::temp_dir().join(format!("nermal-hooks-test-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        ...
        assert_eq!(
            stop.iter()
                ...
                .count(),
            1,
            "exactly one nermal entry after two installs"
        );
        assert!(
            stop.iter()
        ...
        assert!(
            ...
            "an event list that held only the nermal hook is dropped"
        );
    ...
    fn kimi_install_preserves_the_user_s_config_toml() {
        ...
        assert_eq!(
            ...
            "exactly one nermal entry per event after two installs"
        );
    ...
    fn kimi_refuses_a_hooks_key_of_the_wrong_toml_type() {
        let host = FakeRemote::shared();
        let base = std::env::temp_dir().join(format!("nermal-kimi-shapes-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
    ...
    fn kimi_toml_merge_holds_up_across_the_awkward_shapes() {
        let host = FakeRemote::shared();
        let base = std::env::temp_dir().join(format!("nermal-kimi-merge-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        ...
        extra.push_str("\n[[hooks]]\ncommand = \"nermal agent-hook kimi stop\"\n");
        ...
        for event in ["Stop", "Interrupt", "StopFailure"] {
            assert_eq!(
                KIMI_HOOK_EVENTS
                    ...
                    .map(|(_, nermal_event)| *nermal_event),
                Some("stop"),
