...
pub const DATA_DIR_ENV: &str = "nermal_DATA_DIR";
...
pub struct PaneRecord {
    ...
    /// switcher listing a workspace it does not own, `nermal tab ls` across a
...
pub struct PaneSeed {
    ...
    /// tree as a seed — a split, a `nermal` CLI call — names its shell from the
...
impl MachineStore {
    ...
    fn ensure_flusher(self: &Arc<Self>) {
        ...
        let spawned = std::thread::Builder::new()
            .name("nermal-machine-flush".into())
            .spawn(move || {
                loop {
...
/// nermal's default preset is: a daemon that spawns a pane before the GUI has
...
/// two nermals pointed at different config directories — each holding its own
...
///
/// Deliberately still resolved the old way, `nermal_DATA_DIR` included: this
...
fn adopt_into(current: Option<&Path>, legacy: Option<&Path>, machines_instance: bool) {
    ...
    // makes `nermal_DATA_DIR` a no-op: both getters answer with it.
    if legacy == current {
        return;
    }
...
/// Whether this process is the machine's nermal rather than an instance somebody
...
/// `$nermal_CONFIG_DIR` when the box sets one, `$HOME`'s otherwise. Anything
/// aimed elsewhere is a second nermal by definition, and a second nermal starts on
/// an empty tree.
///
...
/// launch a real `nermal-server --config-dir <TempDir>` under the developer's own
/// `HOME`: a start-order rule moves the developer's real `machine.json` into a
/// scratch directory and deletes it with the `TempDir`.
///
/// Counting `$nermal_CONFIG_DIR` as the machine's is what keeps remote hosts
/// upgrading: a remote `nermal-server` is launched with no `--config-dir` and
...
mod tests {
    ...
    fn tabs_rename_move_and_regroup_in_place() {
        ...
        store
            ...
            .unwrap();
        store
            .tab_set_group(ws, first.id, Some("/repo/nermal".into()), None)
            .unwrap();
    ...
    fn an_attachment_travels_by_name_and_never_by_token() {
        ...
        // What `MachineGet` hands a peer: `nermal ls` reads its ATTACHED column
    ...
    /// behind, so two nermals that each believed they were the only server on
    ...
    /// must not be what picks. A second nermal renaming the tree into its own
    ...
    fn only_the_machines_own_instance_inherits_the_legacy_tree() {
        let machines = PathBuf::from("/home/u/.config/nermal");
    ...
    /// `nermal_DATA_DIR` answers both getters, so the sandboxes every test
