...
impl DiffSource {
    ...
    /// The whole argv, minus pathspecs. Every diff nermal runs is built here so
...
/// - the line is in the first parent — the side nermal numbers — iff its own
...
/// shape. The first `-` is the first parent's, which is the side nermal numbers.
...
";
    ...
    fn merge_repo(name: &str) -> Option<PathBuf> {
        let dir = std::env::temp_dir().join(format!("nermal-diff-{name}-{}", std::process::id()));
