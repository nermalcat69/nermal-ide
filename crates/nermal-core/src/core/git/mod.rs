//! Everything nermal knows about git, split by question:
//!
...
//!
//! Nothing here depends on gpui: the headless `nermal-server` answers the same
...
/// A path `git` just printed, in the spelling the rest of nermal keys by.
///
...
fn emit(line: &[u8], dropped: usize, on_line: &mut impl FnMut(&str)) {
    ...
    text.push_str(&format!(
        " …[{dropped} more bytes on this line dropped: past nermal's {MAX_LINE}-byte line cap]"
    ));
    on_line(&text);
}
...
mod tests {
    ...
    #[test]
    fn non_repo_is_none() {
        let dir = std::env::temp_dir().join("nermal-git-status-not-a-repo-xyz");
    ...
    fn missing_path_is_none() {
        assert_eq!(probe(&*h(), Path::new("/no/such/nermal/path/here")), None);
    }
    ...
    fn a_probed_root_is_the_same_path_the_host_canonicalizes_to() {
        let host = h();
        let dir = std::env::temp_dir().join(format!("nermal-root-spelling-{}", std::process::id()));
