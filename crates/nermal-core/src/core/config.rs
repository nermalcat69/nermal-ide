...
pub struct Config {
    ...
    /// Save an open file to disk shortly after every edit, instead of only on
    /// an explicit ctrl-s. Toggled from the editor's own status bar, next to
    /// wrap — the same reasoning as [`Config::document_layout`]'s comment
    /// applies: it is a per-buffer question, asked at the buffer, not in
    /// Settings.
    #[serde(default)]
    ...
    /// Whether the GUI puts the bundled `nermal` CLI on PATH at launch (see
    ...
    /// Off is for people who keep their own `nermal` — a `cargo install` build, a
    ...
    #[serde(default, deserialize_with = "de_lenient")]
    pub bell: BellMode,
    /// Whether nermal edits the shell prompt itself. On by default: the inline
    ...
    /// dotfile work exactly as they do outside nermal. Shell integration itself
    /// stays on: prompt boundaries, cwd, exit status and notifications are
    /// unaffected. `tab_completion` and `history_search` are nermal's own menus,
    ...
    ///
    /// Defaults to the platform modifier, which is what nermal has always done —
    ...
    /// Optional HTTP/SOCKS proxy for nermal's *own* update checks and release
...
/// crosses as user-authored: nermal has no defaults to contribute to a command it
/// did not choose.
#[derive(Debug, Clone, Default, PartialEq, Eq, Deserialize, Serialize)]
#[serde(default)]
...
/// redirects it — `$nermal_CONFIG_DIR` where the box names one, the default under
/// `$HOME` otherwise.
///
/// [`config_dir`] with the `--config-dir` override left off, which is the
/// question "is this the machine's nermal or a second one somebody pointed
...
pub fn machine_config_dir() -> Option<PathBuf> {
    if let Some(dir) = std::env::var_os("nermal_CONFIG_DIR").filter(|d| !d.is_empty()) {
...
pub fn default_config_dir() -> Option<PathBuf> {
    ...
    Some(PathBuf::from(home).join(".config/nermal"))
}

#[cfg(windows)]
pub fn default_config_dir() -> Option<PathBuf> {
    if let Some(appdata) = std::env::var_os("APPDATA").filter(|d| !d.is_empty()) {
        return Some(PathBuf::from(appdata).join("nermal"));
    }
    let profile = std::env::var_os("USERPROFILE").filter(|d| !d.is_empty())?;
    Some(PathBuf::from(profile).join(".config").join("nermal"))
}
...
pub enum LinkFileOpen {
    /// nermal's own editor. The default, and the only one of the three that can
...
mod tests {
    ...
    fn sanitize_clamps_to_the_same_bounds_the_gui_steps_within() {
        ...
        // wrong way. The bounds are one shared set now: `NermalApp::set_font_size`
