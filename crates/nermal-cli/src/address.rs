...
pub const ENV_PANE: &str = "nermal_PANE";
pub const ENV_WS: &str = "nermal_WS";
/// The server's config dir, not a socket path — the server publishes the
/// directory so both endpoints resolve through nermal_core's own derivation
...
pub const ENV_CONFIG_DIR: &str = "nermal_CONFIG_DIR";

pub const OUTSIDE_SHELL: &str = "not inside a nermal shell — pass an explicit %pane/@tab/workspace";
...
pub fn parse_pane(s: &str) -> Result<u64> {
    // The `%` is optional: `nermal pane ls --json` hands back bare ids, so a
...
pub fn parse_tab(s: &str) -> Result<TabAddress> {
    ...
    // `--json` payload spells tabs bare, so the id `nermal tab new --json` just
    ...
    let not_an_address =
        || anyhow!("'{s}' is not a tab address — @7 as numbered by `nermal ls`, or a full tab id");
    if body.is_empty() {
        return Err(not_an_address());
    }
...
mod tests {
    ...
    fn shell_context() -> Context {
        Context {
            ...
            config_dir: Some("C:\\Users\\me\\AppData\\Roaming\\nermal".into()),
        }
    }
    ...
    fn a_bare_pane_id_addresses_the_same_pane_as_the_marked_one() {
        // `nermal pane ls --json` prints bare ids; refusing them here pushed
        // people into "%${nermal_PANE#%}" contortions (#538).
        assert_eq!(parse_pane("42").unwrap(), 42);
        assert_eq!(parse_pane("%42").unwrap(), 42);
    }
    ...
    fn a_bare_tab_id_addresses_the_same_tab_as_the_marked_one() {
        // Every `--json` payload spells tabs bare, and the id `nermal tab new`
    ...
    #[test]
    fn outside_a_nermal_shell_the_error_names_the_fix() {
    ...
    fn a_broken_env_pane_is_reported_not_silently_ignored() {
        ...
        let err = pane_or_context(None, &ctx).unwrap_err().to_string();
        assert!(err.contains("nermal_PANE"), "{err}");
    }
}
