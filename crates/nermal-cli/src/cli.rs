...
#[command(
    ...
    long_about = "One name, three artifacts: this CLI, the nermal GUI (nermal-app), and\n\
                  nermal-server.\n\n\
                  ...
                  protocol of its own. Inside a nermal shell, $nermal_CONFIG_DIR points at\n\
                  the server, and $nermal_PANE / $nermal_WS let the address-taking verbs run\n\
...
pub enum Command {
    #[command(about = "This machine on one screen (= nermal ws ls)")]
    Ls,
    ...
    #[command(about = "Split a pane (= nermal pane split)")]
    Split(SplitArgs),
    ...
    #[command(
        about = "Block until a pane's agent needs input, finishes its turn, or the pane \
                 exits — the orchestration primitive: `nermal wait %3 && nermal capture %3 --plain`"
    )]
    Wait(WaitArgs),
    ...
    #[command(about = "Server health at a glance (= nermal server status)")]
    Status,
    ...
    #[command(subcommand, about = "The nermal-server process on this machine")]
    Server(ServerCmd),
}
...
pub struct RunArgs {
    ...
    #[arg(
        ...
        help = "Sets the pane's nermal_WS and, with --keep, the workspace the kept pane is \
                filed into; defaults to $nermal_WS inside a nermal shell"
    )]
    ...
    #[arg(
        ...
        required = true,
        value_name = "CMD",
        help = "The command, after `--`: nermal run -- cargo test"
    )]
    pub cmd: Vec<String>,
}
...
pub struct SplitArgs {
    #[arg(
        value_name = "%PANE",
        help = "Pane to split; defaults to $nermal_PANE inside a nermal shell"
    )]
...
pub struct SendArgs {
    ...
    #[arg(
        ...
        help = "Send a keystroke instead of text; repeat for a sequence \
                (C-c, escape, up, enter, …). See `nermal send --help`"
    )]
...
pub struct WaitArgs {
    #[arg(
        value_name = "%PANE",
        help = "Pane to watch; defaults to $nermal_PANE inside a nermal shell"
    )]
    pub target: Option<String>,
...
pub struct CaptureArgs {
    #[arg(
        value_name = "%PANE",
        help = "Pane to read; defaults to $nermal_PANE inside a nermal shell"
    )]
    pub target: Option<String>,
    ...
    New {
        ...
        #[arg(
            ...
            help = "Re-home a pane that is already running instead of spawning a shell — \
                    for the orphans `nermal pane ls --all` lists. Defaults the workspace to \
...
mod tests {
    ...
    #[test]
    fn bare_nermal_is_the_gui_launcher() {
        let cli = parse(&["nermal"]);
    ...
    fn a_path_that_is_not_a_verb_still_launches_the_gui() {
        let cli = parse(&["nermal", "C:\\Users\\me\\proj"]);
    ...
    fn a_gui_path_preserves_native_windows_arguments() {
        ...
        let cli = Cli::try_parse_from([OsString::from("nermal"), native_path.clone()])
    ...
    fn every_top_level_verb_parses() {
        ...
        assert!(matches!(
            parse(&["nermal", "new", "C:\\proj"]).command,
            Some(Command::New { path: Some(p), .. }) if p == "C:\\proj"
        ));
        assert!(matches!(
            parse(&["nermal", "agents"]).command,
            Some(Command::Agents)
        ));
        assert!(matches!(
            parse(&["nermal", "events"]).command,
            Some(Command::Events)
        ));
        assert!(matches!(
            parse(&["nermal", "status"]).command,
            Some(Command::Status)
        ));
        assert!(matches!(
            parse(&["nermal", "doctor"]).command,
            Some(Command::Doctor)
        ));
        assert!(matches!(
            parse(&["nermal", "procs", "%3"]).command,
    ...
    fn run_takes_the_command_after_a_double_dash() {
        let cli = parse(&["nermal", "run", "--", "cargo", "test", "--keep"]);
        ...
        assert!(
            !args.keep,
            "--keep after -- belongs to the child, not to nermal"
        );

        let cli = parse(&["nermal", "run", "--keep", "--cwd", "C:\\proj", "--", "make"]);
        let Some(Command::Run(args)) = cli.command else {
            panic!("run did not parse");
        };
        assert!(args.keep);
    ...
    fn run_without_a_command_is_a_usage_error() {
        let err = Cli::try_parse_from(["nermal", "run"]).unwrap_err();
        assert_eq!(err.kind(), ErrorKind::MissingRequiredArgument);
    }

    #[test]
    fn split_requires_exactly_one_axis() {
        let cli = parse(&["nermal", "split", "--v"]);
        let Some(Command::Split(args)) = cli.command else {
            panic!("split did not parse");
        };
        assert!(args.vertical && !args.horizontal);
        assert!(args.target.is_none(), "the pane comes from $nermal_PANE");
        assert_eq!(args.ratio, 0.5);

        let cli = parse(&["nermal", "pane", "split", "%42", "--h", "--ratio", "0.3"]);
        ...
        let err = Cli::try_parse_from(["nermal", "split", "%1"]).unwrap_err();
        assert_eq!(
            err.kind(),
            ErrorKind::MissingRequiredArgument,
            "an axis must be chosen"
        );
        let err = Cli::try_parse_from(["nermal", "split", "--h", "--v"]).unwrap_err();
        assert_eq!(
            err.kind(),
    ...
    fn send_takes_an_optional_address_then_text() {
        let cli = parse(&["nermal", "send", "%42", "make -j8", "--enter"]);
        ...
        let cli = parse(&["nermal", "send", "make -j8"]);
        let Some(Command::Send(args)) = cli.command else {
            panic!("send did not parse");
        };
    ...
    fn send_accepts_keys_with_or_without_text() {
        let cli = parse(&["nermal", "send", "%42", "--key", "C-c"]);
        let Some(Command::Send(args)) = cli.command else {
            panic!("send did not parse");
        };
        assert_eq!(args.first.as_deref(), Some("%42"));
        ...
        let cli = parse(&["nermal", "send", "--key", "down", "--key", "enter"]);
        let Some(Command::Send(args)) = cli.command else {
            panic!("send did not parse");
        };
        assert!(args.first.is_none(), "the pane comes from $nermal_PANE");
        ...
        let err = Cli::try_parse_from(["nermal", "send", "--key", "f7"]).unwrap_err();
    ...
    fn every_pane_verb_parses() {
        ...
        assert!(matches!(
            parse(&["nermal", "pane", "close", "%9"]).command,
        ...
        assert!(matches!(
            parse(&["nermal", "pane", "close", "%9", "%10"]).command,
            Some(Command::Pane(PaneCmd::Close { targets, .. })) if targets == ["%9", "%10"]
        ));
        assert!(matches!(
            parse(&["nermal", "pane", "close", "--orphans"]).command,
        ...
        let err = Cli::try_parse_from(["nermal", "pane", "close", "%9", "--orphans"]).unwrap_err();
    ...
    fn every_machine_and_server_verb_parses() {
        assert!(matches!(
            parse(&["nermal", "machine", "ls"]).command,
            Some(Command::Machine(MachineCmd::Ls))
        ));
        assert!(matches!(
            parse(&["nermal", "machine", "connect", "devbox"]).command,
            Some(Command::Machine(MachineCmd::Connect { profile })) if profile == "devbox"
        ));
        assert!(matches!(
            parse(&["nermal", "machine", "disconnect", "devbox"]).command,
        ...
        for (verb, want) in [
            ...
            let cli = parse(&["nermal", "server", verb]);
        ...
        assert!(matches!(
            parse(&["nermal", "server", "restart", "--hard"]).command,
    ...
    fn global_flags_are_accepted_anywhere() {
        let cli = parse(&["nermal", "-m", "devbox", "ls", "--json"]);
        assert_eq!(cli.machine.as_deref(), Some("devbox"));
        assert!(cli.json);
        assert!(matches!(cli.command, Some(Command::Ls)));

        let cli = parse(&["nermal", "ws", "ls", "-q", "-m", "devbox"]);
        assert!(cli.quiet);
        assert_eq!(cli.machine.as_deref(), Some("devbox"));

        let cli = parse(&["nermal", "capture", "%3", "--scrollback", "--json"]);
        assert!(cli.json);
    ...
    fn capture_plain_is_its_own_flag_and_composes_with_scrollback() {
        ...
        let Some(Command::Capture(args)) = parse(&["nermal", "capture", "--plain"]).command else {
            panic!("capture did not parse");
        };
        ...
        let Some(Command::Capture(args)) =
            parse(&["nermal", "capture", "%3", "--plain", "--scrollback"]).command
        else {
            panic!("capture did not parse");
        };
        assert!(args.plain && args.scrollback);
    ...
    fn capture_tail_takes_a_count_and_refuses_zero() {
        ...
        let Some(Command::Capture(args)) = parse(&["nermal", "capture", "%3"]).command else {
            panic!("capture did not parse");
        };
        ...
        // is about.
        for bad in [
            vec!["nermal", "capture", "%3", "--tail", "0"],
            vec!["nermal", "capture", "%3", "--tail", "-1"],
            vec!["nermal", "capture", "%3", "--tail", "lots"],
        ] {
    ...
    fn usage_errors_exit_with_code_2() {
        let err = Cli::try_parse_from(["nermal", "ws", "frobnicate"]).unwrap_err();
        assert_eq!(
            err.exit_code(),
            2,
            "clap's usage-error exit code is the CLI contract"
        );
        let err = Cli::try_parse_from(["nermal", "tab", "move", "@7", "not-a-number"]).unwrap_err();
        assert_eq!(err.exit_code(), 2);
    }
}
