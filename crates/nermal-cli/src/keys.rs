...
/// answered with, and half of what `nermal send --help` prints.
pub fn vocabulary() -> String {
    let named: Vec<&str> = NAMED.iter().map(|(name, _)| *name).collect();
    format!("{}, C-<char> (Ctrl), M-<char> (Alt)", named.join(", "))
}

/// `nermal send --help`. Assembled from the tables above so that adding a key or
