...
pub const SERVER_EXE_ENV: &str = "nermal_SERVER_EXE";
...
/// `$nermal_CONFIG_DIR`: `spawn::stop` dials `transport::connect`, which derives
...
/// endpoint these verbs reach and the one `nermal status` reports on are one and
...
pub fn stop() -> Result<Outcome> {
    if !running() {
        ...
        let Some(stranded) = nermal_core::daemon::singleton::holder_pid() else {
            return report(
        ...
        if nermal_core::daemon::singleton::holder_pid() == Some(stranded) {
            bail!(
...
/// The daemon execs the nermal-server binary found by [`server_exe`]: same pid,
...
fn restart_in_place(
    client: &PaneClient,
    old: &nermal_core::daemon::protocol::DaemonVersion,
) -> Result<Outcome> {
    let exe = server_exe()?;
    client.hand_off(&exe).map_err(|e| {
        anyhow::anyhow!(
            "the server refused to restart in place and was left running: {e} — \
             `nermal server restart --hard` stops and starts it instead; sessions end"
        )
    })?;
    let deadline = Instant::now() + START_TIMEOUT;
    loop {
    ...
    if nermal_core::daemon::singleton::holder_pid().is_some() {
        bail!(
            "the server took the handoff but has not started answering within \
             ...
             survive; give it a moment and check `nermal server status`"
        );
    }
...
pub fn logs() -> Result<Outcome> {
    let Some(path) = config::config_path("nermal.log") else {
    ...
    match std::fs::read_to_string(&path) {
        ...
        Err(_) => {
            human.push_str("no log file yet — set nermal_LOG=info before starting the server\n");
        }
    }
    report(
        human,
...
/// `exists()`, so a directory named `nermal-server` shadowed the real one on PATH
...
mod tests {
    ...
    fn the_override_wins_and_is_taken_at_its_word() {
        let got = resolve_server_exe(
            Some(OsStr::new("/opt/custom/nermal-server")),
            Some(Path::new("/usr/local/bin")),
            &dirs(&["/usr/bin"]),
            NAME,
            only(&["/usr/local/bin/nermal-server", "/usr/bin/nermal-server"]),
        );
        assert_eq!(got, Some(PathBuf::from("/opt/custom/nermal-server")));
    }
    ...
    fn a_nonexistent_override_is_still_returned() {
        let got = resolve_server_exe(
            Some(OsStr::new("/nope/nermal-server")),
            Some(Path::new("/usr/local/bin")),
            &dirs(&["/usr/bin"]),
            NAME,
            only(&["/usr/bin/nermal-server"]),
        );
        assert_eq!(got, Some(PathBuf::from("/nope/nermal-server")));
    }
    ...
    fn a_sibling_beats_path() {
        let got = resolve_server_exe(
            None,
            Some(Path::new("/opt/nermal")),
            &dirs(&["/usr/bin"]),
            NAME,
            only(&["/opt/nermal/nermal-server", "/usr/bin/nermal-server"]),
        );
        assert_eq!(got, Some(PathBuf::from("/opt/nermal/nermal-server")));
    }
    ...
    fn path_is_searched_in_order_when_there_is_no_sibling() {
        let got = resolve_server_exe(
            ...
            &dirs(&["/a", "/b", "/c"]),
            NAME,
            only(&["/b/nermal-server", "/c/nermal-server"]),
        );
        assert_eq!(got, Some(PathBuf::from("/b/nermal-server")));
    }

    /// The bug behind holding both candidates to `is_file`: a *directory*
    /// named `nermal-server` beside this binary used to satisfy `exists()`, so it
    ...
    fn a_directory_by_that_name_does_not_shadow_the_real_binary() {
        let got = resolve_server_exe(
            ...
            &dirs(&["/usr/bin"]),
            NAME,
            // /opt/nermal/nermal-server exists but is not a file, so it is absent here.
            only(&["/usr/bin/nermal-server"]),
        );
        assert_eq!(got, Some(PathBuf::from("/usr/bin/nermal-server")));
    }
