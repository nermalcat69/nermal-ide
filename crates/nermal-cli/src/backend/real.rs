...
const NOT_RUNNING: &str =
    "could not reach the nermal server on this machine — `nermal server start` brings one up";
...
impl RealBackend {
    ...
    /// `nermal_CONFIG_DIR`, which is what `nermal_core`'s own endpoint derivation
    ...
    fn hello_msg() -> ControlHello {
        ControlHello::host_rpc(format!("nermal-cli-{}", std::process::id()), hostname())
    }
        ...
        let code = loop {
            match session.recv() {
                Ok(DaemonMsg::Output(bytes)) | Ok(DaemonMsg::Snapshot(bytes)) => {
                    ...
                    // (`nermal run -- … | head`) must end the pipeline, not turn
...
fn hostname() -> String {
    std::env::var("COMPUTERNAME")
        ...
        .unwrap_or_else(|_| "nermal-cli".to_string())
}
