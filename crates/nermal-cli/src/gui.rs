...
/// The bundled `nermal-app` this CLI belongs to: `nermal_APP`, then the file next
...
pub fn find_executable() -> Result<PathBuf> {
    if let Some(explicit) = std::env::var_os("nermal_APP") {
        let path = PathBuf::from(explicit);
        if path.is_file() {
            return Ok(path);
        }
        bail!(
            "nermal_APP points to {}, but that file does not exist",
            path.display()
        );
    }
