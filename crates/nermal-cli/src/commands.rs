...
pub fn execute(cli: Cli, ctx: &Context, backend: &mut dyn Backend) -> Result<Outcome> {
    ...
    match cli.command {
        None => match cli.path {
            ...
            // is: launching the GUI for `nermal statu` would hide the typo.
            Some(word) if !looks_like_a_path(&word) => bail!(
                "unknown subcommand '{}' — run `nermal --help` for the list. \
        ...
        Some(Command::Ws(WsCmd::Stop { .. })) => bail!(
            "`nermal ws stop` is not implemented yet — the control dialect has no \
...
fn local_server(
    ...
    if let Some(machine) = machine {
        bail!(
            "`nermal server {verb}` manages only the server on THIS machine — with -m {machine} \
...
fn launch_gui(
    ...
    if let Some(machine) = machine {
        bail!(
            "`nermal [PATH]` controls the GUI on this machine and cannot be combined with -m {machine}"
        );
    }
    ...
    let opened = match open {
        ...
        true => match backend.control(ControlRequest::GuiOpen {
            ...
            Ok(ReplyOk::Bool(opened)) => {
                ...
                if !opened {
                    eprintln!(
                        "nermal: no GUI is running on this machine; \
            ...
            Err(error) => {
                eprintln!(
                    "nermal: could not ask the GUI to open it ({error:#}); \
                     the workspace was made all the same"
                );
                false
            }
...
fn run(args: RunArgs, ctx: &Context, backend: &mut dyn Backend) -> Result<Outcome> {
    ...
    if args.keep && workspace.is_none() {
        bail!(
            "`run --keep` keeps the pane alive, so it must be filed into a workspace — \
             pass --ws, or run inside a nermal shell where $nermal_WS names one"
        );
    }
    ...
    let (code, exact) = match backend.run_wait()? {
        Some(code) => (code, true),
        None => {
            eprintln!("nermal: {EXIT_CODE_UNKNOWN}");
            (1, false)
        }
    };
...
fn capture(args: CaptureArgs, ctx: &Context, backend: &mut dyn Backend) -> Result<Outcome> {
    ...
    if rendered.is_empty() && replayed > 0 {
        eprintln!(
            "nermal: capture %{pane}: {replayed} bytes replayed and the grid they \
...
fn adopt_pane(
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "no pane %{pane} is running on this machine — \
                 `nermal pane ls --all` lists every pane the server holds"
            )
        })?;
    if let Ok(holder) = resolve::workspace_of_pane(machine, pane) {
        bail!(
            "%{pane} is already in a tab of workspace {} — `nermal pane split` adds \
             to that tab, and `nermal tab new --pane` is for panes no tab holds",
    ...
    // not inside nermal, so `$nermal_WS` is not going to answer here.
        ...
        (None, None) => owner_of(info, machine).ok_or_else(|| {
            anyhow::anyhow!(
                "%{pane} does not name a workspace that still exists — \
                 say which one to re-home it into: `nermal tab new <workspace> --pane %{pane}`"
            )
        })?,
...
fn owner_of(
    info: &nermal_core::daemon::protocol::PaneInfo,
    machine: &Machine,
) -> Option<WorkspaceId> {
...
/// interrupted `nermal run` leaves its pane running with nothing referencing it,
...
fn pane_ls_all(backend: &mut dyn Backend) -> Result<Outcome> {
    ...
    if orphans > 0 {
        human.push_str(&format!(
            "\n{orphans} pane(s) held by no workspace — `nermal tab new --pane %<id>` puts one \
             back in a tab, `nermal pane close %<id>` stops one, `nermal pane close --orphans` \
             stops all of them\n"
        ));
    }
...
fn pane_close(
    ...
    if !failures.is_empty() {
        ...
        eprintln!(
            "nermal: closed {} pane(s); {} could not be closed — {}",
            closed.len(),
            failures.len(),
            failures.join("; ")
        );
...
/// stateless question composes into scripts (`nermal wait %3 && nermal capture %3
    ...
    loop {
        ...
        if matched || current == WaitState::Exit {
            ...
            if !matched {
                ...
                eprintln!("nermal: pane %{pane} exited before reaching the awaited state");
                return Ok(Outcome::Exit(
                    1,
                    Report {
        ...
        if free_is_the_only_hope && unknown_polls >= UNKNOWN_POLLS {
            ...
            eprintln!("nermal: pane %{pane}: cannot determine whether it is free");
            let session = entry.as_ref().map(|e| &e.state);
            return Ok(Outcome::Exit(
                1,
                Report {
        ...
        if deadline.is_some_and(|d| Instant::now() >= d) {
            ...
            if current == WaitState::NoAgent && !watch_free {
                human.push_str(
                    "\nnothing is reporting agent status in this pane — for a plain command \
                     wait `--until free`, and for an agent check `nermal agents` for a missing \
                     status hook",
                );
            }
            ...
            eprintln!("nermal: pane %{pane}: still {} — timed out", current.name());
            return Ok(Outcome::Exit(
                124,
                Report {
                    human,
...
/// `nermal run` spawns — reads free for as long as it runs, and a backgrounded
...
fn pane_freeness(backend: &mut dyn Backend, pane: u64) -> Result<Freeness> {
        ...
        return Ok(match ctx.at_prompt {
            ...
            _ => Freeness::Unknown(format!(
                "this pane is {whereabouts}, so its local process tree describes this end of \
                 ...
                 running remote command. Install nermal's shell integration on the remote host, \
    ...
    Ok(Freeness::Unknown(
        "the pane's process tree came back empty and its shell has sent no prompt marks, so \
         there is nothing here to read freeness off — check `nermal procs` on this pane"
            .into(),
    ))
}
...
fn agent_hook_diagnostics(
    states: &[nermal_core::daemon::control::PaneAgentState],
    machine: &Machine,
    backend: &mut dyn Backend,
...
fn agents_human(
    ...
    for diagnostic in diagnostics {
        ...
        human.push_str(&format!(
            "{} is running, but its nermal agent-status hooks are {state}. Open Settings → Agents to {} the hooks, then start a new {} session.\n",
...
fn doctor(ctx: &Context, backend: &mut dyn Backend) -> Result<Outcome> {
    ...
    match backend.hello() {
        Ok(hello) => {
            ...
            // Without hooks an agent reports no status, which means `nermal
            // agents` shows it standing still and `nermal wait` never wakes. That
    ...
    if ctx.config_dir.is_none() && ctx.pane.is_none() {
        human.push_str(
            "\nnot inside a nermal shell — address commands need an explicit %pane/@tab/workspace\n",
        );
    }
    let report = Report {
        human,
    ...
    if report.json["server"]["reachable"] == false {
        ...
        // `nermal doctor || alert` has to fire (#592). The table and JSON go
        // out all the same, and stderr carries the headline under `-q`.
        eprintln!("nermal: doctor: the server is unreachable");
...
fn hooks_summary(hooks: &[(HookAgent, HooksState)]) -> String {
    ...
    // nermal knows about would bury it. "Up to date" rather than "installed":
...
mod tests {
    ...
    use clap::Parser;
    use nermal_core::core::cli_agent::CLIAgent;
    use nermal_core::core::machine::Tab;
    ...
    fn segment(bytes: &[u8]) -> crate::backend::CaptureSegment {
        crate::backend::CaptureSegment {
            size: nermal_core::daemon::protocol::WinSize {
                cols: 20,
                rows: 10,
                cell_w: 8,
                cell_h: 16,
    ...
    fn pane_info(pane_id: u64, owner: Option<&str>) -> nermal_core::daemon::protocol::PaneInfo {
        nermal_core::daemon::protocol::PaneInfo {
            pane_id,
            cwd: None,
            title: "sh".into(),
            osc_title: None,
    ...
    fn pane_ls_all_surfaces_the_panes_no_workspace_holds() {
        ...
        // interrupted `nermal run` leaves behind.
        backend.registry = vec![
            pane_info(1, None),
            pane_info(3, None),
            pane_info(77, Some("nermal-cli")),
        ];
        ...
        assert_eq!(orphan["owner"], serde_json::json!("nermal-cli"), "{orphan}");
    ...
    fn pane_ls_without_all_cannot_see_an_orphan() {
        let mut backend = mock();
        backend.registry = vec![pane_info(77, Some("nermal-cli"))];
    ...
    fn ls_and_ws_ls_are_the_same_request() {
        ...
        let mut a = mock();
        run_cli(&["nermal", "ls"], &ctx, &mut a);
        let mut b = mock();
        run_cli(&["nermal", "ws", "ls"], &ctx, &mut b);
    ...
    fn ws_rename_rm_attach_detach_build_their_requests() {
        ...
        run_cli(&["nermal", "ws", "rm", "web"], &ctx, &mut backend);
        assert_eq!(
            backend.control_calls[1],
        ...
        let out = run_cli(&["nermal", "ws", "attach", "api"], &ctx, &mut backend);
        assert_eq!(
            backend.control_calls[1],
        ...
        backend.control_calls.clear();
        run_cli(&["nermal", "ws", "detach", "api"], &ctx, &mut backend);
        assert_eq!(
            backend.control_calls[1],
    ...
    fn tab_verbs_resolve_machine_wide_ordinals_to_real_ids() {
        ...
        run_cli(&["nermal", "tab", "close", "@3"], &ctx, &mut backend);
        assert_eq!(
            backend.control_calls,
            vec![
                ControlRequest::MachineGet,
        ...
        backend.control_calls.clear();
        run_cli(&["nermal", "tab", "move", "@2", "0"], &ctx, &mut backend);
        assert_eq!(
            backend.control_calls[1],
    ...
    fn tab_new_uses_the_workspace_from_the_environment() {
        ...
        run_cli(
            &["nermal", "tab", "new", "--cwd", "C:\\elsewhere"],
            &ctx,
            &mut backend,
        );
        assert_eq!(
    ...
    fn tab_new_with_a_pane_takes_an_explicit_workspace_and_cwd() {
        ...
        run_cli(
            &[
                "nermal", "tab", "new", "web", "--pane", "%37", "--cwd", "C:\\else",
            ],
            &Context::default(),
            &mut backend,
        );
    ...
    fn tab_new_refuses_a_pane_that_is_not_running() {
        let mut backend = mock();
        let error = execute(
            cli(&["nermal", "tab", "new", "api", "--pane", "%99"]),
            &Context::default(),
            &mut backend,
        )
    ...
    fn pane_ls_all_points_at_the_way_back_as_well_as_the_way_out() {
        ...
        assert!(out.contains("nermal tab new --pane %<id>"), "{out}");
        assert!(out.contains("nermal pane close --orphans"), "{out}");
    }
    ...
    fn pane_split_builds_the_split_and_spawns_the_new_shell() {
        ...
        let api = backend.machine.workspaces[0].id;
        let out = run_cli(
            &["nermal", "pane", "split", "%2", "--v", "--ratio", "0.3"],
            &Context::default(),
            &mut backend,
        );
        assert_eq!(
    ...
    fn split_without_an_address_uses_the_pane_from_the_environment() {
        ...
        run_cli(&["nermal", "split", "--h"], &ctx, &mut backend);
        let web = backend.machine.workspaces[1].id;
    ...
    fn pane_close_orphans_reaps_only_what_no_workspace_holds() {
        ...
        backend.registry = vec![
            ...
            pane_info(3, None),
            pane_info(77, Some("nermal-cli")),
            pane_info(78, Some("nermal-cli")),
        ];

        let json = json_of(run_cli(
            &["nermal", "pane", "close", "--orphans"],
            &Context::default(),
            &mut backend,
        ));
    ...
    fn send_reaches_the_pane_socket_seam_not_the_control_socket() {
        ...
        backend.sent.clear();
        run_cli(&["nermal", "send", "echo hi"], &ctx, &mut backend);
    ...
    fn send_key_presses_keys_in_order() {
        let mut backend = mock();
        run_cli(
            &["nermal", "send", "%1", "--key", "down", "--key", "enter"],
            &Context::default(),
            &mut backend,
        );
        assert_eq!(
    ...
    /// otherwise `nermal send %42` would silently do nothing at all.
    #[test]
    fn send_still_refuses_a_bare_address_when_there_is_nothing_to_press() {
        ...
        // And outside a nermal shell, with neither text nor keys, the complaint
        // is about the missing input rather than the missing pane.
        let mut backend = mock();
        let err = execute(cli(&["nermal", "send"]), &Context::default(), &mut backend)
    ...
    /// $nermal_PANE the fallback errors OUTSIDE_SHELL before the guard matters
    /// (#538).
    #[test]
    fn a_broken_address_errors_instead_of_typing_into_the_callers_pane() {
        ...
        let mut backend = mock();
        let err = execute(cli(&["nermal", "send", "%42a"]), &ctx, &mut backend)
    ...
    fn a_lone_bare_id_refuses_loudly_rather_than_retargeting() {
        ...
        let mut backend = mock();
        let err = execute(cli(&["nermal", "send", "83"]), &ctx, &mut backend)
    ...
    fn enter_alone_presses_enter_at_the_address_it_was_given() {
        ...
        let mut backend = mock();
        run_cli(&["nermal", "send", "--enter"], &ctx, &mut backend);
        ...
        let mut backend = mock();
        run_cli(
            &["nermal", "send", "83", "--key", "enter"],
            &ctx,
            &mut backend,
        );
        assert_eq!(backend.sent, vec![(83, b"\r".to_vec())]);
        let mut backend = mock();
        run_cli(&["nermal", "send", "%83", "--enter"], &ctx, &mut backend);
        assert_eq!(backend.sent, vec![(83, b"\r".to_vec())]);
    }
    ...
    fn percent_led_text_still_types_into_the_current_pane() {
        ...
        let mut backend = mock();
        run_cli(&["nermal", "send", "%s/a/b/"], &ctx, &mut backend);
        assert_eq!(backend.sent, vec![(5, b"%s/a/b/".to_vec())]);

        let mut backend = mock();
        run_cli(&["nermal", "send", "%!sort", "--enter"], &ctx, &mut backend);
        assert_eq!(
            backend.sent,
        ...
        for text in ["3x", "+5", "50%"] {
            let mut backend = mock();
            run_cli(&["nermal", "send", text], &ctx, &mut backend);
            assert_eq!(
                backend.sent,
    ...
    fn capture_tail_keeps_the_last_lines_of_either_form() {
        ...
        let all = human(run_cli(
            &["nermal", "capture", "%2", "--plain", "--tail", "99"],
            &Context::default(),
            &mut backend,
        ));
        assert_eq!(all, "one\ntwo\nthree\nfour");
    ...
    fn run_passes_the_command_and_its_exit_code_through() {
        ...
        let out = execute(
            cli(&["nermal", "run", "--keep", "--", "cargo", "test"]),
            &ctx,
            &mut backend,
        )
        .unwrap();
    ...
    fn run_answers_json_even_though_it_carries_an_exit_code() {
        ...
        let out = execute(
            cli(&["nermal", "run", "--json", "--", "cargo", "test"]),
            &ctx,
            &mut backend,
        )
        .unwrap();
    ...
    fn a_machine_flag_refuses_the_local_server_verbs() {
        for verb in ["start", "stop", "restart", "logs"] {
            let mut backend = mock();
            let err = execute(
                cli(&["nermal", "-m", "devbox", "server", verb]),
                &Context::default(),
                &mut backend,
            )
    ...
    fn a_mistyped_subcommand_is_named_as_one_not_offered_to_the_gui() {
        for typo in ["tree", "statu", "pnae", "workspace"] {
            let err = execute(cli(&["nermal", typo]), &Context::default(), &mut mock())
    ...
    fn a_path_asks_the_running_gui_to_open_an_absolute_directory() {
        ...
        let out = run_cli(&["nermal", "."], &Context::default(), &mut backend);
    ...
    fn an_invalid_gui_path_fails_before_touching_the_wire() {
        let mut backend = mock();
        let missing =
            std::env::temp_dir().join(format!("nermal-cli-missing-path-{}", std::process::id()));
        let arg = missing.to_str().unwrap().to_owned();
        let err = execute(cli(&["nermal", &arg]), &Context::default(), &mut backend)
    ...
    fn a_non_utf8_gui_path_stays_off_the_string_protocol() {
        ...
        let path = std::env::temp_dir().join(OsString::from_vec(b"nermal-\xff".to_vec()));
    ...
    fn the_still_missing_verbs_say_so_without_touching_the_wire() {
        for (args, needle) in [
            (vec!["nermal", "ws", "stop", "api"], "not implemented"),
            (
                vec!["nermal", "machine", "connect", "devbox"],
                "not implemented",
            ),
            (
                vec!["nermal", "machine", "disconnect", "devbox"],
                "not implemented",
            ),
        ] {
            let mut backend = mock();
    ...
    fn agent_state(
        pane_id: u64,
        status: nermal_core::core::cli_agent::AgentStatus,
    ) -> nermal_core::daemon::control::PaneAgentState {
        agent_state_at(pane_id, status, 0)
    }

    /// A pane sitting at its prompt: the shell, and nothing in front of it.
    fn idle_procs() -> nermal_core::daemon::protocol::PaneProcs {
        nermal_core::daemon::protocol::PaneProcs {
    ...
    fn busy_procs() -> nermal_core::daemon::protocol::PaneProcs {
        nermal_core::daemon::protocol::PaneProcs {
            procs: vec![
    ...
    fn local_context(at_prompt: Option<bool>) -> nermal_core::daemon::protocol::PaneContext {
        nermal_core::daemon::protocol::PaneContext {
            remote: None,
            local_pty: true,
            at_prompt,
    ...
    fn ssh_procs(
        ...
        use nermal_core::daemon::protocol::{RemoteContext, RemoteKind};
        nermal_core::daemon::protocol::PaneProcs {
            procs: vec![
                proc_entry(100, "zsh", 0, false),
            ...
            ports: Vec::new(),
            probe: Default::default(),
            context: Some(nermal_core::daemon::protocol::PaneContext {
    ...
    /// A pane routed to a remote nermal daemon: no pty on this machine at all,
    /// so the process list is empty by construction rather than by failure.
    fn routed_procs(at_prompt: Option<bool>) -> nermal_core::daemon::protocol::PaneProcs {
        use nermal_core::daemon::protocol::{RemoteContext, RemoteKind};
        nermal_core::daemon::protocol::PaneProcs {
            procs: Vec::new(),
            ports: Vec::new(),
    ...
    fn proc_entry(
        ...
        depth: u8,
        foreground: bool,
    ) -> nermal_core::daemon::protocol::ProcEntry {
        nermal_core::daemon::protocol::ProcEntry {
            pid,
            name: name.into(),
            depth,
    ...
    fn agent_state_at(
        ...
        nermal_core::daemon::control::PaneAgentState {
            pane_id,
            agent: None,
            state: nermal_core::core::cli_agent::AgentSessionState {
                status,
    ...
    fn detect_agent(backend: &mut MockBackend, pane_id: u64, agent: CLIAgent) {
        use nermal_core::core::machine::AgentFacts;
    ...
    fn wait_returns_the_moment_the_state_matches() {
        ...
        let out = run_cli(&["nermal", "wait", "%3"], &Context::default(), &mut backend);
        let json = json_of(out);
    ...
    fn wait_reads_agentless_panes_from_the_tree() {
        ...
        let mut backend = mock();
        backend.replies.push_back(ReplyOk::AgentStates(Vec::new()));
        let out = run_cli(&["nermal", "wait", "%9"], &Context::default(), &mut backend);
        let json = json_of(out);
        assert_eq!(json["status"], "exit");
        assert_eq!(json["matched"], true);
    ...
    fn wait_does_not_call_a_busy_shell_idle() {
        ...
        let out = execute(
            cli(&["nermal", "wait", "%3", "--until", "idle", "--timeout", "0"]),
            &Context::default(),
            &mut backend,
        )
        .expect("a timeout is an exit code, not an error");
    ...
    fn wait_free_does_not_overrule_a_state_the_caller_asked_for() {
        ...
        let json = json_of(run_cli(
            &["nermal", "wait", "%3", "--until", "waiting,free"],
            &Context::default(),
            &mut backend,
        ));
    ...
    fn wait_free_does_not_read_an_empty_process_tree_as_finished() {
        ...
        let out = execute(
            cli(&["nermal", "wait", "%3", "--until", "free", "--timeout", "0"]),
            &Context::default(),
            &mut backend,
        )
        .expect("a timeout is an exit code, not an error");
    ...
    fn wait_timeout_still_points_an_agentless_wait_at_free() {
        ...
        let out = execute(
            cli(&["nermal", "wait", "%3", "--until", "done", "--timeout", "0"]),
            &Context::default(),
            &mut backend,
        )
        .expect("a timeout is an exit code, not an error");
    ...
    fn wait_only_reads_the_process_tree_when_free_is_asked_for() {
        ...
        run_cli(&["nermal", "wait", "%3"], &Context::default(), &mut backend);
        assert!(
            backend.procs_calls.is_empty(),
    ...
    fn agents_distinguishes_no_agent_from_a_healthy_agent() {
        ...
        let mut backend = mock();
        backend.replies.push_back(ReplyOk::AgentStates(Vec::new()));
        let out = run_cli(&["nermal", "agents"], &Context::default(), &mut backend);
        assert_eq!(
            backend.control_calls,
    ...
    fn agents_reports_outdated_hooks_with_the_update_action() {
        ...
        let report = match run_cli(&["nermal", "agents"], &Context::default(), &mut backend) {
    ...
    fn status_and_machine_ls_are_single_aggregate_requests() {
        use nermal_core::daemon::control::{RouteInfo, ServerStatus};
        ...
        let out = run_cli(&["nermal", "status"], &Context::default(), &mut backend);
    ...
    fn doctor_backend() -> MockBackend {
        use nermal_core::daemon::control::ServerStatus;
    ...
    fn doctor_reports_the_injected_context_and_the_server_half() {
        ...
        assert!(out.contains("nermal_CONFIG_DIR"), "{out}");
        ...
        let ctx = Context {
            pane: Some("7".into()),
            ws: None,
            config_dir: Some("/cfg/nermal".into()),
        };
        let out = human(run_cli(&["nermal", "doctor"], &ctx, &mut doctor_backend()));
        assert!(out.contains("set (/cfg/nermal)"), "{out}");
    }

    /// Missing hooks are the reason a perfectly healthy-looking agent never
    /// reports and `nermal wait` sits there until it times out. `doctor` is the
    ...
    fn doctor_reports_where_the_agent_status_hooks_stand() {
        use nermal_core::core::agent_hooks::HookAgent;
        ...
        backend.agent_hooks_states = HookAgent::ALL
            ...
            .collect();
        let out = run_cli(&["nermal", "doctor"], &Context::default(), &mut backend);
    ...
    /// exits non-zero over it — `nermal doctor || alert` has to fire — while
    ...
    fn doctor_exits_nonzero_when_the_server_is_unreachable() {
        ...
        assert!(r.human.contains("nermal_CONFIG_DIR"), "{}", r.human);
