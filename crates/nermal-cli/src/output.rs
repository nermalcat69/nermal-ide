...
use nermal_core::core::machine::{Machine, PaneNode, Workspace};
use nermal_core::core::session::WorkspaceId;
use nermal_core::core::tab_view::{TabLabel, TabView, strip_host_prefix, tab_views_of};
use nermal_core::daemon::control::{PaneAgentState, RouteInfo, ServerStatus};
use nermal_core::daemon::protocol::{PaneInfo, PaneProcs, PortProbe};
...
pub fn workspace_table(machine: &Machine) -> String {
    if machine.workspaces.is_empty() {
        return "no workspaces — `nermal new <path>` starts one\n".to_string();
    }
...
fn render_node(out: &mut String, node: &PaneNode, machine: &Machine, depth: usize) {
    ...
    match node {
        ...
        PaneNode::Split { axis, ratio, a, b } => {
            let axis = match axis {
                nermal_core::core::machine::Axis::Horizontal => "h",
                nermal_core::core::machine::Axis::Vertical => "v",
            };
...
///
/// This is the line that makes `nermal procs` a diagnostic rather than another
...
mod tests {
    ...
    use nermal_core::daemon::protocol::{PortEntry, ProcEntry};
    ...
    fn an_empty_machine_says_how_to_start() {
        ...
        assert!(rendered.contains("nermal new"), "{rendered}");
    }
    ...
    fn an_unnamed_tab_borrows_its_title_then_an_agent_then_a_place_then_its_process() {
        ...
        assert_eq!(
            tab_label(&view(
                &|v| v.agent = Some(nermal_core::core::cli_agent::CLIAgent::Claude)
            )),
            "Claude Code"
        );
        ...
        assert_eq!(
            tab_label(&view(&|v| {
                ...
                v.agent = Some(nermal_core::core::cli_agent::CLIAgent::Claude);
            })),
            "fixing the switcher"
        );
        assert_eq!(
            tab_label(&view(
                &|v| v.osc_title = Some("user@host:~/repo/nermal".into())
            )),
            "nermal",
        ...
        assert_eq!(
            tab_label(&view(&|v| v.cwd = Some("/Users/me/repo/nermal".into()))),
            "nermal"
        );
        assert_eq!(
    ...
    /// #731's diagnostic. Someone whose ports are missing runs `nermal procs`
