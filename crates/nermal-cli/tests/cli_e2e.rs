...
use nermal_core::client::{ControlClient, PaneClient};
use nermal_core::daemon::control::ControlHello;
use nermal_core::daemon::protocol::{DaemonMsg, PROTOCOL_VERSION, ShellSpec, WinSize};

const DAEMON_ENV: &str = "nermal_CLI_E2E_DAEMON";
const PASTE_AWARE_FIXTURE_ARG: &str = "--nermal-e2e-paste-aware-fixture";
const PASTE_AWARE_TEXT: &str = "nermal paste aware input";
...
fn main() {
    ...
    if std::env::var(DAEMON_ENV).as_deref() == Ok("1") {
        if let Err(e) = nermal_core::daemon::server::run_daemon() {
...
fn run_paste_aware_fixture() {
    ...
    println!("nermal_PASTE_AWARE_READY");
    ...
    loop {
        ...
        for &byte in &chunk[..read] {
            if byte == b'\r' {
                ...
                if submitted {
                    println!("nermal_PASTE_AWARE_SUBMITTED");
                } else {
                    println!("nermal_PASTE_AWARE_NOT_SUBMITTED");
                }
...
impl Daemon {
    fn start() -> Daemon {
        ...
        let child = Command::new(own)
            .env(DAEMON_ENV, "1")
            .env("nermal_CONFIG_DIR", dir.path())
            .env("nermal_DATA_DIR", dir.path())
            // The shell integration's re-entrancy guard. A test run started
            // from inside a nermal pane would otherwise hand it to every pane
            ...
            .env_remove("nermal_SHELL_INTEGRATION")
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
            .expect("start the in-test nermal server");
        #[cfg(windows)]
    ...
    fn cli(&self, args: &[&str]) -> Command {
        let mut cmd = Command::new(env!("CARGO_BIN_EXE_nermal"));
        cmd.args(args)
            .env("nermal_CONFIG_DIR", self.dir.path())
            .env("nermal_DATA_DIR", self.dir.path())
            .env_remove("nermal_PANE")
            .env_remove("nermal_WS")
            .env_remove("nermal_SOCKET");
        cmd
    }
    ...
    fn run(&self, args: &[&str]) -> Output {
        self.cli(args)
            .output()
            .unwrap_or_else(|e| panic!("could not run nermal {args:?}: {e}"))
    }
    ...
    fn run_json(&self, args: &[&str]) -> serde_json::Value {
        ...
        serde_json::from_str(&out)
            .unwrap_or_else(|e| panic!("nermal {args:?} --json printed no JSON ({e}): {out}"))
    }
}
...
/// "nermal-cli", which left a CLI-built workspace rebuilt from scratch the
...
fn run_streams_output_and_passes_the_exit_code(daemon: &Daemon) {
    let echo = one_shot("echo nermal_e2e_run_marker");
    ...
    assert!(stdout.contains("nermal_e2e_run_marker"), "{stdout}");
...
fn run_keep_files_the_pane_so_ls_shows_it(daemon: &Daemon) {
    ...
    args.extend(one_shot("echo nermal_e2e_keep_marker"));
...
fn config_dir_alone_resolves_both_endpoints(daemon: &Daemon) {
    let out = Command::new(env!("CARGO_BIN_EXE_nermal"))
        .args(["status", "--json"])
        .env_remove("nermal_DATA_DIR")
        .env_remove("nermal_CONTROL_SOCK")
        .env_remove("nermal_PANE")
        .env_remove("nermal_WS")
        .env_remove("nermal_SOCKET")
        .env("nermal_CONFIG_DIR", daemon.dir.path())
        .output()
        .expect("run nermal status with only nermal_CONFIG_DIR");
    assert!(
        out.status.success(),
        "status over nermal_CONFIG_DIR failed ({}): {}",
        out.status,
        String::from_utf8_lossy(&out.stderr)
    );
    ...
    assert_eq!(
        ...
        "the answer must come from the daemon nermal_CONFIG_DIR names: {status}"
    );
    ...
    let out = Command::new(env!("CARGO_BIN_EXE_nermal"))
        ...
        .env("nermal_CONFIG_DIR", daemon.dir.path())
        .output()
        .expect("run nermal run with only nermal_CONFIG_DIR");
    assert_eq!(
        out.status.code(),
        Some(9),
...
fn send_then_capture_round_trip(daemon: &Daemon) {
    ...
    loop {
        ...
        if seen.contains("nermal_e2e_capture_marker") {
            return;
        }
        assert!(
            Instant::now() < deadline,
...
fn send_enter_submits_in_a_paste_aware_raw_mode_tui(daemon: &Daemon) {
    ...
    collect_pane_output_until(&mut pane, b"nermal_PASTE_AWARE_READY");

    let address = format!("%{}", pane.pane_id());
    daemon.run_ok(&["send", &address, PASTE_AWARE_TEXT, "--enter"]);

    collect_pane_output_until(&mut pane, b"nermal_PASTE_AWARE_SUBMITTED");
}

fn collect_pane_output_until(session: &mut nermal_core::client::PaneSession, marker: &[u8]) {
...
/// `nermal … | head -1` must end the way `cat … | head -1` ends. Rust ignores
...
fn a_reader_that_hung_up_ends_the_pipeline_quietly(daemon: &Daemon) {
    ...
    let printer = one_shot("echo nermal_e2e_pipe_marker");
    ...
    for args in [vec!["ls"], streaming] {
        let mut child = daemon
            ...
            .stderr(Stdio::piped())
            .spawn()
            .unwrap_or_else(|e| panic!("could not spawn nermal {args:?}: {e}"));
        ...
        child
            ...
            .expect("reading nermal's stderr");
        let status = child.wait().expect("waiting for nermal");

        assert!(
            !stderr.contains("panicked"),
            "nermal {args:?} panicked when its reader hung up: {stderr}"
        );
        assert!(
            !stderr.to_lowercase().contains("broken pipe"),
            "a hung-up reader is how a pipeline ends, not something to report: \
             nermal {args:?} said {stderr}"
        );
        ...
        assert_ne!(
            status.code(),
            Some(1),
            "nermal {args:?} treated a hung-up reader as a failure: {stderr}"
        );
    }
}
...
fn capture_plain_returns_text_not_escapes(daemon: &Daemon) {
    ...
    daemon.run_ok(&["send", &address, "echo nermal_e2e_plain_marker", "--enter"]);
    ...
    loop {
        ...
        if plain.contains("nermal_e2e_plain_marker") && raw.contains('\r') {
            assert!(
                !plain.contains('\r'),
...
fn capture_still_answers_after_a_resize(daemon: &Daemon) {
    ...
    daemon.run_ok(&["send", &address, "echo nermal_e2e_resize_marker", "--enter"]);
    let deadline = Instant::now() + SETTLE_WITHIN;
    loop {
        if daemon
            .run_ok(&["capture", &address, "--scrollback"])
            .contains("nermal_e2e_resize_marker")
        {
            break;
        }
        assert!(
    ...
    loop {
        ...
        let held_still = before["text"] == after["text"]
            && whole.contains("nermal_e2e_resize_marker")
            && plain.contains("nermal_e2e_resize_marker");
        if held_still {
            assert!(
...
fn capture_tail_trims_a_real_panes_answer(daemon: &Daemon) {
    ...
    for n in 1..=6 {
        let line = format!("echo nermal_e2e_tail_line_{n}");
    ...
    loop {
        ...
        let settled = before == after
            && before.contains("nermal_e2e_tail_line_1")
            && before.contains("nermal_e2e_tail_line_6")
            && tail_text.contains("nermal_e2e_tail_line_6");
        if settled {
            assert!(
            ...
            assert!(
                !tail_text.contains("nermal_e2e_tail_line_1"),
