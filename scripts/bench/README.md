...
Reproducible throughput/FPS/memory benchmarks for nermal against Alacritty,
...
| **Memory** | cold launch, default shell, idle 6 s | RSS in KB (nermal = GUI + daemon) |
...
cargo build --release                   # the nermal under test

scripts/bench/run_one.sh nermal io        # ALWAYS first: records the reference grid
scripts/bench/run_one.sh alacritty io   # matches nermal's grid automatically
scripts/bench/run_one.sh ghostty io
scripts/bench/run_one.sh kitty io
scripts/bench/run_one.sh nermal fire      # same order for the fire test
...
`$nermal_BENCH_DIR`, the nermal binary with `$nermal_BIN`).
...
- **nermal**: an isolated `--config-dir` under the work dir whose `config.json`
  sets `shell` to the script. GUI + daemon are launched fresh and killed
  (by config-dir-scoped `pkill`) after the run — a daily-driver nermal and its
...
Grid fairness: nermal has no size flag, so its default window is the reference —
run nermal first, and the driver reads the recorded `grid:` line to size the
...
| Test | nermal (before) | nermal (after) | Alacritty | Ghostty | Kitty |
| --- | ---: | ---: | ---: | ---: | ---: |
...
"run 1 fast, later runs slow" (the ring fills during run 1). `nermal_TRACE=1`
...
¹ The nermal-before numbers were measured while the machine was busy (builds +
...
- **The day's fps band matters more than the run.** The same pre-pass binary
  ...
  reproduced within ±3%. nermal's fire number is drain-rate-bound and therefore
  sensitive to ambient machine load in a way the (slower) competitors aren't.
  Only compare nermal-vs-nermal fire numbers from the same session.
- **DOOM-fire is producer-bound, not terminal-bound, at this level.** Under a
  ...
  60 MB/s → 690 fps). nermal's steady seconds already drain at 93–98 MB/s; the
