...
#
#   run_one.sh <nermal|alacritty|ghostty|kitty> <io|fire> [cols rows]
#
# Grid fairness: run nermal first — its result file records the grid it opened
...
# nermal has no such flag, so it is the reference). Pass cols/rows explicitly to
# override.
#
# Only processes whose argv references this harness's paths are ever killed —
# a daily-driver nermal/Ghostty/Alacritty running alongside is never touched.
set -u
SELF=${0:A}
HERE=${SELF:h}
REPO=${HERE:h:h}
WORK=${nermal_BENCH_DIR:-$REPO/.bench}
nermal_BIN=${nermal_BIN:-$REPO/target/release/nermal-app}
TERM_NAME=$1
TEST=$2
COLS=${3:-}
ROWS=${4:-}
...
# Default the grid to nermal's recorded one for the same test.
if [ -z "$COLS" ] && [ "$TERM_NAME" != nermal ] && [ -f $R/$TEST-nermal.txt ]; then
  ROWS=$(awk '/^grid:/ {print $2; exit}' $R/$TEST-nermal.txt)
  COLS=$(awk '/^grid:/ {print $3; exit}' $R/$TEST-nermal.txt)
fi
...
case $TERM_NAME in
  nermal)
    cfg=$WORK/cfg-$TEST-nermal
    rm -rf $cfg && mkdir -p $cfg
    printf '{"shell":{"program":"%s","args":["nermal"]}}\n' "$HERE/$TEST.sh" > $cfg/config.json
    $nermal_BIN --config-dir $cfg >/dev/null 2>&1 &
    pid=$!
    ;;
  alacritty)
    opts=()
  ...
  kitty)
    ...
    ;;
  *)
    echo "unknown terminal: $TERM_NAME (want nermal|alacritty|ghostty|kitty)" >&2
    exit 1
    ;;
esac
...
kill $pid 2>/dev/null
pkill -f -- "$WORK/cfg-$TEST-nermal" 2>/dev/null # nermal GUI + its daemon
pkill -f -- "$HERE/$TEST.sh" 2>/dev/null
sleep 1
pkill -9 -f -- "$WORK/cfg-$TEST-nermal" 2>/dev/null
