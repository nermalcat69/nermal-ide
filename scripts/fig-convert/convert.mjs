// Fig autocomplete spec -> nermal signature JSON converter.
//
...
//
// The output schema mirrors nermal's `terminal::completion::spec` serde model
// one-to-one. Runtime JS (postProcess) is intentionally lost: nermal runs a
...
// last. `postProcess` (how to parse output) is a fn we always drop; nermal
...
function normArg(a) {
  ...
  return {
    ...
    template: strList(a.template), // "filepaths" | "folders" -> nermal path completion
